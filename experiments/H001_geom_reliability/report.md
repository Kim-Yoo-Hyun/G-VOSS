...
- Qwen-VL contract status: `full_source_inference_remaining_loop_running_non_metric`; runtime smoke status `tiny_inference_smoke_passed_non_metric`; full-source promotion plan `full_source_promotion_plan_frozen_no_metric_run`; full-source input audit `full_source_input_ready_with_missing_rows_no_inference`; full-source crop status `full_source_crop_preflight_ready_no_inference`; inference runner plan `full_source_inference_runner_frozen_no_inference`; remaining shard loop `running_non_metric`. This is non-metric third-source modern-VLM extension evidence, not a VL-SAT/Open3DSG replacement.
