#!/usr/bin/env python3
"""Freeze post-fit H001 protocols around the no-family-indicator model lock."""
...
ROOT_REL = Path("experiments/H001_geom_reliability/no_family_indicator_v1")
...
def freeze_initial(root: Path, exp: Path, protocols: Path) -> list[Path]:
    ...
    structured = copy.deepcopy(
        load(root / "experiments/H001_geom_reliability/structured_main_v1/protocol.json")
    )
    ...
    routing = copy.deepcopy(
        load(root / "experiments/H001_geom_reliability/support_contact_routing_v1/protocol.json")
    )
    ...
    open_route = copy.deepcopy(
        load(root / "experiments/H001_geom_reliability/open3dsg_official_route_v1/protocol.json")
    )
    ...
    nonlinear = copy.deepcopy(
        load(root / "experiments/H001_geom_reliability/supervision_matched_nonlinear_v1/protocol.json")
    )
    ...
    for path, payload in zip(outputs, (structured, routing, open_route, nonlinear)):
        write(path, payload)
    freeze = {
        "schema_version": "h001_no_family_indicator_initial_protocol_freeze_v1",
...
def freeze_downstream(root: Path, exp: Path, protocols: Path) -> list[Path]:
    ...
    comparator = copy.deepcopy(
        load(root / "experiments/H001_geom_reliability/routed_comparators_v1/protocol.json")
    )
    ...
    ablation = copy.deepcopy(
        load(root / "experiments/H001_geom_reliability/structured_ablation_v1/routed_public_full_protocol.json")
    )
    ...
    scan = copy.deepcopy(
        load(root / "experiments/H001_geom_reliability/support_contact_routing_v1/scan_cluster_protocol.json")
    )
    ...
    structured_scan = copy.deepcopy(
        load(root / "experiments/H001_geom_reliability/structured_main_v1/scan_cluster_protocol.json")
    )
    ...
    surface = copy.deepcopy(
        load(root / "experiments/H001_geom_reliability/orthogonal_geometry_audit_v1/protocol.json")
    )
    ...
    heldout = copy.deepcopy(
        load(root / "experiments/H001_geom_reliability/held_out_primitive_v1/protocol.json")
    )
    ...
    counterfactual = copy.deepcopy(
        load(root / "experiments/H001_geom_reliability/counterfactual_sensitivity_v1/protocol.json")
    )
    ...
    for path, payload in zip(
        ...
        write(path, payload)
    freeze = {
        "schema_version": "h001_no_family_indicator_downstream_protocol_freeze_v1",
