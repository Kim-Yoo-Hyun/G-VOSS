...
METHODS = ("source_score", "family_slot_rerank")
...
def summarize(values: dict[str, Any], weights: np.ndarray) -> dict[str, Any]:
    ...
    for k in base.KS:
        ...
        for metric in METRICS:
            ...
            delta = cache["family_slot_rerank"][str(k)][metric] - cache["source_score"][str(k)][metric]
...
def main() -> int:
    ...
    if protocol.get("status") != "frozen_before_routing_scan_cluster_sensitivity":
    ...
    if base.sha256_file(paths["structured_models"]) != protocol["model_sha256"]:
    ...
    scorer = base.make_structured_scorer(models)
    gt, gt_family = base.strict.load_gt(paths["final_ground_truth"])
    source_paths = {
    ...
    try:
        for index, (source, path) in enumerate(source_paths.items()):
            grouped, _ = base.load_candidates(path, scorer, strict_models)
            ...
            for method in METHODS:
                for k in base.KS:
                    for metric in METRICS:
                        if source == "open3dsg":
                            expected = open_summary["routes"]["official_strict_full_548"]["overall"][method][str(k)][metric]["point"]
                        else:
            ...
            validations["k100_recall_ci_positive"] &= k100["recall"]["paired_scan_cluster_ci95"][0] > 0.0
            validations["k100_violation_ci_below_zero"] &= k100["violation_all"]["paired_scan_cluster_ci95"][1] < 0.0
    finally:
        base.METHODS = original_methods
    status = "completed" if all(validations.values()) else "failed_validation"
    payload = {
        "schema_version": "h001_support_routing_scan_cluster_v1",
        ...
        "claim_boundary": "dependence sensitivity for the internal-development-selected applicability route",
    }
    ...
    for source in ("vlsat", "open3dsg", "sgfn"):
        ...
        lines.append(
            f"| {source} | {dr['point']:+.4f} [{dr['paired_scan_cluster_ci95'][0]:+.4f}, {dr['paired_scan_cluster_ci95'][1]:+.4f}] | "
            f"{dv['point']:+.4f} [{dv['paired_scan_cluster_ci95'][0]:+.4f}, {dv['paired_scan_cluster_ci95'][1]:+.4f}] |"
        )
    (out / "summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    base.write_json(out / "manifest.json", {
        "schema_version": "h001_support_routing_scan_cluster_manifest_v1",
        "status": status,
        ...
        "validations": validations,
        "docker_command": "env UID=$(id -u) GID=$(id -g) docker compose -f configs/h001/compose.structured.yaml run --rm support_routing_scan_cluster",
    })
