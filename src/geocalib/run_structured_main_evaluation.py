#!/usr/bin/env python3
"""Evaluate the promoted train-only relation-algebra compatibility on 3DSSG."""
...
import run_relation_algebra_development as algebra
import run_train_only_evaluation as strict
...
METHODS = (
    "source_score",
    "structured_product",
    "structured_rank_average",
    "structured_rrf_c60",
    "pooled_product",
    "hard_rule_filter",
    "family_product_continuity",
    "structured_compatibility_only",
)
RATIO_METRICS = (
    "recall",
    "violation_all",
...
def make_structured_scorer(models: dict[str, Any]) -> Callable[[str, str, dict[str, float]], float]:
...
def load_candidates(
    path: Path,
    structured_score: Callable[[str, str, dict[str, float]], float],
    ...
    family_models = strict_models["family_models"]
    pooled_model = strict_models["factor_models"]["M_int"]
    with path.open("rb") as handle:
        for raw_line in handle:
            ...
            structured = structured_score(family, predicate, raw)
            previous = strict.probability(family_models[family], family, predicate, raw)
            pooled = strict.probability(pooled_model, family, predicate, raw)
            transformed = algebra.transformed_view(family, predicate, raw)
            if transformed is not None:
                transformed_predicate, transformed_raw = transformed
                error = abs(structured - structured_score(family, transformed_predicate, transformed_raw))
    ...
    for candidates in grouped.values():
        ...
        structured_order = sorted(candidates, key=lambda item: (-item["structured"], item["key"]))
        ...
        for item in candidates:
            ...
            item["scores"] = {
                ...
                "structured_product": item["semantic"] * item["structured"],
                "structured_rank_average": 0.5 * (q_z + q_c),
                "structured_rrf_c60": 1.0 / (60 + rank_z) + 1.0 / (60 + rank_c),
                ...
                "structured_compatibility_only": item["structured"],
            }
    ...
    for method in METHODS[1:]:
        ...
        for k in KS:
            ...
            for metric in RATIO_METRICS:
                ...
                right = report["source_score"][str(k)][metric]["point"]
                delta = (
            ...
            for metric in ("recall", "violation_all"):
                ...
                for family in FAMILIES:
                    ...
                    boot = (
                        ...
                        - caches[family]["source_score"][str(k)][metric]
                    )
...
def main() -> int:
    ...
    if protocol.get("status") != "frozen_before_structured_main_re_evaluation":
    ...
    if sha256_file(paths["structured_models"]) != protocol["main_compatibility"]["model_bundle_sha256"]:
        raise ValueError("structured_model_hash_mismatch")
    ...
    if strict_models.get("schema_version") != "h001_strict_train_only_calibrators_v1":
        raise ValueError("strict_models_not_train_only")
    train = read_scans(paths["train_scans"])
    dev = read_scans(paths["internal_dev_scans"])
    ...
    structured_score = make_structured_scorer(structured_models)
    ...
    for source_index, (source, path) in enumerate(source_paths.items()):
        grouped, load_info = load_candidates(path, structured_score, strict_models)
    ...
    validations = {
        ...
        "strict_model_schema": strict_models.get("schema_version") == "h001_strict_train_only_calibrators_v1",
        "structured_model_hash_locked": sha256_file(paths["structured_models"]) == protocol["main_compatibility"]["model_bundle_sha256"],
        "structured_model_excludes_source": structured_models.get("source_score_used") is False and structured_models.get("source_identity_used") is False,
        "all_methods_reported": all(
            set(payload["overall"]) - {"deltas_vs_source_score"} == set(METHODS)
            for payload in source_results.values()
        ),
        "all_k_reported": all(
            set(payload["overall"]["structured_product"]) == {str(k) for k in KS}
            for payload in source_results.values()
        ),
    ...
    summary = {
        "schema_version": "h001_structured_main_evaluation_v1",
        ...
        "docker_command": "env UID=$(id -u) GID=$(id -g) docker compose -f configs/h001/compose.yaml run --rm structured_main_evaluation",
    }
    ...
    manifest = {
        "schema_version": "h001_structured_main_manifest_v1",
