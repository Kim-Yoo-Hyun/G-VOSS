...
import run_structured_main_evaluation as base
import run_support_contact_routing as routing
...
def evaluate(
    ...
    elif context_policy == "full_official_validation":
...
def main() -> int:
    ...
    if protocol.get("status") != "frozen_before_official_route_sensitivity":
    ...
    routes = {
        "official_eligible_533": evaluate(paths["official_verification"], paths["ground_truth"], structured_score, strict_models, "public_pipeline_eligible", context_universe, seed, resamples),
        "official_strict_full_548": evaluate(paths["official_verification"], paths["ground_truth"], structured_score, strict_models, "full_official_validation", context_universe, seed, resamples),
        "recovered_full_548": evaluate(paths["recovered_verification"], paths["ground_truth"], structured_score, strict_models, "full_official_validation", context_universe, seed, resamples),
    }
    validations = {
        ...
        "strict_full_has_15_zero_prediction_contexts": routes["official_strict_full_548"]["counts"]["evaluation_contexts"] == 548 and routes["official_strict_full_548"]["counts"]["zero_prediction_contexts"] == 15,
        ...
        "full_routes_gt_denominator_3972": routes["official_strict_full_548"]["counts"]["gt_denominator"] == routes["recovered_full_548"]["counts"]["gt_denominator"] == 3972,
    ...
    summary = {
        "schema_version": "h001_open3dsg_official_route_sensitivity_v1",
        ...
        "route_interpretation": {
            ...
            "official_strict_full_548": "common-target sensitivity retaining all official GT contexts and assigning no predictions to the 15 public-pipeline drops",
    ...
    for route_name, route in routes.items():
        for method in ("source_score", "family_slot_rerank", "structured_product"):
            for k in KS:
