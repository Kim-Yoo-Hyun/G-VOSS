#!/usr/bin/env python3
"""Docker wrapper for Open3DSG train/validation-split preprocessing."""
...
def parse_args() -> argparse.Namespace:
    ...
    if args.output_dir is None:
        label = "train_preprocess" if args.split == "train" else f"{args.split}_preprocess"
...
def main() -> int:
    root = repo_root()
    tools_dir = root / "src/geocalib"
    sys.path.insert(0, str(tools_dir))
    import run_open3dsg_preprocess  # noqa: PLC0415

    args = parse_args()
    forwarded = [
        "run_open3dsg_preprocess.py",
        "--open3dsg-source",
    ...
    if args.deep_inspect:
        forwarded.append("--deep-inspect")

    sys.argv = forwarded
    return run_open3dsg_preprocess.main()
