#!/usr/bin/env python3
"""Export H001 calibration rows and high-margin counterfactual negatives."""
...
DEFAULT_OUTPUT_DIR = H001_ROOT / "artifacts" / "calibration" / "mini_schema_smoke"

ROW_SCHEMA_VERSION = "h001_calibration_row_v1"
MANIFEST_SCHEMA_VERSION = "h001_calibration_manifest_v1"
NEGATIVE_SCHEMA_VERSION = "h001_counterfactual_negative_v1"
EXPORTER_VERSION = "v0"
NEGATIVE_POLICY_VERSION = "h001-counterfactual-v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export H001 calibration table rows for schema smoke checks."
    )
...
def notes_for_split(split_name: str) -> list[str]:
    ...
    if split_name == "mini_schema_smoke":
        return [
            "H001-Mini export is schema smoke validation only; do not use it as calibration train/dev data.",
            *notes,
        ]
    if split_name == "train_dev_calib":
        return [
            "H001-Calib-Pilot train/dev export is fitting input; assign train/dev roles from 25_pilot.md scan lists.",
            "Do not mix this export with held-out H001-Mini validation scans.",
            *notes,
        ]
    return [
