#!/usr/bin/env python3
"""Freeze H001 factor-isolation diagnostics and audit current-score equivalence."""
...
SCHEMA_VERSION = "h001_factor_isolation_protocol_v1"
...
TRAIN_SCANS_PATH = Path(
    "archive/hypothesis_records/hypothesis/CAND-001/"
    "H001_geometry-grounded-verification/artifacts/subset/"
    "h001_calib_pilot/train_scans.txt"
)
DEV_SCANS_PATH = Path(
    "archive/hypothesis_records/hypothesis/CAND-001/"
    "H001_geometry-grounded-verification/artifacts/subset/"
    "h001_calib_pilot/dev_scans.txt"
)
...
SOURCE_SPECS = {
    ...
    "sgfn_official_full_l160": {
        ...
        "verification": Path(
            "experiments/H001_geom_reliability/sources/sgfn/geometry/verification.jsonl"
        ),
        "expected_rows": 957008,
        "expected_in_scope_rows": 220848,
        "expected_subgraphs": 548,
...
SMALL_PROVENANCE_PATHS = {
    ...
    "sgfn_metrics": Path(
        "experiments/H001_geom_reliability/sources/sgfn/"
...
def load_eval_module(root: Path) -> Any:
    ...
    spec = importlib.util.spec_from_file_location("h001_factor_eval", path)
    ...
    return {
        ...
        "docker": {
            "freeze_command": (
                ...
                "configs/h001/compose.yaml run --rm factor_isolation_protocol_freeze"
            ),
            "future_metric_command_reserved": (
                "env UID=$(id -u) GID=$(id -g) docker compose -f "
                "configs/h001/compose.yaml run --rm factor_isolation_metrics"
            ),
