...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("results/h001_geom_reliability/full_validation_transition/scope_contract"),
    )
...
```bash
python src/geocalib/stage_vlsat.py \\
...
python src/geocalib/join_predictions.py \\
  --predictions-jsonl {vlsat_root}/adapter/predictions.jsonl \\
  --model-json archive/hypothesis_records/hypothesis/CAND-001/H001_geometry-grounded-verification/artifacts/calibration/p_geom_valid_family/model.json \\
  --output-dir {vlsat_root}/geometry \\
  --selected-scans {selected_scans} \\
  --verification-policy point_subtype

python src/geocalib/evaluate_predictions.py \\
  --predictions-jsonl {vlsat_root}/adapter/predictions.jsonl \\
  ...
  --family-specific-model-json archive/hypothesis_records/hypothesis/CAND-001/H001_geometry-grounded-verification/artifacts/calibration/p_geom_valid_family/model.json

python src/geocalib/evaluate_gt_verifier.py \\
  --selected-scans {selected_scans} \\
  --ground-truth-jsonl {vlsat_root}/adapter/ground_truth.jsonl \\
  --model-json archive/hypothesis_records/hypothesis/CAND-001/H001_geometry-grounded-verification/artifacts/calibration/p_geom_valid_family/model.json \\
