#!/usr/bin/env python3
"""Scan-cluster bootstrap sensitivity for the promoted RelCompat3D evaluation."""
...
METHODS = ("source_score", "structured_product")
        ...
        for ki, k in enumerate(main_eval.KS):
            ...
            for metric in METRICS:
                ...
                report[method][str(k)][metric] = {
                    "point": point,
                    "scan_cluster_ci95": main_eval.ci95(samples),
                    "numerator": int(numerator.sum()),
                    "denominator": int(denominator.sum()),
                }
                cache[method][str(k)][metric] = samples

    report["structured_product_minus_source_score"] = {}
    for k in main_eval.KS:
        report["structured_product_minus_source_score"][str(k)] = {}
        for metric in METRICS:
            left = report["structured_product"][str(k)][metric]["point"]
            right = report["source_score"][str(k)][metric]["point"]
            delta = (
                cache["structured_product"][str(k)][metric]
                - cache["source_score"][str(k)][metric]
            )
            report["structured_product_minus_source_score"][str(k)][metric] = {
                "point": left - right,
                "paired_scan_cluster_ci95": main_eval.ci95(delta),
            }
    return report
...
def markdown(payload: dict[str, Any]) -> str:
    ...
    for source in ("vlsat", "open3dsg", "sgfn"):
        cell = payload["sources"][source]["results"][
            "structured_product_minus_source_score"
        ]["100"]
        dr = cell["recall"]
        dv = cell["violation_all"]
        lines.append(
            f"| {main_eval.SOURCE_LABELS[source]} | {dr['point']:+.4f} "
            f"[{dr['paired_scan_cluster_ci95'][0]:+.4f}, {dr['paired_scan_cluster_ci95'][1]:+.4f}] | "
            f"{dv['point']:+.4f} [{dv['paired_scan_cluster_ci95'][0]:+.4f}, "
            f"{dv['paired_scan_cluster_ci95'][1]:+.4f}] |"
        )
    lines.extend(
        [
            "",
...
def main() -> int:
    ...
    if protocol.get("status") != "frozen_before_scan_cluster_sensitivity":
    ...
    if main_eval.sha256_file(paths["structured_models"]) != protocol["model_sha256"]:
        raise ValueError("structured_model_hash_mismatch")
    strict_models = json.loads(paths["strict_models"].read_text(encoding="utf-8"))
    scorer = main_eval.make_structured_scorer(structured_models)
    gt, gt_family = main_eval.strict.load_gt(paths["ground_truth"])
    source_paths = {
    ...
    validations: dict[str, bool] = {
        "model_hash_locked": True,
        "all_sources_have_157_scans": True,
        "all_sources_have_548_contexts": True,
        "point_estimates_match_promoted_summary": True,
    ...
    for source_index, (source, path) in enumerate(source_paths.items()):
        grouped, _ = main_eval.load_candidates(path, scorer, strict_models)
        ...
        for method in METHODS:
            for k in main_eval.KS:
                for metric in METRICS:
                    ...
                    validations["point_estimates_match_promoted_summary"] &= abs(actual - expected) <= 1e-12
        k100 = results["structured_product_minus_source_score"]["100"]
        validations["k100_recall_ci_nonnegative"] &= k100["recall"]["paired_scan_cluster_ci95"][0] >= 0
        validations["k100_violation_ci_below_zero"] &= k100["violation_all"]["paired_scan_cluster_ci95"][1] < 0

    status = "completed" if all(validations.values()) else "failed_validation"
    payload = {
        "schema_version": "relcompat3d_scan_cluster_bootstrap_sensitivity_v1",
        ...
        "claim_boundary": "dependence sensitivity for the promoted unchanged rankings",
    }
    ...
    manifest = {
        "schema_version": "relcompat3d_scan_cluster_bootstrap_manifest_v1",
        "status": status,
        ...
        "validations": validations,
        "docker_command": "env UID=$(id -u) GID=$(id -g) docker compose -f configs/relcompat3d/compose.structured.yaml run --rm scan_cluster_sensitivity",
    }
