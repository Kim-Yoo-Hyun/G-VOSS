...
- CAND-001 next gate: no active main-claim metric blocker. `relative_horizontal`
  ...
  blocks promotion. `attachment_deferred` is now the preferred future
  relation-family expansion if H001 is upgraded beyond the current AAAI claim.
  Docker G0 scope/schema audit through G5c full-source protocol freeze are
  complete under `experiments/H001_geom_reliability/sources/attachment_deferred/`:
  it adds 967 GT rows (`attached to` 808, `hanging on` 126, `connected to` 33),
  expanding the candidate denominator from 2,545 to 3,512 if validated, with
  VL-SAT 77,748 and Open3DSG 57,300 candidate prediction rows. G5a fits model
  `h001-attachment-deferred-p-geom-valid-strict-v1`; G5b scores 120
  scan-diverse bounded source rows with evidence ready 120/120 and validation
  errors 0; G5c freezes 69 deterministic full-source shards for 135,048 rows
  and source-specific denominators: VL-SAT 967/967, Open3DSG 768/967. G5d
  full-source scoring/metrics/controls/bootstrap is complete with exit 0:
  69/69 shards, 135,048 scored rows, validation errors 0, log
  `logs/h001_attachment_g5d_full_20260606_113803.log`, and output
  `experiments/H001_geom_reliability/sources/attachment_deferred/full_source_g5d/`.
  This is not current main-claim evidence until the user explicitly confirms
  promotion after reviewing the caveats and audit need. The Open3DSG R1 exact non-avg
  downstream branch is regenerated under
  `experiments/H001_geom_reliability/sources/open3dsg/non_avg/`: raw stream
  19,162 rows / 377 batches, identity/export/geometry/metrics/bootstrap/Table
  6-caveat report all ready. For the historical 127-scan comparison, the
  existing avg-BLIP route remains stronger on train-dev loss
  (`0.32881081104278564` vs non-avg `0.5724539160728455`), and R2 H001
  covered-context recovery is complete at 388/388 with downstream
  metrics/bootstrap/table-caveat report ready. Docker provenance review
  confirms the R2 clean-return raw files are row/predicate-score equivalent to
  the canonical R2 raw dump after excluding run metadata; use R2 as the
  representative appendix/sensitivity branch and the old 377/388 branch as its
  comparison row. The separate full-validation 533/548 clean-exit retry review
  found the expected retry artifact missing after cleanup, so that unmodified
  ...
  Qwen-VL full official validation inference and downstream validation are
  complete as third-source modern-VLM extension evidence: parser validation,
...
- Qwen-VL is staged only as a third semantic source / modern VLM extension: input/output contract, parser skeleton, 30-row non-held-out tiny pilot, model-lock plan, 30/30 pair crops, Qwen3-VL-4B cache verification, runtime preflight, 3-row tiny inference smoke, raw-response validation, historical full-source route, and paper-facing full official validation route are ready through parser validation, adapter export, geometry join, metrics/controls, bootstrap CI, failure rows, and deterministic qualitative inspection. Current full-validation Qwen scope has 110,424 universe query rows, 46,506 inferable input rows, 63,918 missing query rows, 187 shards, 35,131 exported predictions, 32,236 in-scope predictions, and 3,972 H001-family GT rows. Qwen downstream metrics are third-source extension evidence only: semantic_only R@50/R@100 `0.2815/0.3600`, probabilistic_recalibrated `0.3215/0.3653`, rule_verified_point_subtype `0.3009/0.3630`, and family_specific control `0.3379/0.3653`.
