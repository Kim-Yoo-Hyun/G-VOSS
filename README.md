...
- `22_prep.md` 작성 완료: source dataset과 staged runtime root를 분리하고 aligned PLY / `multi_view` 준비 정책 고정
- `23_mini.md` 작성 완료: H001-Mini validation scan 8개 선택, support/contact 224개 coverage 고정
...
Local dataset validation is passed for one sample scan. H001-Mini validation scan selection is fixed. The current CAND-001 gate is implementing faithful `VL-SAT` staged-root prep for selected scan files, `references.txt`, `rescans.txt`, and aligned PLY.
...
- faithful layout prep policy is stored at `hypothesis/CAND-001/H001_geometry-grounded-verification/22_prep.md`.
- H001-Mini selection is stored at `hypothesis/CAND-001/H001_geometry-grounded-verification/23_mini.md`.
- selected H001-Mini scan ids are stored at `hypothesis/CAND-001/H001_geometry-grounded-verification/artifacts/subset/h001_mini/scans.txt`.
- checker output is stored at `hypothesis/CAND-001/H001_geometry-grounded-verification/artifacts/layout/vlsat/`.
- full baseline reproduction remains blocked until selected validation scan payloads, aligned PLY, and `multi_view` are staged.
