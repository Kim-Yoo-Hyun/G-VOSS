...
- CAND-001 next gate: decide whether to continue paper polish, optional Figure 3 rendering polish, or optional Qwen-VL smoke. Docker `paper/aaai/` PDF build is verified with `h001-aaai-tex:20260526`; `main.pdf` builds to 9 total pages with technical content on pages 1-7, references on page 8, and the AAAI reproducibility checklist on page 9. The manuscript treats Open3DSG as the main open-vocabulary relation-source case study and VL-SAT as a controlled reproduced anchor.
...
- `paper/aaai/`: current AAAI-style LaTeX manuscript source using AAAI-26 style files until the exact target-year kit is fixed
