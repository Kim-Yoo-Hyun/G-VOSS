...
- CAND-001: `attachment_deferred` is the preferred future relation-family upgrade, not current AAAI main-claim evidence. Docker G0 scope/schema audit through G5d full-source scoring/metrics/controls/bootstrap are complete under `experiments/H001_geom_reliability/sources/attachment_deferred/`; it covers `attached to`, `hanging on`, and `connected to`, adds 967 GT rows, and aligns better with H001's physical-consistency thesis than `relative_horizontal`. G5d status is `attachment_deferred_g5d_full_source_metrics_ready` with 69/69 shards, 135,048 scored rows, validation errors 0. Decision on 2026-06-11 KST: keep it as appendix/preliminary extension evidence and future-upgrade path, not current AAAI main evidence, because G5d is on the older H001 388/377-context scope rather than the current full official validation route, Open3DSG covers only 768/967 exact-label rows, `connected to` has no dev strict rows, and post-G5d audit is incomplete. Main-claim promotion requires full-validation rerun, denominator update, pairwise bootstrap deltas, post-G5d failure/visual audit, and explicit user confirmation.
- CAND-001: Qwen-VL은 third semantic source / modern VLM extension으로 고정한다. Primary model은 `Qwen/Qwen3-VL-4B-Instruct` revision `ebb281ec70b05090aa6165b016eac8ec08e71b17`, local-dir `local_dataset/model_cache/huggingface/qwen_vl/Qwen3-VL-4B-Instruct/ebb281ec70b05090aa6165b016eac8ec08e71b17`이다. Historical 127-scan full-source route는 inference/downstream validation까지 완료됐지만 non-main extension/sanity evidence로만 유지한다. Paper-facing Qwen route는 full official validation branch `experiments/H001_geom_reliability/sources/qwen_vl/full_validation/`이며, Docker input audit, crop render/preflight, 187/187 shard inference, aggregate/contract validation, adapter export, geometry join, metrics/controls, bootstrap CI, failure rows, and deterministic qualitative case inspection이 모두 완료됐다. Full-validation Qwen counts: 157 scans / 548 contexts / 110,424 query rows / 46,506 inferable input rows / 63,918 missing query rows / 35,131 exported predictions / 32,236 in-scope predictions / 3,972 H001-family GT rows. Contract validation parsed 46,506/46,506 rows with 0 errors. Key metrics: semantic_only R@50/R@100 `0.2815/0.3600`, V@50/@100 `0.1226/0.1246`; probabilistic_recalibrated `0.3215/0.3653`, V `0.0795/0.1166`; rule_verified_point_subtype `0.3009/0.3630`, V `0.0/0.0`; family_specific control `0.3379/0.3653`, V `0.0510/0.1113`. Bootstrap CI is ready: probabilistic vs semantic @50 delta R `+4.00 pp`, V `-4.31 pp`; @100 delta R `+0.53 pp`, V `-0.80 pp`. Failure rows: 31,881 metric-eligible diagnostic rows and 36 deterministic cases. Decision on 2026-06-12 KST: keep Qwen as appendix/extension evidence for the current AAAI route, not a main/table source, because recall is substantially lower than VL-SAT/Open3DSG and it is not an end-to-end 3DSSG reproduction.
...
- [x] Qwen-VL full official validation downstream:
      ...
      paper-facing 157-scan / 548-context full official validation scope before
      considering any Qwen paper table. Docker input audit, explicit missing-row
      denominator, input contract validation, crop render/preflight, 187/187
      shard inference, aggregate/contract validation, adapter export, geometry
      join, metrics/controls, bootstrap CI, failure rows, and deterministic
      qualitative case inspection are complete under
      `sources/qwen_vl/full_validation/`. Counts: 110,424 universe query rows,
      46,506 inferable input rows, 63,918 missing query rows, 35,131 exported
      predictions, 32,236 in-scope predictions, 3,972 H001-family GT rows, and
      31,881 metric-eligible failure-analysis rows. Key metrics:
      semantic_only R@50/R@100 `0.2815/0.3600`, V@50/@100 `0.1226/0.1246`;
      probabilistic_recalibrated `0.3215/0.3653`, V `0.0795/0.1166`;
      rule_verified_point_subtype `0.3009/0.3630`, V `0.0/0.0`; family_specific
      control `0.3379/0.3653`, V `0.0510/0.1113`. Logs:
      `logs/h001_qwen_fullval_downstream_20260612_031601.status.tsv` and
      `logs/h001_qwen_fullval_failure_tail_20260612_031805.status.tsv`.
      Paper decision fixed on 2026-06-12 KST: keep Qwen as
      appendix/extension evidence for the current AAAI route, not a main/table
      source.
...
- [x] Full official validation transition metric-bundle generation: build Docker-reproducible full
      ...
      157-scan scope. Key full-validation VL-SAT metrics:
      semantic_only R@50/R@100 `0.9272/0.9635`, V@50/@100 `0.0268/0.0476`;
      probabilistic_recalibrated R@50/R@100 `0.9305/0.9688`, V@50/@100
      `0.0229/0.0404`; rule_verified_point_subtype R@50/R@100
      `0.9257/0.9627`, V@50/@100 `0.0/0.0`; family_specific control
      R@50/R@100 `0.9288/0.9683`, V@50/@100 `0.0206/0.0333`.
      ...
      `0.0/0.0`; family_specific control R@50/R@100 `0.4612/0.5999`,
      ...
      `open3dsg_full_validation_table_caveats_ready`. Recovery Table 6
      candidate: semantic_only R@50/R@100 `0.4096/0.5161`, V@50/@100
      `0.1386/0.1242`; probabilistic_recalibrated R@50/R@100
      `0.3975/0.5723`, V@50/@100 `0.0606/0.0811`;
      rule_verified_point_subtype R@50/R@100 `0.4295/0.5368`,
      V@50/@100 `0.0/0.0`; family_specific control R@50/R@100
...
- [x] AAAI final consistency/content-polish pass repeated on 2026-06-11 KST
      after later document/table edits. Confirmed the main text, appendix/
      checklist, source-result table, and table artifacts consistently keep
      full-validation as the main route, Open3DSG 548/548 recovery caveat,
      exact-label denominator 3,972, 533/548 sensitivity branch, and residual
      calibration risk. Added reviewer-defense wording against three attacks:
      H001 as a hand-coded verifier, Open3DSG recovery as threshold tuning, and
      denominator caveats driving the result. Patched Table 1/2/3 `.csv`/`.json`
      artifacts to match the full-validation `.md` tables and added
      attachment as extension-only in Table 5. Docker PDF rebuild
      `logs/h001_aaai_pdf_build_final_consistency_20260611_020517.log` exited
      0 with 9 pages; targeted stale-value/LaTeX/diff checks passed. Upload
      bundle checksums were refreshed and verification passed with
      `logs/h001_fullval_upload_verify_20260611_020619.log`.
- [x] Problem-principle-fit paperization rule added on 2026-06-11 KST. Updated
      `AGENTS.md`, `docs/paper.md`, and `paper/risk.md` so future paper work
      must follow `failure mechanism -> design necessity -> fixed protocol ->
      falsifiable evidence -> claim boundary`, rather than fitting experiments
      to a desired hypothesis. Current defense audit: adequate for the narrow
      AAAI scoped relation-reliability claim, but Open3DSG recovery wording,
      family scope, and optional Qwen/attachment/lateral expansion boundaries
      must remain explicit.
- [x] Open3DSG preprocessing-route wording clarified on 2026-06-11 KST:
      manuscript and paper-risk docs now state that the 533/548 covered
      full-validation branch follows the unmodified Open3DSG source
      preprocessing pipeline, while the 548/548 main branch is a documented
      recovery-policy coverage branch. This preserves the distinction between
      official/source preprocessing behavior and full-denominator recovery.
- [x] Reproducibility upload artifact bundle finalized for Google Drive/Zenodo/HF
      ...
      passed with refreshed log `logs/h001_fullval_upload_verify_20260611_020619.log`.
      Archive creation is also complete:
      `release/h001_full_validation_results_20260611_025158.tar.zst`, size
      `1.4G`, sha256
      ...
      passed and tar listing sanity returned 219 members.
...
- [x] Qwen-VL remaining shard loop completed as background job
      `h001_qwen_vl_infer_remaining_resume_20260611_000531`, run id
      `20260611_000531`, from `qwen_full_source_shard_0014` through
      `qwen_full_source_shard_0133`. Exit file
      `logs/qwen_vl_full_source_infer_remaining_20260611_000531.exit`
      contains `0`; final shard `0133` finished at
      `2026-06-11T06:47:32+09:00`. Runtime counts are 134/134 manifests,
      predictions, raw-response, completed-progress, and report files; 33,384
      prediction/raw/completed rows; parser status `parsed:33383`,
      `parsed_with_warning:1`. This is still non-metric runtime generation.
...
- [x] AAAI final submission hygiene target-year check completed for AAAI-27:
      official AAAI-27 main technical track page and `https://aaai.org/authorkit27/`
      were checked on 2026-06-11 KST; `paper/aaai/` now uses
      `aaai2027.sty` / `aaai2027.bst`, Docker image
      `h001-aaai-tex:20260611`, and AAAI-27 checklist question structure.
      Verified AAAI-27 hygiene build log
...
- [x] Current-state Markdown synchronization completed after Qwen full-validation
      ...
      `hypothesis/README.md`, and
      `hypothesis/CAND-001/H001_geometry-grounded-verification/07_experiment_spec.md`
      so main-route wording uses full official validation, Open3DSG 548/548
      recovery as the paper-facing branch, 533/548 as unmodified-source
      sensitivity, historical 127-scan rows as appendix/sensitivity, and Qwen
      as third-source appendix/extension evidence only. The
      `hypothesis/CAND-001/H002_factorized-relation-confidence/` folder was not
      touched.
...
- [x] Run Qwen-VL all-shard parser validation, prediction aggregation/export,
      geometry join, metrics, controls, bootstrap CI, and audit; keep it as a
      third semantic source, not a VL-SAT/Open3DSG replacement. Docker
      downstream services completed after Qwen shard inference: aggregation
      `qwen_vl_full_source_aggregate_ready`; contract validation 33,384/33,384
      parsed rows with 0 input/output errors; adapter export 25,262 prediction
      rows and 23,084 in-scope rows; geometry join 25,262 preserved rows;
      metrics/controls `ready`; bootstrap CI `ready`; failure-analysis rows
      22,787 with 0 validation errors; qualitative inspection 36 deterministic
      cases. Key Qwen metrics: semantic_only R@50/R@100 `0.2684/0.3580`,
      V@50/@100 `0.1239/0.1260`; probabilistic_recalibrated
      `0.3092/0.3654`, V `0.0787/0.1167`; rule_verified_point_subtype
      `0.2904/0.3631`, V `0.0/0.0`; family_specific control
      `0.3308/0.3654`, V `0.0499/0.1106`. Boundary at that time:
      third-source modern-VLM extension only; the historical 127-scan route is
      now superseded for paper-facing Qwen evidence by the completed
      full-validation branch above. For the current AAAI route, Qwen is fixed
      as appendix/extension evidence only.
...
- [x] Open3DSG full-validation missing-15 recovery downstream 완료:
      ...
      regeneration ready. Key metrics: semantic_only R@50/R@100
      `0.4096/0.5161`, V@50/@100 `0.1386/0.1242`;
      ...
      R@50/R@100 `0.4658/0.6047`, V@50/@100 `0.0286/0.0341`. Caveat:
...
- [x] Historical H001 AAAI reproducibility checklist 추가 완료: official AAAI-26 reproducibility checklist page를 확인하고 `paper/aaai/sec/9_reproducibility_checklist.tex`를 추가한 뒤 `paper/aaai/main.tex`에서 references 뒤에 연결했다. Docker rebuild `logs/h001_aaai_pdf_build_20260526_102601.log` exit 0. This is superseded by the 2026-06-11 AAAI-27 checklist migration above.
- [x] Historical H001 AAAI-style source conversion 완료: `paper/aaai/`에 AAAI-style `main.tex`, `preamble.tex`, `sec/*.tex`, `aaai2026.sty`, `aaai2026.bst`, `Dockerfile.tex`, `.gitignore`, `README.md`, and `inspection/report.md`를 생성했다. Official AAAI-26 submission page를 확인했고 최신 공개 AAAI-26 style route를 사용했다. Docker image `h001-aaai-tex:20260526` build 성공, `paper/aaai/main.pdf` build 성공. This was later superseded by the official AAAI-27 route above.
