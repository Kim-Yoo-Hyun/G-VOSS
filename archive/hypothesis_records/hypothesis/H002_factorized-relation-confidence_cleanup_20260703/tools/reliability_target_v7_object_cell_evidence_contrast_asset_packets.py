#!/usr/bin/env python3
"""Generate packets for H002 v7 object-cell evidence-contrast candidates."""
...
CANDIDATE_DIR = RGA_ROOT / "reliability_target_v7_object_cell_evidence_contrast_candidate_mining_codex_proxy_user_requested"
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v7_object_cell_evidence_contrast_asset_packets_codex_proxy_user_requested"
DEFAULT_SCAN_ROOT = REPO_ROOT / "local_dataset/3RScan/scans"

SCHEMA_VERSION = "h002_reliability_target_v7_object_cell_evidence_contrast_asset_packets_v1"
REVIEW_SCOPE = "h002_reliability_v7_object_cell_evidence_contrast_packeted_review"
GENERATED_PACKET_SOURCE = "generated_v7_object_cell_evidence_contrast_asset_packet"
EXISTING_PACKET_SOURCE = "existing_prior_asset_packet"
MISSING_PACKET_SOURCE = "missing_v7_object_cell_evidence_contrast_asset_packet"
...
def validate_inputs(
    ...
    errors: list[dict[str, Any]] = []
    if candidate_summary.get("status") != "h002_reliability_target_v7_object_cell_evidence_contrast_candidate_mining_ready_needs_asset_packets":
        errors.append({"error_type": "unexpected_candidate_status", "value": candidate_summary.get("status")})
    if candidate_summary.get("next_todo") != "reliability_target_v7_object_cell_evidence_contrast_asset_packets":
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# H002 V7 Object-Cell Evidence Contrast Asset Packets",
        "",
        ...
        "- Semantic score/rank, geometry score/status, queue kind, strict group key, and object-cell metadata remain hidden.",
        "",
        "## Status",
        "",
        "```text",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if all_ready and not generated_non_ready and not has_errors:
        status = "h002_reliability_target_v7_object_cell_evidence_contrast_asset_packets_ready_for_label_readiness"
        next_todo = "reliability_target_v7_object_cell_evidence_contrast_label_readiness"
        decision = "The full 240-row v7 object-cell evidence-contrast sheet is packet-complete and can proceed to label-readiness checks."
    elif status_counts.get("ready", 0) > 0 and not validation_errors and not leakage_hits:
        status = "h002_reliability_target_v7_object_cell_evidence_contrast_asset_packets_partial_needs_gap_audit"
        next_todo = "reliability_target_v7_object_cell_evidence_contrast_asset_packet_gap_audit"
        decision = "V7 object-cell evidence-contrast packets are partial. Inspect non-ready rows or missing paths before label fill."
    else:
        status = "h002_reliability_target_v7_object_cell_evidence_contrast_asset_packets_blocked"
        next_todo = "fix_reliability_target_v7_object_cell_evidence_contrast_asset_packets"
        decision = "V7 object-cell evidence-contrast packet generation is blocked by validation, leakage, or path errors."

    output_paths = {
        "summary": output_dir / "summary.json",
        "report": output_dir / "report.md",
        "full_label_sheet": output_dir / "v7_object_cell_evidence_contrast_full_label_sheet.tsv",
        "label_ready_sheet": output_dir / "v7_object_cell_evidence_contrast_label_ready_sheet.tsv",
        "full_manifest_post_label_only": output_dir / "v7_object_cell_evidence_contrast_full_manifest_post_label_only.jsonl",
        "label_ready_manifest_post_label_only": output_dir / "v7_object_cell_evidence_contrast_label_ready_manifest_post_label_only.jsonl",
