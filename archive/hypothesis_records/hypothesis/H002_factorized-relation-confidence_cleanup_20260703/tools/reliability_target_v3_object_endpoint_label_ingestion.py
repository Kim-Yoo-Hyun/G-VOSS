#!/usr/bin/env python3
"""Ingest object/endpoint-controlled H002 reliability target v3 labels."""
...
FILL_DIR = RGA_ROOT / "reliability_target_v3_object_endpoint_label_fill_codex_proxy_user_requested"
MINING_DIR = RGA_ROOT / "reliability_target_v3_object_endpoint_candidate_mining"

DEFAULT_COMPLETED_SHEET = FILL_DIR / "completed_object_endpoint_label_sheet_codex_proxy_user_requested.tsv"
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v3_object_endpoint_label_ingestion_codex_proxy_user_requested"

LABEL_SOURCE = "codex_proxy_reliability_target_v3_object_endpoint_user_requested"
...
def validate_fill_summary(fill_summary: dict[str, Any]) -> list[dict[str, Any]]:
    errors: list[dict[str, Any]] = []
    expected_status = "h002_reliability_target_v3_object_endpoint_label_filled_codex_proxy_user_requested"
    ...
    if fill_summary.get("next_todo") != "reliability_target_v3_object_endpoint_label_ingestion":
        errors.append({"error_type": "fill_summary_next_todo_unexpected", "value": fill_summary.get("next_todo")})
    ...
    expected_false = [
        ...
        "used_candidate_proxy_class_for_label_decision",
...
def hidden_metadata(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    keys = [
        "sampling_tier_hidden",
        "sampling_cell_type_hidden",
        "sampling_cell_key_hidden",
        "sampling_proxy_label_key_hidden",
        "sampling_requested_for_proxy_hidden",
        "sampling_within_cell_index_hidden",
        "selection_pass_hidden",
        "candidate_proxy_class_hidden",
        "endpoint_flag_pattern_hidden",
        "endpoint_pair_note_hidden",
        "subject_object_family_cell_hidden",
        "subject_object_cell_hidden",
        "object_family_cell_hidden",
        "object_predicate_cell_hidden",
...
def deployable_evidence_after_label_lock(row: dict[str, str], manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "forbidden_as_posterior_input": {
            ...
            "sampling_tier_hidden": True,
            "sampling_cell_type_hidden": True,
            "sampling_proxy_label_key_hidden": True,
            "candidate_proxy_class_hidden": True,
...
def make_label(row: dict[str, str], manifest: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "h002_reliability_target_v3_object_endpoint_ingested_label_v1",
...
def target_row(label: dict[str, Any], target_key: str, schema_version: str) -> dict[str, Any] | None:
    ...
    return {
        ...
        "sampling_tier_hidden": hidden.get("sampling_tier_hidden"),
        "sampling_cell_type_hidden": hidden.get("sampling_cell_type_hidden"),
        "sampling_proxy_label_key_hidden": hidden.get("sampling_proxy_label_key_hidden"),
        "candidate_proxy_class_hidden": hidden.get("candidate_proxy_class_hidden"),
...
def multiclass_target_row(label: dict[str, Any]) -> dict[str, Any]:
    row = target_row(label, "relation_reliability_v3_multiclass_target", "h002_reliability_target_v3_object_endpoint_multiclass_row_v1")
    ...
    return {
        **row,
        ...
        "audit_note": (
            ...
            "Do not use v3 review fields, hidden bucket fields, sampling tier/cell, "
            "candidate proxy class, geometry_status, label_match_status, or audit packet paths as model input."
        ),
    }
...
def excluded_target_row(label: dict[str, Any], target_key: str) -> dict[str, Any] | None:
    ...
    if target["target_y"] is not None:
        return None
    return {
        "schema_version": "h002_reliability_target_v3_object_endpoint_excluded_target_v1",
        "target_name": target["target_name"],
        "target_y": None,
        "target_use": target["target_use"],
        "target_reason": target["reason"],
...
def target_independence_probe(target_rows: list[dict[str, Any]], target_name: str) -> dict[str, Any]:
    ...
    if not target_rows:
        return {
            "schema_version": "h002_reliability_target_v3_object_endpoint_independence_probe_v1",
            "target_name": target_name,
...
def count_binary_target(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "by_sampling_tier": nested_counts(rows, "sampling_tier_hidden"),
        "by_sampling_proxy": nested_counts(rows, "sampling_proxy_label_key_hidden"),
    }
...
def build_summary_status(errors: list[dict[str, Any]], probes: dict[str, dict[str, Any]], reliability_count: dict[str, Any]) -> tuple[str, str, str]:
    ...
    if errors:
        return (
            "h002_reliability_target_v3_object_endpoint_label_ingestion_errors",
            "Fix object/endpoint v3 label ingestion errors before target audit.",
            "fix_reliability_target_v3_object_endpoint_label_ingestion_errors",
        )
    if not enough_reliability_mass and any_probe_risk:
        return (
            ...
            (
                "Object/endpoint v3 labels are ingested, but the main relation reliability target is positive-sparse "
                "and shortcut probes still flag construction risk. Run target-independence audit before any posterior smoke."
            ),
            "reliability_target_v3_object_endpoint_target_independence_audit",
        )
    if not enough_reliability_mass:
        return (
            "h002_reliability_target_v3_object_endpoint_label_ingested_positive_sparse",
            "Object/endpoint v3 labels are ingested, but the main relation reliability target is positive-sparse.",
            "reliability_target_v3_object_endpoint_target_independence_audit",
        )
    if any_probe_risk:
        return (
            "h002_reliability_target_v3_object_endpoint_label_ingested_with_probe_risk",
            (
                "Object/endpoint v3 labels are ingested and binary target mass is usable, but hidden/visible "
                "shortcut probes still flag target-construction risk. Run target-independence audit before posterior smoke."
            ),
            "reliability_target_v3_object_endpoint_target_independence_audit",
        )
    return (
        "h002_reliability_target_v3_object_endpoint_label_ingested_ready_for_target_independence_audit",
        "Object/endpoint v3 labels are ingested. Run dedicated target-independence audit before posterior smoke.",
        "reliability_target_v3_object_endpoint_target_independence_audit",
    )
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    reliability_targets = [
        row
        for row in (
            target_row(label, "relation_reliability_v3_binary_target", "h002_reliability_target_v3_object_endpoint_binary_row_v1")
            for label in labels
        )
        if row is not None
    ]
    geometry_targets = [
        row
        for row in (
            target_row(label, "geometry_support_v3_binary_target", "h002_geometry_support_v3_object_endpoint_binary_row_v1")
            for label in labels
        )
        if row is not None
    ]
    usefulness_targets = [
        row
        for row in (
            target_row(label, "relation_usefulness_v3_binary_target", "h002_relation_usefulness_v3_object_endpoint_binary_row_v1")
            for label in labels
        )
        if row is not None
    ]
    ...
    reliability_posterior = [
        row
        for row in (
            posterior_candidate_row(label, "relation_reliability_v3_binary_target", "h002_reliability_target_v3_object_endpoint_posterior_candidate_row_v1")
            for label in labels
        )
        if row is not None
    ]
    geometry_posterior = [
        row
        for row in (
            posterior_candidate_row(label, "geometry_support_v3_binary_target", "h002_geometry_support_v3_object_endpoint_posterior_candidate_row_v1")
            for label in labels
        )
        if row is not None
    ]
    usefulness_posterior = [
        row
        for row in (
            posterior_candidate_row(label, "relation_usefulness_v3_binary_target", "h002_relation_usefulness_v3_object_endpoint_posterior_candidate_row_v1")
            for label in labels
        )
        if row is not None
    ]
    ...
    output_paths = {
        ...
        "validated_v3_labels": output_dir / "validated_object_endpoint_v3_labels.jsonl",
    ...
    summary = {
        "schema_version": "h002_reliability_target_v3_object_endpoint_label_ingestion_summary_v1",
        "status": status,
    ...
    write_json(
        ...
        {
            "schema_version": "h002_reliability_target_v3_object_endpoint_target_independence_probe_bundle_v1",
            "probes": probes,
        },
    )
