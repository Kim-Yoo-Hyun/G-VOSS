#!/usr/bin/env python3
"""Generate packets for H002 reliability target v4 matched-contrast rows."""
...
import reliability_target_v4_matched_contrast_candidate_mining as mining
...
CANDIDATE_DIR = RGA_ROOT / "reliability_target_v4_matched_contrast_candidate_mining"
...
DEFAULT_PREVIOUS_MANIFEST = CANDIDATE_DIR / "matched_contrast_manifest_post_label_only.jsonl"
DEFAULT_ASSET_REQUESTS = CANDIDATE_DIR / "asset_request_plan.jsonl"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v4_matched_contrast_asset_packets"
DEFAULT_SCAN_ROOT = REPO_ROOT / "local_dataset/3RScan/scans"

SCHEMA_VERSION = "h002_reliability_target_v4_matched_contrast_asset_packets_v1"
REVIEW_SCOPE = "h002_reliability_v4_relation_packeted"
...
def update_seed_with_packet(seed: dict[str, Any], packet: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if packet is not None:
        ...
        updated["asset_packet_source_hidden"] = "generated_v4_matched_contrast_asset_packet"
        ...
        updated["asset_packet_source_hidden"] = "missing_v4_matched_contrast_asset_packet"
        updated["generated_blind_review_id_hidden"] = blind_id
    return updated
...
def full_manifest_row(seed: dict[str, Any], previous_manifest_by_id: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    row["batch_name"] = "reliability_target_v4_matched_contrast_asset_packets"
    row["schema_version"] = "h002_reliability_target_v4_matched_contrast_asset_packet_manifest_v1"
...
def packet_text_leakage_hits(packet_rows: list[dict[str, Any]], output_dir: Path) -> list[dict[str, Any]]:
    hits: list[dict[str, Any]] = []
...
def pair_summary_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for pair_id in sorted(by_pair):
        ...
        roles = Counter(str(row.get("matched_contrast_role_hidden")) for row in group)
        ...
        output.append(
            {
                ...
                "generated_rows": sources.get("generated_v4_matched_contrast_asset_packet", 0),
                ...
                "matched_contrast_stratum_hidden": group[0].get("matched_contrast_stratum_hidden", ""),
            }
        )
    return output


def stratum_summary_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        output.append(
            {
                ...
                "rows": len(group),
                "pairs": len({row.get("matched_contrast_pair_id") for row in group}),
                "ready_rows": statuses.get("ready", 0),
                "partial_rows": statuses.get("partial", 0),
                "missing_rows": statuses.get("missing", 0),
                "generated_rows": sources.get("generated_v4_matched_contrast_asset_packet", 0),
...
def validate_inputs(
    ...
    errors: list[dict[str, Any]] = []
    if candidate_summary.get("next_todo") != "reliability_target_v4_matched_contrast_asset_packets":
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    for key, value in summary["counts"]["by_family"].items():
        lines.append(f"| `{key}` | {value} |")
    lines.extend(
        [
            "",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    generated_manifest_rows = [
        row for row in manifest_rows if row.get("asset_packet_source_hidden") == "generated_v4_matched_contrast_asset_packet"
    ]

    full_label_sheet = output_dir / "matched_contrast_full_label_sheet.tsv"
    ...
    packet_text_hits = packet_text_leakage_hits(generated_packets, output_dir)
    ...
    stratum_rows = stratum_summary_rows(updated_seeds)

    all_ready = status_counts.get("ready", 0) == len(label_rows)
    status = (
        "h002_reliability_target_v4_matched_contrast_asset_packets_ready"
        if all_ready and not generated_non_ready and not path_errors and not leakage_hits and not validation_errors
        else "h002_reliability_target_v4_matched_contrast_asset_packets_partial"
        if status_counts.get("ready", 0) > 0 and not leakage_hits
        else "h002_reliability_target_v4_matched_contrast_asset_packets_blocked"
    )
    next_todo = (
        "reliability_target_v4_matched_contrast_label_fill"
        if status == "h002_reliability_target_v4_matched_contrast_asset_packets_ready"
        else "reliability_target_v4_matched_contrast_asset_packet_gap_audit"
        if status == "h002_reliability_target_v4_matched_contrast_asset_packets_partial"
        else "fix_reliability_target_v4_matched_contrast_asset_packets"
    )
    decision = (
        "The full 160-row matched-contrast sheet is packet-complete and can proceed to label fill."
        if status == "h002_reliability_target_v4_matched_contrast_asset_packets_ready"
        else "Matched-contrast packets are partial. Inspect generated non-ready rows or missing paths before label fill."
        if status == "h002_reliability_target_v4_matched_contrast_asset_packets_partial"
        else "Matched-contrast packet generation is blocked by validation, leakage, or path errors."
    )
    ...
    output_paths = {
        ...
        "full_manifest_post_label_only": output_dir / "matched_contrast_full_manifest_post_label_only.jsonl",
        ...
        "stratum_summary": output_dir / "stratum_summary.csv",
