#!/usr/bin/env python3
"""Fill the v8 endpoint-pair counterfactual reliability label sheet as a Codex proxy."""
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v8_endpoint_pair_counterfactual_label_fill_codex_proxy_user_requested"

SCHEMA_VERSION = "h002_reliability_target_v8_endpoint_pair_counterfactual_label_fill_summary_v1"
STATUS_READY = "h002_reliability_target_v8_endpoint_pair_counterfactual_label_filled_codex_proxy_user_requested"
STATUS_ERROR = "h002_reliability_target_v8_endpoint_pair_counterfactual_label_fill_errors"
REVIEWER_ID = "(codex_proxy_v8_user_requested_visible_only)"
REVIEW_ROUND = "1"
...
def validate_inputs(
    ...
    if readiness_summary.get("next_todo") != "reliability_target_v8_endpoint_pair_counterfactual_label_fill":
        errors.append({"error_type": "unexpected_readiness_next_todo", "value": readiness_summary.get("next_todo")})
    if readiness_summary.get("status") != "h002_reliability_target_v8_endpoint_pair_counterfactual_label_readiness_ready_for_label_fill":
...
def fill_row(row: dict[str, str]) -> dict[str, Any]:
    ...
    filled["label_notes_v6"] = (
        "codex proxy v8 visible-only fill; used visible subject/object/predicate labels, "
        "predicate family question, cue text, evidence packet status, and packet gap caveat; "
        "did not use hidden semantic-geometry bucket, semantic rank/score, p_geom_valid, "
        "geometry status, source queue, label-match status, object-cell metadata, or "
        "endpoint-pair/v8 group metadata, or target-construction metadata before label lock"
    )
    return filled
...
def label_record(row: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "h002_reliability_target_v8_endpoint_pair_counterfactual_proxy_label_v1",
        ...
        "provenance": {
            "batch_name": "reliability_target_v8_endpoint_pair_counterfactual_label_fill",
            ...
            "used_object_cell_metadata_for_label_decision": False,
            "used_endpoint_pair_metadata_for_label_decision": False,
            "used_v8_group_metadata_for_label_decision": False,
...
def family_bucket_counter(filled_rows: list[dict[str, Any]], manifest_rows: list[dict[str, Any]]) -> dict[str, int]:
    manifest_by_id = {str(row["blind_review_id"]): row for row in manifest_rows}
    counts: Counter[str] = Counter()
    for row in filled_rows:
        manifest = manifest_by_id.get(str(row["blind_review_id"]), {})
        bucket = diagnostic_group_value(manifest, "semantic_geometry_bucket_hidden")
        counts[f"{row['predicate_family']}|{bucket}"] += 1
    return dict(sorted(counts.items()))
...
def multiclass_target_rows(filled_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in filled_rows:
        ...
        rows.append(
            {
                "schema_version": "h002_reliability_target_v8_endpoint_pair_counterfactual_multiclass_target_v1",
                "blind_review_id": row["blind_review_id"],
                "scan_id": row["scan_id"],
                "scene_context_id": row["scene_context_id"],
                "subject_id": row["subject_id"],
                ...
                "target_source": "codex_proxy_user_requested_visible_only_v8_endpoint_pair_counterfactual_label_fill",
                "split": "train_only",
                "paper_evidence_allowed": False,
            }
        )
...
def binary_target_rows(filled_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in filled_rows:
        ...
        rows.append(
            {
                "schema_version": "h002_reliability_target_v8_endpoint_pair_counterfactual_binary_target_v1",
                "blind_review_id": row["blind_review_id"],
                "scan_id": row["scan_id"],
                "scene_context_id": row["scene_context_id"],
                "subject_id": row["subject_id"],
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# H002 Reliability Target V8 Endpoint-Pair Counterfactual Label Fill",
        "",
        ...
        "- Hidden semantic-geometry bucket, semantic score/rank, `p_geom_valid`, geometry status, source queue, endpoint-pair/v8 group metadata, object-cell metadata, label-match status, and target-construction metadata are not used before label lock.",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    output_paths = {
        ...
        "completed_sheet": output_dir / "completed_v8_endpoint_pair_counterfactual_label_sheet_codex_proxy_user_requested.tsv",
        "v8_proxy_labels": output_dir / "v8_proxy_labels.jsonl",
    ...
    next_todo = "fix_reliability_target_v8_endpoint_pair_counterfactual_label_fill" if errors else "reliability_target_v8_endpoint_pair_counterfactual_label_ingestion"
    decision = (
        "Input or fill validation errors block ingestion."
        if errors
        else (
            "Filled the 240-row v8 endpoint-pair counterfactual sheet as a user-requested Codex proxy. "
    ...
    summary = {
        ...
        "counts": {
            ...
            "semantic_geometry_bucket_post_label_only": manifest_counter(manifest_rows, "semantic_geometry_bucket_hidden"),
            "source_queue_post_label_only": manifest_counter(manifest_rows, "source_queue_hidden"),
            "family_bucket_post_label_only": family_bucket_counter(filled_rows, manifest_rows),
    ...
    write_jsonl(output_paths["v8_proxy_labels"], label_rows)
