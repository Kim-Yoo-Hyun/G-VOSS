#!/usr/bin/env python3
"""Audit external-review support/vertical target independence."""
...
DEFAULT_INGESTION_DIR = RGA_ROOT / "independent_support_vertical_v2_external_review_ingestion_codex_proxy_user_requested"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "independent_support_vertical_v2_external_review_target_independence_audit_codex_proxy_user_requested"

GEOMETRY_TARGET = "geometry_validity_external_target"
RELIABILITY_TARGET = "relation_reliability_external_target"
TARGET_INPUTS = {
    GEOMETRY_TARGET: "geometry_validity_external_posterior_rows.jsonl",
    RELIABILITY_TARGET: "relation_reliability_external_posterior_rows.jsonl",
}
...
SLICE_SPECS = {
    "original_external": {
        "balanced_keys": [],
        "reason": "full user-requested external-proxy binary target",
        "priority": 99,
    },
    "prior_relation_validity_balanced_external": {
    ...
    "queue_family_balanced_external": {
    ...
    "prior_label_rank_balanced_external": {
...
def validate_rows(target_name: str, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if row.get("user_requested_proxy_review") is not True:
            errors.append({"target_name": target_name, "error_type": "missing_user_requested_proxy_review_flag", "row_number": index, "blind_review_id": blind_id})
        if row.get("actual_user_reviewer") is not False:
            errors.append(
                {
                    "target_name": target_name,
                    "error_type": "unexpected_actual_user_reviewer_flag",
                    "row_number": index,
                    "blind_review_id": blind_id,
                    "actual_user_reviewer": row.get("actual_user_reviewer"),
                }
            )
        if row.get("user_review_pending") is not True:
            errors.append({"target_name": target_name, "error_type": "missing_user_review_pending_flag", "row_number": index, "blind_review_id": blind_id})
        ...
        if "audit_only_external_review_fields" not in row:
            errors.append({"target_name": target_name, "error_type": "missing_audit_only_external_review_fields", "row_number": index, "blind_review_id": blind_id})
        ...
        if forbidden.get("multi_view_as_model_input") is not True:
        ...
        if forbidden.get("external_review_fields") is not True:
            errors.append({"target_name": target_name, "error_type": "missing_external_review_fields_forbidden_flag", "row_number": index, "blind_review_id": blind_id})
    return errors
...
def per_target_decision(target_name: str, summaries: list[dict[str, Any]], errors: list[dict[str, Any]]) -> dict[str, Any]:
    original = next(item for item in summaries if item["slice_name"] == "original_external")
    ...
    if errors:
        ...
        decision = "Fix row validation errors before using any external target slice."
        next_step = "fix_external_target_independence_audit_errors"
    elif strict:
        status = "strict_controlled_slice_ready"
        decision = (
            "A strict external-proxy controlled slice clears harmful prior-label carryover and "
            "construction-risk checks. Source-score feature join is still required before posterior smoke."
        )
        next_step = "external_source_feature_join_then_controlled_posterior_smoke"
    elif construction:
        ...
        next_step = "revise_external_review_or_collect_true_user_labels"
    else:
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# H002 External Review Target Independence Audit",
        "",
        ...
        "- They are not paper-level external human annotation before user confirmation.",
        "- Hidden metadata is used only after label lock for audit and controlled-slice construction.",
        "- External review fields, hidden strata, previous proxy labels, and audit packet paths are not posterior inputs.",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if all_validation_errors:
        status = "full_train_independent_support_vertical_v2_external_review_target_independence_audit_errors"
        decision = "Fix validation errors before any external target slice can be used."
        next_todo = "fix_full_train_independent_support_vertical_v2_external_review_target_independence_audit_errors"
    elif relation_strict:
        status = "full_train_independent_support_vertical_v2_external_review_target_independence_audit_relation_strict_slice_ready"
        decision = (
            "A strict relation-reliability slice exists under the user-requested external-proxy target. "
            "Before posterior smoke, join source rank/score and p_geom_valid as deployable features."
        )
        next_todo = "full_train_independent_support_vertical_v2_external_source_feature_join"
        ...
        next_todo = "revise_external_review_or_collect_true_user_labels"
    else:
        status = "full_train_independent_support_vertical_v2_external_review_target_independence_audit_blocked"
        decision = "No size-ready strict or construction-only controlled slice exists for external relation reliability."
        next_todo = "revise_external_review_or_collect_true_user_labels"
    ...
    summary = {
        "schema_version": "h002_support_vertical_v2_external_review_target_independence_audit_summary_v1",
        "status": status,
        ...
        "boundary": {
            ...
            "external_review_fields_used_for_target_or_audit_only": True,
