#!/usr/bin/env python3
"""Ingest user-requested external review fields after label lock."""
...
DEFAULT_FILL_DIR = RGA_ROOT / "independent_support_vertical_v2_external_review_fill_codex_proxy_user_requested"
DEFAULT_PROTOCOL_DIR = RGA_ROOT / "independent_support_vertical_v2_external_review_protocol"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "independent_support_vertical_v2_external_review_ingestion_codex_proxy_user_requested"

DEFAULT_COMPLETED_SHEET = DEFAULT_FILL_DIR / "completed_external_evidence_review_sheet_codex_proxy_user_requested.tsv"
DEFAULT_FILL_SUMMARY = DEFAULT_FILL_DIR / "summary.json"
DEFAULT_SCHEMA = DEFAULT_PROTOCOL_DIR / "external_review_schema.json"
DEFAULT_INTERNAL_MANIFEST = DEFAULT_PROTOCOL_DIR / "external_manifest_post_label_only.jsonl"

LABEL_SOURCE = "codex_proxy_external_review_user_requested"
GEOMETRY_TARGET_NAME = "geometry_validity_external_target"
RELIABILITY_TARGET_NAME = "relation_reliability_external_target"
...
def external_review_fields(row: dict[str, str]) -> dict[str, Any]:
    fields = {
...
def hidden_metadata(internal: dict[str, Any]) -> dict[str, Any]:
    hidden = internal.get("hidden_audit_metadata_post_label_only", {})
    output = {key: hidden.get(key) for key in HIDDEN_GROUP_KEYS}
    output["previous_codex_proxy_human_fields_post_label_only"] = internal.get("previous_codex_proxy_human_fields_post_label_only", {})
...
def derive_geometry_target(row: dict[str, str]) -> dict[str, Any]:
    ...
    if "contradicts_predicate" in answers and "supports_predicate" not in answers:
        return {"target_name": GEOMETRY_TARGET_NAME, "target_use": "negative", "target_y": 0, "reason": "external_geometry_contradicts_predicate"}
    if "supports_predicate" in answers and "contradicts_predicate" not in answers:
        return {"target_name": GEOMETRY_TARGET_NAME, "target_use": "positive", "target_y": 1, "reason": "external_geometry_supports_predicate"}
    ...
    return {"target_name": GEOMETRY_TARGET_NAME, "target_use": "exclude", "target_y": None, "reason": f"exclude_external_geometry_uncertain={uncertainty}"}
...
def derive_reliability_target(row: dict[str, str], geometry_target: dict[str, Any]) -> dict[str, Any]:
    ...
    uncertainty = row["uncertainty_reason_external"]
    if final == "uncertain":
        return {"target_name": RELIABILITY_TARGET_NAME, "target_use": "exclude", "target_y": None, "reason": f"exclude_external_reliability_uncertain={uncertainty}"}
    if endpoint != "both_valid":
        return {"target_name": RELIABILITY_TARGET_NAME, "target_use": "negative", "target_y": 0, "reason": f"external_endpoint_invalid={endpoint}"}
    if final == "reliable" and geometry_target["target_y"] == 1 and informativeness == "informative":
        return {"target_name": RELIABILITY_TARGET_NAME, "target_use": "positive", "target_y": 1, "reason": "external_relation_reliable"}
    if final == "unreliable":
        return {"target_name": RELIABILITY_TARGET_NAME, "target_use": "negative", "target_y": 0, "reason": "external_relation_unreliable"}
    if geometry_target["target_y"] == 0:
        return {"target_name": RELIABILITY_TARGET_NAME, "target_use": "negative", "target_y": 0, "reason": "external_geometry_contradicts_predicate"}
    if informativeness in {"trivial_dense_or_room_structure", "ontology_mismatch"}:
        return {"target_name": RELIABILITY_TARGET_NAME, "target_use": "negative", "target_y": 0, "reason": f"external_informativeness={informativeness}"}
    return {"target_name": RELIABILITY_TARGET_NAME, "target_use": "exclude", "target_y": None, "reason": "exclude_external_reliability_ambiguous_contract"}
...
def make_label(row: dict[str, str], internal: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "h002_support_vertical_v2_external_ingested_label_v1",
        **base_identity(row),
        ...
        "external_review_fields_are_target_only": True,
        "external_review_fields": external_review_fields(row),
        "geometry_validity_external_target": geometry_target,
        "relation_reliability_external_target": reliability_target,
...
def posterior_row(label: dict[str, Any], target_key: str, schema_version: str) -> dict[str, Any] | None:
    ...
    return {
        **row,
        ...
        "audit_only_external_review_fields": label["external_review_fields"],
        "hidden_audit_metadata_post_label_only": label["hidden_audit_metadata_post_label_only"],
        "audit_note": "Use only deployable evidence after target gate. External review fields are target/audit only.",
    }
...
def excluded_row(label: dict[str, Any], target_key: str) -> dict[str, Any] | None:
    ...
    if target["target_y"] is not None:
        return None
    return {
        "schema_version": "h002_support_vertical_v2_external_excluded_target_v1",
        "target_name": target["target_name"],
        "target_use": target["target_use"],
        "target_y": None,
        "target_reason": target["reason"],
...
def axis_counts(labels: list[dict[str, Any]]) -> dict[str, dict[str, int]]:
    ...
    for axis in EXTERNAL_AXIS_KEYS:
        output[axis] = dict(sorted(Counter(row["external_review_fields"].get(axis) for row in labels).items()))
    return output
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# H002 External Review Ingestion",
        "",
        ...
        "- External review fields are target/audit only and are not posterior inputs.",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if boundary.get("filled_by") != "codex_proxy" or boundary.get("user_requested_treat_as_user_review") is not True:
        errors.append({"error_type": "fill_summary_does_not_mark_user_requested_codex_proxy"})
    if boundary.get("used_hidden_manifest") is not False:
        errors.append({"error_type": "fill_summary_does_not_block_hidden_manifest"})
    ...
    geometry_targets = [
        row
        for row in (
            target_row(label, "geometry_validity_external_target", "h002_support_vertical_v2_external_geometry_validity_target_v1")
            for label in labels
        )
        if row is not None
    ]
    reliability_targets = [
        row
        for row in (
            target_row(label, "relation_reliability_external_target", "h002_support_vertical_v2_external_relation_reliability_target_v1")
            for label in labels
        )
        if row is not None
    ]
    geometry_posterior = [
        row
        for row in (
            posterior_row(label, "geometry_validity_external_target", "h002_support_vertical_v2_external_geometry_validity_posterior_row_v1")
            for label in labels
        )
        if row is not None
    ]
    reliability_posterior = [
        row
        for row in (
            posterior_row(label, "relation_reliability_external_target", "h002_support_vertical_v2_external_relation_reliability_posterior_row_v1")
            for label in labels
        )
        if row is not None
    ]
    excluded = [
        ...
        for label in labels
        for row in (
            excluded_row(label, "geometry_validity_external_target"),
            excluded_row(label, "relation_reliability_external_target"),
        )
        if row is not None
    ]
    ...
    if errors:
        status = "full_train_independent_support_vertical_v2_external_review_ingestion_errors"
        decision = "Fix external review ingestion errors before target audit."
        next_todo = "fix_full_train_independent_support_vertical_v2_external_review_ingestion_errors"
        ...
        decision = "External proxy labels are materialized, but basic probe still detects hidden/visible correlation risk."
        next_todo = "external_evidence_review_target_independence_audit"
    else:
        status = "full_train_independent_support_vertical_v2_external_review_ingested_ready_for_target_audit"
        decision = "External proxy labels are materialized. Run dedicated target-independence audit before posterior smoke."
        next_todo = "external_evidence_review_target_independence_audit"

    output_paths = {
        "summary": output_dir / "summary.json",
        "report": output_dir / "report.md",
        "validated_labels": output_dir / "validated_external_review_labels.jsonl",
        "geometry_validity_targets": output_dir / "geometry_validity_external_targets.jsonl",
        "relation_reliability_targets": output_dir / "relation_reliability_external_targets.jsonl",
        "geometry_validity_posterior_rows": output_dir / "geometry_validity_external_posterior_rows.jsonl",
        "relation_reliability_posterior_rows": output_dir / "relation_reliability_external_posterior_rows.jsonl",
        "excluded_targets": output_dir / "excluded_external_targets.jsonl",
    ...
    summary = {
        "schema_version": "h002_support_vertical_v2_external_review_ingestion_summary_v1",
        "status": status,
        ...
        "boundary": {
            ...
            "external_review_fields_as_model_input": False,
