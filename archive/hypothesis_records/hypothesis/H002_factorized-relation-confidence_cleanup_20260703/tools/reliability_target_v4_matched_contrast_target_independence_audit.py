#!/usr/bin/env python3
"""Audit H002 v4 matched-contrast target independence."""
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v4_matched_contrast_target_independence_audit_codex_proxy_user_requested"
...
TARGET_INPUTS = {
    RELIABILITY_TARGET: "relation_reliability_v4_posterior_candidates.jsonl",
    GEOMETRY_TARGET: "geometry_support_v4_posterior_candidates.jsonl",
    USEFULNESS_TARGET: "relation_usefulness_v4_posterior_candidates.jsonl",
}

TARGET_SCHEMA_VERSIONS = {
    RELIABILITY_TARGET: "h002_reliability_target_v4_matched_contrast_posterior_candidate_row_v1",
    GEOMETRY_TARGET: "h002_geometry_support_v4_matched_contrast_posterior_candidate_row_v1",
    USEFULNESS_TARGET: "h002_relation_usefulness_v4_matched_contrast_posterior_candidate_row_v1",
}

MATCHED_CONTRAST_KEYS = [
...
SLICE_SPECS = {
    ...
    "object_family_cell_balanced_v4": {
        ...
        "reason": "matched positives/negatives within object-family cell",
        "priority": 10,
    },
    "endpoint_flag_pattern_balanced_v4": {
    ...
    "object_label_family_balanced_v4": {
    ...
    "object_family_rank_balanced_v4": {
    ...
    "role_object_family_balanced_v4": {
        "balanced_keys": ["matched_contrast_role_hidden", "object_family_cell_hidden"],
        "reason": "matched positives/negatives within matched role and object-family cell",
        "priority": 17,
    },
}
...
def all_group_summaries(
    ...
    key_groups = [
        ("matched_contrast_design", "hidden_post_label_audit", MATCHED_CONTRAST_KEYS),
...
def counts_for(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "by_matched_role": dict(sorted(Counter(group_value(row, "matched_contrast_role_hidden") for row in rows).items())),
...
def slice_summary(
    ...
    matched = risk_summaries(summaries, "matched_contrast_design")
    ...
    return {
        ...
        "matched_contrast_design_risk_count": len(matched),
        ...
        "top_matched_contrast_design_risks": top_risks(matched),
...
def validate_ingestion_summary(summary: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    expected_status = "h002_reliability_target_v4_matched_contrast_label_ingested_with_probe_risk"
    expected_next = "reliability_target_v4_matched_contrast_target_independence_audit"
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if row.get("schema_version") != expected_schema:
            errors.append(
                {
                    "target_name": target_name,
                    "error_type": "unexpected_schema_version",
                    "row_number": index,
        ...
        if row.get("audit_only_v4_review_fields", {}).get("not_model_input") is not True:
            errors.append({"target_name": target_name, "error_type": "v4_review_fields_not_audit_only", "row_number": index, "blind_review_id": blind_id})
...
def per_target_decision(target_name: str, summaries: list[dict[str, Any]], errors: list[dict[str, Any]]) -> dict[str, Any]:
    original = next(item for item in summaries if item["slice_name"] == "original_matched_contrast_v4")
    ...
    if errors:
        ...
        decision = "Fix row validation errors before using v4 matched-contrast target slices."
        next_step = "fix_reliability_target_v4_matched_contrast_target_independence_errors"
    elif original["positive_sparse"]:
        status = "blocked_positive_sparse"
        decision = "The target is positive-sparse for posterior use."
        next_step = "reliability_target_v4_matched_contrast_path_decision"
    elif strict:
        status = "strict_controlled_slice_ready"
        decision = "A strict controlled slice clears matched-role, endpoint/object, construction, and object-label shortcut checks."
        next_step = "reliability_target_v4_matched_contrast_source_feature_join"
    elif diagnostic:
        ...
        decision = "No strict or diagnostic controlled slice clears construction and object-surface shortcut risks."
        next_step = "reliability_target_v4_matched_contrast_path_decision"
    return {
        "target_name": target_name,
        "status": status,
...
def global_decision(target_decisions: dict[str, Any], errors: list[dict[str, Any]]) -> tuple[str, str, str]:
    if errors:
        return (
            "h002_reliability_target_v4_matched_contrast_target_independence_audit_errors",
            "Fix v4 target audit validation errors before any target decision.",
            "fix_reliability_target_v4_matched_contrast_target_independence_errors",
        )
    reliability = target_decisions[RELIABILITY_TARGET]
    if reliability["status"] == "strict_controlled_slice_ready":
        return (
            "h002_reliability_target_v4_matched_contrast_target_independence_audit_reliability_ready",
            "The main relation reliability target has a strict controlled slice; source feature join may be planned next.",
            "reliability_target_v4_matched_contrast_source_feature_join",
        )
    if reliability["status"] == "diagnostic_slice_only":
        return (
            "h002_reliability_target_v4_matched_contrast_target_independence_audit_diagnostic_only",
    ...
    return (
        ...
        (
            "The main relation reliability target is balanced but no controlled slice clears matched-role, "
            "endpoint/object, construction, and visible object shortcut risks. Posterior smoke remains blocked."
        ),
        "reliability_target_v4_matched_contrast_path_decision",
    )
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# H002 Reliability Target V4 Matched Contrast Target Independence Audit",
        "",
        ...
        "- V4 labels are user-requested Codex proxy labels, not independent human annotation.",
        "- Hidden matched-contrast, construction, geometry-status, and packet metadata is used only after label lock for audit and slice construction.",
        "- V4 review fields, hidden sampling axes, audit packet paths, and multi-view evidence are not posterior inputs.",
    ...
    risk_fields = [
        ("matched_contrast_design", "top_matched_contrast_design_risks"),
    ...
    lines.extend(
        [
            ...
            "## Controlled Slices",
            "",
            "| Target | Slice | Rows | Pos | Neg | Matched | Endpoint/Object | Construction | Geometry Align | Object Risk | Strict | Diagnostic |",
            "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- |",
        ]
    )
    for item in sorted(
        ...
        lines.append(
            ...
            f"{item['matched_contrast_design_risk_count']} | {item['endpoint_object_structure_risk_count']} | "
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary = {
        "schema_version": "h002_reliability_target_v4_matched_contrast_target_independence_audit_summary_v1",
        "status": status,
