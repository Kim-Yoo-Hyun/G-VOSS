#!/usr/bin/env python3
"""Audit user-confirmed rank-band review target independence."""
...
import full_train_independent_support_vertical_v2_user_submitted_review_target_independence_audit as submitted_audit
...
DEFAULT_INGESTION_DIR = RGA_ROOT / "independent_support_vertical_v2_user_confirmed_review_ingestion_rank_band70"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "independent_support_vertical_v2_user_confirmed_review_target_independence_audit_rank_band70"

GEOMETRY_TARGET = "geometry_validity_user_confirmed_review_target"
RELIABILITY_TARGET = "relation_reliability_user_confirmed_review_target"
TARGET_INPUTS = {
    GEOMETRY_TARGET: "geometry_validity_user_confirmed_review_posterior_rows.jsonl",
    RELIABILITY_TARGET: "relation_reliability_user_confirmed_review_posterior_rows.jsonl",
}
...
def build_slices_for_target(target_name: str, rows: list[dict[str, Any]], output_dir: Path) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for slice_name, spec in submitted_audit.SLICE_SPECS.items():
        renamed = slice_name.replace("user_submitted", "user_confirmed")
        slice_rows = base.balanced_slice(rows, target_name, renamed, spec)
        path = slice_dir / f"{renamed}.jsonl"
        base.write_jsonl(path, slice_rows)
        groups, summaries = base.all_group_summaries(slice_rows, target_name, renamed)
        group_table.extend(groups)
        group_summary_rows.extend(summaries)
        slice_summaries.append(base.slice_summary(target_name, renamed, spec, slice_rows, summaries, path))
...
def per_target_decision(target_name: str, summaries: list[dict[str, Any]], errors: list[dict[str, Any]]) -> dict[str, Any]:
    original = next(item for item in summaries if item["slice_name"] == "original_user_confirmed_review")
    ...
    if errors:
        ...
        decision = "Fix row validation errors before using any user-confirmed review target slice."
        next_step = "fix_user_confirmed_review_target_independence_audit_errors"
    elif strict:
        status = "strict_controlled_slice_ready"
        decision = "A strict user-confirmed review slice clears harmful prior carryover and construction-risk checks."
        next_step = "user_confirmed_rank_band70_source_feature_join_then_controlled_posterior_smoke"
    elif construction:
        ...
        next_step = "expand_user_confirmed_labels_or_revise_sampling_protocol"
    else:
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# H002 User-Confirmed Review Target Independence Audit",
        "",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if all_validation_errors:
        status = "full_train_independent_support_vertical_v2_user_confirmed_review_target_independence_audit_errors"
        decision = "Fix validation errors before any user-confirmed target slice can be used."
        next_todo = "fix_user_confirmed_rank_band70_target_independence_audit_errors"
    elif relation_strict:
        status = "full_train_independent_support_vertical_v2_user_confirmed_review_target_independence_audit_relation_strict_slice_ready"
        decision = "A strict relation-reliability slice exists after user confirmation."
        next_todo = "user_confirmed_rank_band70_source_feature_join"
    elif relation_construction:
        status = "full_train_independent_support_vertical_v2_user_confirmed_review_target_independence_audit_strict_blocked_construction_slice_available"
        decision = "No strict relation-reliability slice clears harmful prior carryover. Construction-only diagnostic slice exists."
        next_todo = "expand_user_confirmed_labels_or_revise_sampling_protocol"
    else:
        status = "full_train_independent_support_vertical_v2_user_confirmed_review_target_independence_audit_blocked"
        decision = "User confirmation resolves provenance, but no size-ready strict or construction-only controlled slice exists."
        next_todo = "expand_user_confirmed_labels_or_revise_sampling_protocol"
    ...
    summary = {
        "schema_version": "h002_support_vertical_v2_user_confirmed_review_target_independence_audit_summary_v1",
        "status": status,
        ...
        "boundary": {
            ...
            "artifact_reviewer_id_indicates_codex_diagnostic": ingestion_summary.get("boundary", {}).get("artifact_reviewer_id_indicates_codex_diagnostic"),
