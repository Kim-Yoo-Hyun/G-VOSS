#!/usr/bin/env python3
"""Fill the informative-anchor H002 reliability target v3 sheet."""
...
DEFAULT_INPUT_SHEET = ASSET_DIR / "informative_anchor_full_label_sheet.tsv"
...
DEFAULT_ASSET_SUMMARY = ASSET_DIR / "summary.json"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v3_informative_anchor_label_fill_codex_proxy_user_requested"

SCHEMA_VERSION = "h002_reliability_target_v3_informative_anchor_label_fill_summary_v1"
STATUS_READY = "h002_reliability_target_v3_informative_anchor_label_filled_codex_proxy_user_requested"
STATUS_ERROR = "h002_reliability_target_v3_informative_anchor_label_fill_errors"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--asset-summary", type=Path, default=DEFAULT_ASSET_SUMMARY)
...
def validate_inputs(
    ...
    if asset_summary.get("next_todo") != "reliability_target_v3_informative_anchor_label_fill":
        errors.append({"error_type": "unexpected_asset_next_todo", "value": asset_summary.get("next_todo")})

    boundary = asset_summary.get("boundary", {})
    for key in ["validation_usage", "test_usage", "posterior_trained", "posterior_smoke_allowed"]:
        if boundary.get(key) is not False:
            errors.append({"error_type": f"asset_boundary_{key}_not_false", "value": boundary.get(key)})

    summary_rows = asset_summary.get("counts", {}).get("full_label_sheet_rows")
    if summary_rows != len(visible_rows):
        errors.append(
            {
                "error_type": "visible_row_count_mismatch",
                "asset_summary_rows": summary_rows,
                "visible_rows": len(visible_rows),
            }
        )
    if len(manifest_rows) != len(visible_rows):
        errors.append(
            {
                "error_type": "manifest_visible_row_count_mismatch",
            ...
            errors.append(
                {
                    ...
                    "error_type": "visible_row_not_packet_ready",
...
def fill_visible_rows(rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        filled["label_notes_v3"] = (
            "codex proxy informative-anchor v3 fill; label decision used only visible "
            "subject/object/predicate identity and packet availability heuristic; hidden "
            "sampling/proxy manifest was not used before label lock"
        )
...
def label_record(row: dict[str, Any]) -> dict[str, Any]:
    ...
    record["schema_version"] = "h002_reliability_target_v3_informative_anchor_proxy_label_v1"
    record["provenance"]["batch_name"] = "reliability_target_v3_informative_anchor_label_fill"
    record["provenance"]["informative_anchor_sheet"] = True
    return record
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# H002 Reliability Target V3 Informative Anchor Label Fill",
        "",
        ...
        "- Hidden anchor category, sampling category, source score/rank, `p_geom_valid`, geometry status, label-match status, endpoint pattern, and numeric witness values are not used for label decisions.",
        ...
        f"| fill validation errors | {counts['validation_errors']} |",
        "",
        "## Geometry Support",
        "",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    asset_summary_path = as_abs(args.asset_summary)
    ...
    asset_summary = read_json(asset_summary_path)

    input_errors = validate_inputs(asset_summary, visible_rows, manifest_rows)
    ...
    output_paths = {
        ...
        "completed_sheet": output_dir / "completed_informative_anchor_label_sheet_codex_proxy_user_requested.tsv",
        "v3_proxy_labels": output_dir / "informative_anchor_v3_proxy_labels.jsonl",
    ...
    next_todo = (
        "fix_reliability_target_v3_informative_anchor_label_fill"
        if errors
        else "reliability_target_v3_informative_anchor_label_ingestion"
    )
    decision = (
        "Input or fill validation errors block ingestion."
        if errors
        else (
            "Filled the full 160-row informative-anchor v3 sheet as a user-requested Codex proxy. "
    ...
    summary = {
        ...
        "decision": decision,
        "input_paths": {
            "informative_anchor_full_label_sheet": rel_path(input_sheet),
            "v3_label_schema": rel_path(schema_path),
            "post_label_manifest": rel_path(manifest_path),
            "asset_packet_summary": rel_path(asset_summary_path),
        },
        ...
        "boundary": {
            ...
            "used_anchor_category_for_label_decision": False,
