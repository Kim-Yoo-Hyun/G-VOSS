#!/usr/bin/env python3
"""Audit target independence for H002 v18 attachment-deferred targets."""
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v18_attachment_deferred_target_independence_audit"

EXPECTED_INGESTION_STATUS = "h002_reliability_target_v18_attachment_deferred_label_ingested_positive_sparse_with_probe_risk"
EXPECTED_NEXT_TODO = "reliability_target_v18_attachment_deferred_target_independence_audit"
NEXT_TODO = "reliability_target_v18_attachment_deferred_path_decision_after_audit"

SCHEMA_VERSION = "h002_reliability_target_v18_attachment_deferred_target_independence_audit_v1"
...
TARGET_SPECS = {
    ...
    "usefulness_binary": {
        "field": "relation_usefulness_binary_target",
        "usable": "relation_usefulness_binary_usable",
        "role": "auxiliary",
    },
    ...
    "endpoint_multiclass": {
        "field": "endpoint_identity_state_v18",
        "usable": None,
        "role": "diagnostic",
    },
    "coverage_multiclass": {
        "field": "coverage_state_v18",
        "usable": None,
        "role": "provenance",
    },
}
...
RISK_PREDICTORS = [
    ...
    "attachment_support_score_bin_hidden",
    "attachment_contradiction_score_bin_hidden",
...
SLICE_SPECS = {
    ...
    "same_relation_family": ["relation_family_visible"],
    "same_candidate_role": ["candidate_role_hidden"],
    "same_cell": ["cell_id_hidden"],
    "same_provisional_status": ["provisional_status_hidden"],
    "same_anchor_bucket": ["anchor_bucket_hidden"],
    "same_rank_band": ["rank_band_hidden"],
    "same_semantic_score_bin": ["semantic_score_bin_hidden"],
    "same_bucket_top100": ["bucket_top100_hidden"],
    "same_sampling_queue": ["sampling_queue_hidden"],
    "same_geometry_status": ["geometry_status_hidden"],
    "same_reason_family": ["reason_family_hidden"],
    "same_machine_hint": ["machine_hint_hidden"],
    "same_label_match": ["label_match_status_hidden"],
    "same_source_geometry_family": ["source_geometry_family_hidden"],
    "same_source_geometry_predicate": ["source_geometry_predicate_hidden"],
    "same_support_score_bin": ["attachment_support_score_bin_hidden"],
    "same_contradiction_score_bin": ["attachment_contradiction_score_bin_hidden"],
    "same_visible_pair": ["subject_object_visible_pair"],
    "same_scan": ["scan_id"],
    "same_subject_label": ["subject_label"],
    "same_object_label": ["object_label"],
    "same_geometry_witness": ["geometry_witness_summary_v18"],
    "same_attachment_witness": ["attachment_witness_summary_v18"],
    "same_uncertainty_summary": ["uncertainty_summary_v18"],
    "same_predicate_rank": ["predicate_label", "rank_band_hidden"],
    "same_predicate_geometry": ["predicate_label", "geometry_status_hidden"],
    "same_predicate_anchor": ["predicate_label", "anchor_bucket_hidden"],
    "same_predicate_cell": ["predicate_label", "cell_id_hidden"],
    "same_cell_rank": ["cell_id_hidden", "rank_band_hidden"],
    "same_cell_geometry": ["cell_id_hidden", "geometry_status_hidden"],
    "same_rank_geometry": ["rank_band_hidden", "geometry_status_hidden"],
    "same_visible_pair_rank": ["subject_object_visible_pair", "rank_band_hidden"],
    "same_visible_pair_geometry": ["subject_object_visible_pair", "geometry_status_hidden"],
    "same_anchor_geometry": ["anchor_bucket_hidden", "geometry_status_hidden"],
    "same_reason_rank": ["reason_family_hidden", "rank_band_hidden"],
    "same_attachment_uncertainty": ["attachment_witness_summary_v18", "uncertainty_summary_v18"],
}

CONTROL_EQUIVALENCE = {
    "predicate_label": {"predicate_label", "relation_family_visible"},
    "relation_family_visible": {"predicate_label", "relation_family_visible"},
    "candidate_role_hidden": {"candidate_role_hidden"},
    "cell_id_hidden": {"cell_id_hidden", "sampling_queue_hidden", "reason_family_hidden", "machine_hint_hidden"},
    "sampling_queue_hidden": {"cell_id_hidden", "sampling_queue_hidden", "reason_family_hidden", "machine_hint_hidden"},
    "reason_family_hidden": {"cell_id_hidden", "sampling_queue_hidden", "reason_family_hidden", "machine_hint_hidden"},
    "machine_hint_hidden": {"cell_id_hidden", "sampling_queue_hidden", "reason_family_hidden", "machine_hint_hidden"},
    ...
    "semantic_score_bin_hidden": {"semantic_score_bin_hidden"},
    "geometry_status_hidden": {"geometry_status_hidden", "provisional_status_hidden"},
    "provisional_status_hidden": {"geometry_status_hidden", "provisional_status_hidden"},
    "attachment_support_score_bin_hidden": {"attachment_support_score_bin_hidden", "attachment_contradiction_score_bin_hidden"},
    "attachment_contradiction_score_bin_hidden": {"attachment_support_score_bin_hidden", "attachment_contradiction_score_bin_hidden"},
    "geometry_witness_summary_v18": {"geometry_witness_summary_v18", "attachment_witness_summary_v18"},
    "attachment_witness_summary_v18": {"geometry_witness_summary_v18", "attachment_witness_summary_v18"},
    "source_geometry_family_hidden": {"source_geometry_family_hidden", "source_geometry_predicate_hidden"},
    "source_geometry_predicate_hidden": {"source_geometry_family_hidden", "source_geometry_predicate_hidden"},
}

ALLOWED_PREDICATES = {"attached to", "hanging on", "connected to"}
ALLOWED_FAMILY = "attachment_deferred"
...
def validate_ingestion(summary: dict[str, Any], rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if row.get("predicate_label") not in ALLOWED_PREDICATES:
            errors.append({"error_type": "unexpected_predicate", "blind_review_id": row.get("blind_review_id"), "predicate": row.get("predicate_label")})
        if row.get("predicate_family") != ALLOWED_FAMILY:
            errors.append({"error_type": "unexpected_family", "blind_review_id": row.get("blind_review_id"), "family": row.get("predicate_family")})
    return errors
...
def score_bin(value: Any, *, prefix: str) -> str:
    try:
        score = float(value)
    except (TypeError, ValueError):
...
def enrich_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        item["semantic_score_bin_hidden"] = score_bin(row.get("semantic_score_norm_hidden"), prefix="semantic_score")
        item["attachment_support_score_bin_hidden"] = score_bin(row.get("attachment_witness_support_score_hidden"), prefix="support")
        item["attachment_contradiction_score_bin_hidden"] = score_bin(row.get("attachment_witness_contradiction_score_hidden"), prefix="contradiction")
...
def majority_risk(rows: list[dict[str, Any]], predictor: str, target_name: str) -> dict[str, Any]:
    ...
    for group_value, counter in groups.items():
        ...
        if len(counter) > 1:
            large_pure = large_pure or (
                total >= RISK_THRESHOLDS["large_group_rows"]
                and majority_rate >= RISK_THRESHOLDS["large_group_purity"]
            )
        top_groups.append(
            {
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# H002 V18 Attachment-Deferred Target Independence Audit",
        "",
    ...
    lines.extend(
        [
            ...
            "## Key Findings",
            "",
            f"- Primary attachment reliability target: rows `{relation['rows']}`, classes `{relation['class_counts']}`, min class `{relation['min_class_count']}`.",
            ...
            "- The primary target is still not posterior-ready because positive class mass remains below the predeclared gate.",
            "",
            "## Boundary",
            "",
            "- Train-only rows.",
            ...
            "- Multi-view is not used as a model input.",
            "",
            "## Next",
            "",
            f"`{summary['next_todo']}`",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if validation_errors:
        status = "h002_reliability_target_v18_attachment_deferred_target_independence_audit_errors"
        decision = "Validation errors remain; posterior smoke is not allowed."
    elif relation["posterior_allowed"]:
        status = "h002_reliability_target_v18_attachment_deferred_target_independence_audit_ready"
        ...
        decision = "Primary attachment reliability target is blocked by positive sparsity; shortcut-risk probes also remain high."
    else:
        status = "h002_reliability_target_v18_attachment_deferred_target_independence_audit_blocked_shortcut_risk"
        decision = "Primary attachment reliability target has no strict clear independent slice; posterior smoke remains blocked."
