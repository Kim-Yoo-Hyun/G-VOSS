#!/usr/bin/env python3
"""Fill the v5 cell-contrast reliability label sheet as a Codex proxy."""
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v5_cell_contrast_label_fill_codex_proxy_user_requested"

SCHEMA_VERSION = "h002_reliability_target_v5_cell_contrast_label_fill_summary_v1"
STATUS_READY = "h002_reliability_target_v5_cell_contrast_label_filled_codex_proxy_user_requested"
STATUS_ERROR = "h002_reliability_target_v5_cell_contrast_label_fill_errors"
REVIEWER_ID = "(codex_proxy_v5_user_requested_visible_only)"
REVIEW_ROUND = "1"
...
GENERIC_LABELS = {"item", "object", "objects", "furniture", "thing", "stuff"}
...
def validate_inputs(
    ...
    if readiness_summary.get("next_todo") != "reliability_target_v5_cell_contrast_label_fill":
        errors.append({"error_type": "unexpected_readiness_next_todo", "value": readiness_summary.get("next_todo")})
    if readiness_summary.get("status") != "h002_reliability_target_v5_cell_contrast_label_readiness_ready_for_label_fill":
    ...
    for row_number, row in enumerate(rows, start=2):
        for field in REVIEW_FIELDS + ["label_notes_v5"]:
    ...
    for manifest in manifest_rows:
        if not manifest_packet_paths_exist(manifest):
            errors.append({"blind_review_id": manifest.get("blind_review_id"), "error_type": "manifest_packet_path_missing"})
    return errors
...
def visible_geometry_and_usefulness(row: dict[str, str]) -> tuple[str, str, str, str]:
    ...
    return geometry_v5, usefulness_v5, usefulness, uncertainty
...
def label_axes(row: dict[str, str]) -> dict[str, str]:
    ...
    if endpoint == "endpoint_identity_issue":
        return {
            "endpoint_identity_v5": endpoint,
            "pair_evaluability_v5": "not_evaluable",
            "geometry_support_v5": "not_evaluable",
            "relation_usefulness_v5": "uncertain",
            "relation_reliability_v5": "uncertain",
            "primary_reason_v5": "endpoint_identity_issue",
            "uncertainty_reason_v5": "object_segmentation_issue",
        }
    if endpoint == "uncertain":
        return {
            "endpoint_identity_v5": endpoint,
            "pair_evaluability_v5": "needs_more_evidence",
            "geometry_support_v5": "ambiguous",
            "relation_usefulness_v5": "uncertain",
            "relation_reliability_v5": "uncertain",
            "primary_reason_v5": "endpoint_identity_issue",
            "uncertainty_reason_v5": "object_segmentation_issue",
    ...
    if packet_status == "limited_view_evaluable":
        return {
            ...
            "pair_evaluability_v5": "needs_more_evidence",
            "geometry_support_v5": "ambiguous",
            "relation_usefulness_v5": usefulness if usefulness != "not_a_relation" else "uncertain",
            "relation_reliability_v5": "uncertain",
            "primary_reason_v5": "insufficient_evidence",
            "uncertainty_reason_v5": "occlusion_or_view_limit",
        }
    if geometry == "contradicts":
        return {
            "endpoint_identity_v5": endpoint,
            "pair_evaluability_v5": "evaluable",
            "geometry_support_v5": geometry,
            "relation_usefulness_v5": usefulness,
            "relation_reliability_v5": "unreliable",
            "primary_reason_v5": "geometric_contradiction",
            "uncertainty_reason_v5": "",
        }
    if usefulness == "trivial_or_redundant":
        return {
            ...
            "relation_usefulness_v5": usefulness,
            "relation_reliability_v5": "unreliable",
            "primary_reason_v5": "trivial_room_surface_or_structure",
            "uncertainty_reason_v5": "",
        }
    if usefulness == "not_a_relation":
        return {
            ...
            "relation_usefulness_v5": usefulness,
            "relation_reliability_v5": "unreliable",
            "primary_reason_v5": "semantic_ontology_mismatch",
            "uncertainty_reason_v5": "",
        }
    if geometry == "supports" and usefulness == "useful_nontrivial":
        return {
            ...
            "geometry_support_v5": geometry,
            "relation_usefulness_v5": usefulness,
            "relation_reliability_v5": "reliable",
            "primary_reason_v5": "geometric_support",
            "uncertainty_reason_v5": "",
        }
    if geometry == "ambiguous" and raw_usefulness == "ontology_mismatch":
        return {
            ...
            "pair_evaluability_v5": "evaluable",
            "geometry_support_v5": "ambiguous",
            "relation_usefulness_v5": "not_a_relation",
            "relation_reliability_v5": "unreliable",
            "primary_reason_v5": "semantic_ontology_mismatch",
            "uncertainty_reason_v5": "",
        }
    return {
        ...
        "relation_reliability_v5": "uncertain",
        "primary_reason_v5": "insufficient_evidence",
        "uncertainty_reason_v5": uncertainty_reason_for(row, uncertainty_hint),
    }
...
def fill_row(row: dict[str, str]) -> dict[str, Any]:
    ...
    filled["label_notes_v5"] = (
        "codex proxy v5 visible-only fill; used visible subject/object/predicate labels, "
        "predicate family question, cue text, and packet status; did not use hidden cell-contrast "
...
def validate_filled_rows(rows: list[dict[str, Any]], schema: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        for field in REVIEW_FIELDS:
            value = row.get(field)
            if value is None or (value == "" and field != "uncertainty_reason_v5"):
        ...
        if not str(row.get("label_notes_v5", "")).strip():
            errors.append({"row_number": row_number, "blind_review_id": blind_id, "field": "label_notes_v5", "error_type": "missing_label_notes"})
    return errors


def label_record(row: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "h002_reliability_target_v5_cell_contrast_proxy_label_v1",
        ...
        "v5_review_fields": {
            ...
            "endpoint_identity_v5": row["endpoint_identity_v5"],
            "pair_evaluability_v5": row["pair_evaluability_v5"],
            "geometry_support_v5": row["geometry_support_v5"],
            "relation_usefulness_v5": row["relation_usefulness_v5"],
            "relation_reliability_v5": row["relation_reliability_v5"],
            "primary_reason_v5": row["primary_reason_v5"],
            "uncertainty_reason_v5": row["uncertainty_reason_v5"],
            "label_notes_v5": row["label_notes_v5"],
        },
        "provenance": {
            "batch_name": "reliability_target_v5_cell_contrast_label_fill",
            ...
            "used_cell_contrast_role_for_label_decision": False,
...
def grouped_diagnostics(filled_rows: list[dict[str, Any]], manifest_rows: list[dict[str, Any]], group_key: str) -> list[dict[str, Any]]:
    ...
    for value, rows in sorted(grouped.items()):
        reliability = Counter(row["relation_reliability_v5"] for row in rows)
        geometry = Counter(row["geometry_support_v5"] for row in rows)
        usefulness = Counter(row["relation_usefulness_v5"] for row in rows)
        primary = Counter(row["primary_reason_v5"] for row in rows)
        diagnostics.append(
            {
...
def pair_diagnostics(filled_rows: list[dict[str, Any]], manifest_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in filled_rows:
        manifest = manifest_by_id.get(str(row["blind_review_id"]), {})
        pair_id = str(manifest.get("cell_contrast_pair_id_hidden", "missing"))
    ...
    for pair_id, rows in sorted(grouped.items()):
        ...
        for row in rows:
            manifest = manifest_by_id.get(str(row["blind_review_id"]), {})
            roles[str(manifest.get("cell_contrast_role_hidden", "missing"))] += 1
        diagnostics.append(
            {
                ...
                "unreliable": reliability.get("unreliable", 0),
                "uncertain": reliability.get("uncertain", 0),
                "pair_label_pattern": "/".join(sorted(row["relation_reliability_v5"] for row in rows)),
            }
        )
    return diagnostics
...
def binary_target_rows(filled_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in filled_rows:
        label = row["relation_reliability_v5"]
        if label == "uncertain":
            continue
        targets.append(
            {
                "schema_version": "h002_reliability_target_v5_cell_contrast_binary_target_v1",
                "blind_review_id": row["blind_review_id"],
                "scan_id": row["scan_id"],
                "scene_context_id": row["scene_context_id"],
                "subject_id": row["subject_id"],
                ...
                "predicate_label": row["predicate_label"],
                "predicate_family": row["predicate_family"],
                "relation_reliability_v5": label,
                "relation_reliability_v5_binary": 1 if label == "reliable" else 0,
                "target_source": "codex_proxy_user_requested_visible_only_v5_label_fill",
                "split": "train_only",
                "paper_evidence_allowed": False,
            }
        )
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    reliability = counts["relation_reliability_v5"]
    geometry = counts["geometry_support_v5"]
    usefulness = counts["relation_usefulness_v5"]
    lines = [
        "# H002 Reliability Target V5 Cell Contrast Label Fill",
        "",
        ...
        "- Hidden cell-contrast role, pair id, semantic score/rank, `p_geom_valid`, geometry status, label-match status, and target-construction metadata are not used before label lock.",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    reliability_counts = Counter(row["relation_reliability_v5"] for row in filled_rows)
    geometry_counts = Counter(row["geometry_support_v5"] for row in filled_rows)
    usefulness_counts = Counter(row["relation_usefulness_v5"] for row in filled_rows)
    family_counts = Counter(row["predicate_family"] for row in filled_rows)
    endpoint_counts = Counter(row["endpoint_identity_v5"] for row in filled_rows)
    evaluability_counts = Counter(row["pair_evaluability_v5"] for row in filled_rows)
    primary_counts = Counter(row["primary_reason_v5"] for row in filled_rows)
    uncertainty_counts = Counter(row["uncertainty_reason_v5"] for row in filled_rows)
    packet_status_counts = Counter(row["evidence_packet_status"] for row in filled_rows)
    target_counts = Counter(row["relation_reliability_v5_binary"] for row in targets)

    output_paths = {
        "summary": output_dir / "summary.json",
        "report": output_dir / "report.md",
        "completed_sheet": output_dir / "completed_v5_cell_contrast_label_sheet_codex_proxy_user_requested.tsv",
        "v5_proxy_labels": output_dir / "v5_proxy_labels.jsonl",
        "relation_reliability_v5_binary_targets": output_dir / "relation_reliability_v5_binary_targets.jsonl",
    ...
    next_todo = "fix_reliability_target_v5_cell_contrast_label_fill" if errors else "reliability_target_v5_cell_contrast_label_ingestion"
    decision = (
        "Input or fill validation errors block ingestion."
        if errors
        else (
            "Filled the 72-row / 36-pair v5 cell-contrast sheet as a user-requested Codex proxy. "
            "This creates hypothesis-stage labels and binary target rows for ingestion and "
            "target-independence audit, but it is not paper metric evidence and does not unlock "
            "posterior smoke by itself."
        )
    )
    ...
    summary = {
        ...
        "counts": {
            ...
            "endpoint_identity_v5": dict(sorted(endpoint_counts.items())),
            "pair_evaluability_v5": dict(sorted(evaluability_counts.items())),
            "geometry_support_v5": dict(sorted(geometry_counts.items())),
            "relation_usefulness_v5": dict(sorted(usefulness_counts.items())),
            "relation_reliability_v5": dict(sorted(reliability_counts.items())),
            "primary_reason_v5": dict(sorted(primary_counts.items())),
            "uncertainty_reason_v5": dict(sorted(uncertainty_counts.items())),
    ...
    write_jsonl(output_paths["v5_proxy_labels"], label_rows)
    write_jsonl(output_paths["relation_reliability_v5_binary_targets"], targets)
