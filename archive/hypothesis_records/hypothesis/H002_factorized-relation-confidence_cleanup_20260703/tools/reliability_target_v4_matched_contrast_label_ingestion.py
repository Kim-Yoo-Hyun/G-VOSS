#!/usr/bin/env python3
"""Ingest v4 matched-contrast labels after proxy fill."""
...
FILL_DIR = RGA_ROOT / "reliability_target_v4_matched_contrast_label_fill_codex_proxy_user_requested"
READINESS_DIR = RGA_ROOT / "reliability_target_v4_matched_contrast_label_readiness"

DEFAULT_COMPLETED_SHEET = FILL_DIR / "completed_v4_matched_contrast_label_sheet_codex_proxy_user_requested.tsv"
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v4_matched_contrast_label_ingestion_codex_proxy_user_requested"

LABEL_SOURCE = "codex_proxy_reliability_target_v4_matched_contrast_user_requested"
...
RELIABILITY_MULTICLASS = "relation_reliability_v4_multiclass_target"
...
def validate_fill_summary(fill_summary: dict[str, Any]) -> list[dict[str, Any]]:
    errors: list[dict[str, Any]] = []
    expected_status = "h002_reliability_target_v4_matched_contrast_label_filled_codex_proxy_user_requested"
    ...
    expected_next = "reliability_target_v4_matched_contrast_label_ingestion"
    ...
    expected_false = [
        ...
        "used_contrast_role_for_label_decision",
...
def validate_row(row: dict[str, str], row_number: int, schema: dict[str, Any], manifest: dict[str, Any] | None) -> list[dict[str, Any]]:
    ...
    for field in COMPLETION_FIELDS:
        ...
        if not value and field != "uncertainty_reason_v4":
...
def deployable_evidence_after_label_lock(row: dict[str, str], manifest: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "forbidden_as_posterior_input": {
            "v4_review_fields": True,
            "matched_contrast_role_hidden": True,
            "matched_contrast_pair_id_hidden": True,
...
def review_fields(row: dict[str, str]) -> dict[str, Any]:
    return {
        ...
        "endpoint_identity_v4": row.get("endpoint_identity_v4"),
        "pair_evaluability_v4": row.get("pair_evaluability_v4"),
        "geometry_support_v4": row.get("geometry_support_v4"),
        "relation_usefulness_v4": row.get("relation_usefulness_v4"),
        "relation_reliability_v4": row.get("relation_reliability_v4"),
        "primary_reason_v4": row.get("primary_reason_v4"),
        "uncertainty_reason_v4": row.get("uncertainty_reason_v4"),
        "label_notes_v4": row.get("label_notes_v4"),
...
def derive_reliability_binary(row: dict[str, str]) -> dict[str, Any]:
    value = row["relation_reliability_v4"]
    if value == "reliable":
        return {"target_name": RELIABILITY_TARGET, "target_y": 1, "target_use": "positive", "reason": "v4_relation_reliability_reliable"}
    if value == "unreliable":
        return {"target_name": RELIABILITY_TARGET, "target_y": 0, "target_use": "negative", "reason": "v4_relation_reliability_unreliable"}
...
def derive_geometry_binary(row: dict[str, str]) -> dict[str, Any]:
    value = row["geometry_support_v4"]
    if value == "supports":
        return {"target_name": GEOMETRY_TARGET, "target_y": 1, "target_use": "positive", "reason": "v4_geometry_supports"}
    if value == "contradicts":
        return {"target_name": GEOMETRY_TARGET, "target_y": 0, "target_use": "negative", "reason": "v4_geometry_contradicts"}
...
def derive_usefulness_binary(row: dict[str, str]) -> dict[str, Any]:
    value = row["relation_usefulness_v4"]
    if value == "useful_nontrivial":
        return {"target_name": USEFULNESS_TARGET, "target_y": 1, "target_use": "positive", "reason": "v4_relation_usefulness_useful_nontrivial"}
    if value in {"trivial_or_redundant", "not_a_relation"}:
        return {"target_name": USEFULNESS_TARGET, "target_y": 0, "target_use": "negative", "reason": f"v4_relation_usefulness={value}"}
...
def relation_multiclass(row: dict[str, str]) -> dict[str, Any]:
    return {
        ...
        "target_y": row["relation_reliability_v4"],
        "target_use": "multiclass",
        "reason": f"v4_relation_reliability={row['relation_reliability_v4']}",
    }


def make_label(row: dict[str, str], manifest: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "h002_reliability_target_v4_matched_contrast_ingested_label_v1",
        **base_identity(row, manifest),
        ...
        "v4_review_fields": review_fields(row),
        "relation_reliability_v4_binary_target": derive_reliability_binary(row),
        "geometry_support_v4_binary_target": derive_geometry_binary(row),
        "relation_usefulness_v4_binary_target": derive_usefulness_binary(row),
        "relation_reliability_v4_multiclass_target": relation_multiclass(row),
...
def target_row(label: dict[str, Any], target_key: str, schema_version: str) -> dict[str, Any] | None:
    ...
    return {
        ...
        "actual_user_reviewer": False,
        "paper_locked": False,
        "matched_contrast_role_hidden": hidden.get("matched_contrast_role_hidden"),
        "contrast_role_hidden": hidden.get("contrast_role_hidden"),
        "matched_contrast_level_hidden": hidden.get("matched_contrast_level_hidden"),
...
def multiclass_target_row(label: dict[str, Any]) -> dict[str, Any]:
    row = target_row(label, "relation_reliability_v4_multiclass_target", "h002_reliability_target_v4_matched_contrast_multiclass_row_v1")
...
def posterior_candidate_row(label: dict[str, Any], target_key: str, schema_version: str) -> dict[str, Any] | None:
    ...
    return {
        **row,
        ...
        "audit_only_v4_review_fields": label["v4_review_fields"],
        ...
        "audit_note": (
            ...
            "Do not use v4 review fields, matched contrast role, pair id, source queue, "
...
def excluded_target_row(label: dict[str, Any], target_key: str) -> dict[str, Any] | None:
    ...
    if target["target_y"] is not None:
        return None
    return {
        "schema_version": "h002_reliability_target_v4_matched_contrast_excluded_target_v1",
        "target_name": target["target_name"],
        "target_y": None,
        "target_use": target["target_use"],
        "target_reason": target["reason"],
        ...
        "subject_label": label["subject_label"],
        "object_label": label["object_label"],
        "v4_review_fields": label["v4_review_fields"],
    }
...
def target_independence_probe(target_rows: list[dict[str, Any]], target_name: str) -> dict[str, Any]:
    ...
    if not target_rows:
        return {
            "schema_version": "h002_reliability_target_v4_matched_contrast_independence_probe_v1",
            "target_name": target_name,
...
def count_binary_target(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "by_matched_role": nested_counts(rows, "matched_contrast_role_hidden"),
...
def axis_counts(labels: list[dict[str, Any]]) -> dict[str, dict[str, int]]:
    ...
    for axis in [
        ...
        output[axis] = dict(sorted(Counter(label["v4_review_fields"].get(axis) for label in labels).items()))
    return output
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "- V4 review fields are target/audit fields and must not be posterior input.",
...
def build_summary_status(errors: list[dict[str, Any]], probes: dict[str, dict[str, Any]], reliability_count: dict[str, Any]) -> tuple[str, str, str]:
    ...
    if errors:
        return (
            "h002_reliability_target_v4_matched_contrast_label_ingestion_errors",
            "Fix v4 matched-contrast label ingestion errors before target audit.",
            "fix_reliability_target_v4_matched_contrast_label_ingestion_errors",
        )
    if not enough_reliability_mass and any_probe_risk:
        return (
            "h002_reliability_target_v4_matched_contrast_label_ingested_positive_sparse_with_probe_risk",
            (
                "V4 labels are ingested, but the relation reliability target is positive-sparse and "
                "shortcut probes flag construction risk. Run target-independence audit before posterior smoke."
            ),
            "reliability_target_v4_matched_contrast_target_independence_audit",
        )
    if not enough_reliability_mass:
        return (
            "h002_reliability_target_v4_matched_contrast_label_ingested_positive_sparse",
            "V4 labels are ingested, but the relation reliability target is positive-sparse.",
            "reliability_target_v4_matched_contrast_target_independence_audit",
        )
    if any_probe_risk:
        return (
            ...
            (
                "V4 labels are ingested and binary target mass is balanced, but hidden/visible shortcut "
                "probes still flag target-construction risk. Run target-independence audit before posterior smoke."
            ),
            "reliability_target_v4_matched_contrast_target_independence_audit",
        )
    return (
        "h002_reliability_target_v4_matched_contrast_label_ingested_ready_for_target_independence_audit",
        "V4 labels are ingested. Run dedicated target-independence audit before posterior smoke.",
        "reliability_target_v4_matched_contrast_target_independence_audit",
    )
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    reliability_targets = [
        row
        for row in (
            target_row(label, "relation_reliability_v4_binary_target", "h002_reliability_target_v4_matched_contrast_binary_row_v1")
            for label in labels
        )
        if row is not None
    ]
    geometry_targets = [
        row
        for row in (
            target_row(label, "geometry_support_v4_binary_target", "h002_geometry_support_v4_matched_contrast_binary_row_v1")
            for label in labels
        )
        if row is not None
    ]
    usefulness_targets = [
        row
        for row in (
            target_row(label, "relation_usefulness_v4_binary_target", "h002_relation_usefulness_v4_matched_contrast_binary_row_v1")
            for label in labels
        )
        if row is not None
    ]
    ...
    reliability_posterior = [
        row
        for row in (
            posterior_candidate_row(label, "relation_reliability_v4_binary_target", "h002_reliability_target_v4_matched_contrast_posterior_candidate_row_v1")
            for label in labels
        )
        if row is not None
    ]
    geometry_posterior = [
        row
        for row in (
            posterior_candidate_row(label, "geometry_support_v4_binary_target", "h002_geometry_support_v4_matched_contrast_posterior_candidate_row_v1")
            for label in labels
        )
        if row is not None
    ]
    usefulness_posterior = [
        row
        for row in (
            posterior_candidate_row(label, "relation_usefulness_v4_binary_target", "h002_relation_usefulness_v4_matched_contrast_posterior_candidate_row_v1")
            for label in labels
        )
        if row is not None
    ]
    ...
    excluded_targets = [
        ...
        for label in labels
        for row in (
            excluded_target_row(label, "relation_reliability_v4_binary_target"),
            excluded_target_row(label, "geometry_support_v4_binary_target"),
            excluded_target_row(label, "relation_usefulness_v4_binary_target"),
        )
        if row is not None
    ]
    ...
    status, decision, next_todo = build_summary_status(errors, probes, reliability_count)

    output_paths = {
        "summary": output_dir / "summary.json",
        "report": output_dir / "report.md",
        "validated_v4_labels": output_dir / "validated_v4_labels.jsonl",
        "relation_reliability_binary_targets": output_dir / "relation_reliability_v4_binary_targets.jsonl",
        "geometry_support_binary_targets": output_dir / "geometry_support_v4_binary_targets.jsonl",
        "relation_usefulness_binary_targets": output_dir / "relation_usefulness_v4_binary_targets.jsonl",
        "relation_reliability_multiclass_targets": output_dir / "relation_reliability_v4_multiclass_targets.jsonl",
        "relation_reliability_posterior_candidates": output_dir / "relation_reliability_v4_posterior_candidates.jsonl",
        "geometry_support_posterior_candidates": output_dir / "geometry_support_v4_posterior_candidates.jsonl",
        "relation_usefulness_posterior_candidates": output_dir / "relation_usefulness_v4_posterior_candidates.jsonl",
        "excluded_targets": output_dir / "excluded_v4_targets.jsonl",
    ...
    summary = {
        "schema_version": "h002_reliability_target_v4_matched_contrast_label_ingestion_summary_v1",
        "status": status,
        ...
        "input_paths": {
            ...
            "v4_label_schema": rel_path(schema_path),
    ...
    write_jsonl(output_paths["validated_v4_labels"], labels)
    ...
    write_json(
        ...
        {
            "schema_version": "h002_reliability_target_v4_matched_contrast_target_independence_probe_bundle_v1",
            "probes": probes,
        },
    )
