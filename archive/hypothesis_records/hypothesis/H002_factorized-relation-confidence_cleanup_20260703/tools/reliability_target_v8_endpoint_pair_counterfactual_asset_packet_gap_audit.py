#!/usr/bin/env python3
"""Audit v8 endpoint-pair counterfactual partial asset packets before labels."""
...
PACKET_DIR = RGA_ROOT / "reliability_target_v8_endpoint_pair_counterfactual_asset_packets_codex_proxy_user_requested"
DEFAULT_PACKET_SUMMARY = PACKET_DIR / "summary.json"
DEFAULT_FULL_LABEL_SHEET = PACKET_DIR / "v8_endpoint_pair_counterfactual_full_label_sheet.tsv"
DEFAULT_FULL_MANIFEST = PACKET_DIR / "v8_endpoint_pair_counterfactual_full_manifest_post_label_only.jsonl"
DEFAULT_GENERATED_PACKET_MANIFEST = PACKET_DIR / "generated_packet_manifest.jsonl"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v8_endpoint_pair_counterfactual_asset_packet_gap_audit_codex_proxy_user_requested"

SCHEMA_VERSION = "h002_reliability_target_v8_endpoint_pair_counterfactual_asset_packet_gap_audit_v1"
...
VISIBLE_FORBIDDEN_TOKENS = [
    ...
    "undirected_endpoint_pair_key",
    "scene_label_pair_key",
    "structural_pair",
    "endpoint_pattern",
    "v8_group",
    "b2_semantic",
...
def decide_row(packet_row: dict[str, Any], manifest_row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "exact_endpoint_pair_key_hidden": manifest_row.get("exact_endpoint_pair_key_hidden"),
        "undirected_endpoint_pair_key_hidden": manifest_row.get("undirected_endpoint_pair_key_hidden"),
        "scene_label_pair_key_hidden": manifest_row.get("scene_label_pair_key_hidden"),
        "endpoint_pattern_hidden": manifest_row.get("endpoint_pattern_hidden"),
        "v8_group_key_hidden": manifest_row.get("v8_group_key_hidden"),
        "v8_group_level_hidden": manifest_row.get("v8_group_level_hidden"),
...
def validate_inputs(packet_summary: dict[str, Any], label_rows: list[dict[str, Any]], manifests: list[dict[str, Any]]) -> list[dict[str, Any]]:
    errors: list[dict[str, Any]] = []
    if packet_summary.get("next_todo") != "reliability_target_v8_endpoint_pair_counterfactual_asset_packet_gap_audit":
        errors.append({"error": "unexpected_next_todo", "value": packet_summary.get("next_todo")})
    if packet_summary.get("status") != "h002_reliability_target_v8_endpoint_pair_counterfactual_asset_packets_partial_needs_gap_audit":
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# H002 V8 Endpoint-Pair Counterfactual Asset Packet Gap Audit",
        "",
        ...
        "- This decides packet usability before v8 label readiness.",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    replacement_requests = [
        {
            "replacement_request_reason": "v8_endpoint_pair_counterfactual_row_excluded_by_asset_gap_audit",
            "blind_review_id": row.get("blind_review_id"),
            ...
            "undirected_endpoint_pair_key_hidden": row.get("undirected_endpoint_pair_key_hidden"),
            "scene_label_pair_key_hidden": row.get("scene_label_pair_key_hidden"),
            "endpoint_pattern_hidden": row.get("endpoint_pattern_hidden"),
            "v8_group_key_hidden": row.get("v8_group_key_hidden"),
            "v8_group_level_hidden": row.get("v8_group_level_hidden"),
    ...
    status = (
        "h002_reliability_target_v8_endpoint_pair_counterfactual_asset_packet_gap_audit_ready_for_label_readiness"
        if len(ready_label_rows) == len(label_rows) and not output_errors and not leakage_hits and not input_errors
        else "h002_reliability_target_v8_endpoint_pair_counterfactual_asset_packet_gap_audit_needs_replacement"
    )
    next_todo = (
        "reliability_target_v8_endpoint_pair_counterfactual_label_readiness"
        if status == "h002_reliability_target_v8_endpoint_pair_counterfactual_asset_packet_gap_audit_ready_for_label_readiness"
        else "reliability_target_v8_endpoint_pair_counterfactual_replacement_mining"
    )
    decision = (
        "Proceed to label readiness with the full 240-row queue; partial rows are kept with explicit limited-view evidence caveats."
        if status == "h002_reliability_target_v8_endpoint_pair_counterfactual_asset_packet_gap_audit_ready_for_label_readiness"
        else "Do not proceed to label fill: replacement rows are needed to restore the fixed 240-row endpoint-pair counterfactual queue without weak endpoint evidence."
    )
