#!/usr/bin/env python3
"""Ingest informative-anchor H002 reliability target v3 labels."""
...
FILL_DIR = RGA_ROOT / "reliability_target_v3_informative_anchor_label_fill_codex_proxy_user_requested"
SCHEMA_DIR = RGA_ROOT / "reliability_target_v3_informative_anchor_candidate_mining"
ASSET_DIR = RGA_ROOT / "reliability_target_v3_informative_anchor_asset_packets"

DEFAULT_COMPLETED_SHEET = FILL_DIR / "completed_informative_anchor_label_sheet_codex_proxy_user_requested.tsv"
DEFAULT_FILL_SUMMARY = FILL_DIR / "summary.json"
DEFAULT_SCHEMA = SCHEMA_DIR / "v3_label_schema.json"
DEFAULT_MANIFEST = ASSET_DIR / "informative_anchor_full_manifest_post_label_only.jsonl"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v3_informative_anchor_label_ingestion_codex_proxy_user_requested"

LABEL_SOURCE = "codex_proxy_reliability_target_v3_informative_anchor_user_requested"
...
def validate_fill_summary(fill_summary: dict[str, Any]) -> list[dict[str, Any]]:
    errors: list[dict[str, Any]] = []
    expected_status = "h002_reliability_target_v3_informative_anchor_label_filled_codex_proxy_user_requested"
    ...
    expected_next = "reliability_target_v3_informative_anchor_label_ingestion"
    ...
    expected_false = [
        ...
        "used_anchor_category_for_label_decision",
...
def hidden_metadata(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    keys = [
        ...
        "reason_codes_hidden",
        "original_blind_review_id_hidden",
    ]
    output = {
    ...
    if output.get("anchor_category_hidden") is None:
        output["anchor_category_hidden"] = manifest.get("sampling_category_hidden")
    return output
...
def make_label(row: dict[str, str], manifest: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "h002_reliability_target_v3_informative_anchor_ingested_label_v1",
...
def target_row(label: dict[str, Any], target_key: str, schema_version: str) -> dict[str, Any] | None:
    ...
    return {
        ...
        "actual_user_reviewer": False,
        "paper_locked": False,
        "anchor_category_hidden": hidden.get("anchor_category_hidden"),
...
def multiclass_target_row(label: dict[str, Any]) -> dict[str, Any]:
    row = target_row(label, "relation_reliability_v3_multiclass_target", "h002_reliability_target_v3_informative_anchor_multiclass_row_v1")
    ...
    return {
        **row,
        ...
        "audit_note": (
            ...
            "Do not use v3 review fields, anchor/sampling category, packet source, "
            "geometry_status, label_match_status, rank band, endpoint flags, or audit packet paths as model input."
        ),
    }
...
def excluded_target_row(label: dict[str, Any], target_key: str) -> dict[str, Any] | None:
    ...
    if target["target_y"] is not None:
        return None
    return {
        "schema_version": "h002_reliability_target_v3_informative_anchor_excluded_target_v1",
        "target_name": target["target_name"],
        "target_y": None,
        "target_use": target["target_use"],
        "target_reason": target["reason"],
...
def target_independence_probe(target_rows: list[dict[str, Any]], target_name: str) -> dict[str, Any]:
    ...
    if not target_rows:
        return {
            "schema_version": "h002_reliability_target_v3_informative_anchor_independence_probe_v1",
            "target_name": target_name,
...
def count_binary_target(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "by_anchor_category": nested_counts(rows, "anchor_category_hidden"),
        "by_packet_source": nested_counts(rows, "asset_packet_source_hidden"),
        "by_endpoint_flag_pattern": nested_counts(rows, "endpoint_flag_pattern_hidden"),
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# H002 Reliability Target V3 Informative Anchor Label Ingestion",
        "",
...
def build_summary_status(errors: list[dict[str, Any]], probes: dict[str, dict[str, Any]], reliability_count: dict[str, Any]) -> tuple[str, str, str]:
    ...
    if errors:
        return (
            "h002_reliability_target_v3_informative_anchor_label_ingestion_errors",
            "Fix informative-anchor v3 label ingestion errors before target audit.",
            "fix_reliability_target_v3_informative_anchor_label_ingestion_errors",
        )
    if not enough_reliability_mass and any_probe_risk:
        return (
            "h002_reliability_target_v3_informative_anchor_label_ingested_positive_sparse_with_probe_risk",
            (
                "Informative-anchor v3 labels are ingested, but the main relation reliability target is "
                "positive-sparse and shortcut probes flag construction risk. Run target-independence audit before posterior smoke."
            ),
            "reliability_target_v3_informative_anchor_target_independence_audit",
        )
    if not enough_reliability_mass:
        return (
            "h002_reliability_target_v3_informative_anchor_label_ingested_positive_sparse",
            "Informative-anchor v3 labels are ingested, but the main relation reliability target is positive-sparse.",
            "reliability_target_v3_informative_anchor_target_independence_audit",
        )
    if any_probe_risk:
        return (
            ...
            (
                "Informative-anchor v3 labels are ingested and binary target mass is usable, but hidden/visible "
                "shortcut probes still flag target-construction risk. Run target-independence audit before posterior smoke."
            ),
            "reliability_target_v3_informative_anchor_target_independence_audit",
        )
    return (
        "h002_reliability_target_v3_informative_anchor_label_ingested_ready_for_target_independence_audit",
        "Informative-anchor v3 labels are ingested. Run dedicated target-independence audit before posterior smoke.",
        "reliability_target_v3_informative_anchor_target_independence_audit",
    )
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    reliability_targets = [
        row
        for row in (
            target_row(label, "relation_reliability_v3_binary_target", "h002_reliability_target_v3_informative_anchor_binary_row_v1")
            for label in labels
        )
        if row is not None
    ]
    geometry_targets = [
        row
        for row in (
            target_row(label, "geometry_support_v3_binary_target", "h002_geometry_support_v3_informative_anchor_binary_row_v1")
            for label in labels
        )
        if row is not None
    ]
    usefulness_targets = [
        row
        for row in (
            target_row(label, "relation_usefulness_v3_binary_target", "h002_relation_usefulness_v3_informative_anchor_binary_row_v1")
            for label in labels
        )
        if row is not None
    ]
    ...
    reliability_posterior = [
        row
        for row in (
            posterior_candidate_row(label, "relation_reliability_v3_binary_target", "h002_reliability_target_v3_informative_anchor_posterior_candidate_row_v1")
            for label in labels
        )
        if row is not None
    ]
    geometry_posterior = [
        row
        for row in (
            posterior_candidate_row(label, "geometry_support_v3_binary_target", "h002_geometry_support_v3_informative_anchor_posterior_candidate_row_v1")
            for label in labels
        )
        if row is not None
    ]
    usefulness_posterior = [
        row
        for row in (
            posterior_candidate_row(label, "relation_usefulness_v3_binary_target", "h002_relation_usefulness_v3_informative_anchor_posterior_candidate_row_v1")
            for label in labels
        )
        if row is not None
    ]
    ...
    output_paths = {
        ...
        "validated_v3_labels": output_dir / "validated_informative_anchor_v3_labels.jsonl",
    ...
    summary = {
        "schema_version": "h002_reliability_target_v3_informative_anchor_label_ingestion_summary_v1",
        "status": status,
    ...
    write_json(
        ...
        {
            "schema_version": "h002_reliability_target_v3_informative_anchor_target_independence_probe_bundle_v1",
            "probes": probes,
        },
    )
