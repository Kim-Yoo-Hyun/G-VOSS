#!/usr/bin/env python3
"""Materialize H002 v22 hanging-on strict audit packets."""
...
DEFAULT_PLAN_DIR = RGA_ROOT / "reliability_target_v22_hanging_on_strict_conditional_contrast_audit_packet_plan"
DEFAULT_MATCH_ROWS = RGA_ROOT / "match_rows.jsonl"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v22_hanging_on_strict_conditional_contrast_audit_packet_materialization"

EXPECTED_PLAN_STATUS = "h002_reliability_target_v22_hanging_on_strict_conditional_contrast_audit_packet_plan_ready_for_materialization"
EXPECTED_PLAN_NEXT = "reliability_target_v22_hanging_on_strict_conditional_contrast_audit_packet_materialization"

STATUS_READY = "h002_reliability_target_v22_hanging_on_strict_conditional_contrast_audit_packet_materialization_ready_for_leakage_review"
STATUS_ERRORS = "h002_reliability_target_v22_hanging_on_strict_conditional_contrast_audit_packet_materialization_validation_errors"
NEXT_TODO_READY = "reliability_target_v22_hanging_on_strict_conditional_contrast_audit_packet_leakage_review"
...
def validate_inputs(plan_summary: dict[str, Any], visible_rows: list[dict[str, str]], hidden_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if len(visible_rows) != TARGET_ROWS:
        errors.append({"error_type": "unexpected_visible_rows", "expected": TARGET_ROWS, "actual": len(visible_rows)})
    if len(hidden_rows) != TARGET_ROWS:
        errors.append({"error_type": "unexpected_hidden_rows", "expected": TARGET_ROWS, "actual": len(hidden_rows)})
    ...
    if len(set(visible_ids)) != len(visible_ids):
        errors.append({"error_type": "duplicate_visible_packet_id"})
    if len(set(hidden_ids)) != len(hidden_ids):
        errors.append({"error_type": "duplicate_hidden_packet_id"})
...
def materialize_packets(
    ...
    for visible in visible_rows:
        ...
        materialized_manifest.append(
            {
                "schema_version": "h002_reliability_target_v22_hanging_on_audit_packet_materialized_hidden_manifest_v1",
                "packet_id": packet_id,
                ...
                "prediction_id_hidden": gt_fields.get("prediction_id_hidden"),
                "subject_label": visible["subject_label"],
                "predicate_label": visible["predicate_label"],
                "object_label": visible["object_label"],
                "packet_role": visible["packet_role"],
                "evidence_tier": visible["evidence_tier"],
                "planned_proxy_role_hidden": hidden["planned_proxy_role_hidden"],
                "rank_band_hidden": hidden["rank_band_hidden"],
                "geometry_bucket_hidden": hidden["geometry_bucket_hidden"],
                "object_family_pair_hidden": hidden["object_family_pair_hidden"],
                "coverage_proxy_hidden": hidden["coverage_proxy_hidden"],
                "uncertainty_bucket_hidden": hidden["uncertainty_bucket_hidden"],
                "candidate_gt_label_match_status_hidden": hidden["gt_label_match_status_hidden"],
                "strict_group_value_hidden": hidden["strict_group_value_hidden"],
...
def validate_materialized(
    ...
    errors: list[dict[str, Any]] = []
    if len(packet_index) != TARGET_ROWS:
        errors.append({"error_type": "unexpected_packet_index_rows", "expected": TARGET_ROWS, "actual": len(packet_index)})
    if len(materialized_manifest) != TARGET_ROWS:
        errors.append({"error_type": "unexpected_materialized_manifest_rows", "expected": TARGET_ROWS, "actual": len(materialized_manifest)})
    ...
    packet_roles = Counter(row["packet_role"] for row in visible_rows)
    all_tiers = Counter(row["evidence_tier"] for row in visible_rows)
    if packet_roles != Counter({"primary_hanging_on_reliability_candidate": TARGET_ROWS}):
        errors.append({"error_type": "unexpected_packet_roles", "actual": dict(packet_roles)})
    if Counter(row["predicate_label"] for row in visible_rows) != Counter({"hanging on": TARGET_ROWS}):
        errors.append({"error_type": "unexpected_predicate_counts"})
    if all_tiers.get("T1_strong_pair_visual", 0) != 73:
        errors.append({"error_type": "unexpected_t1_rows", "expected": 73, "actual": all_tiers.get("T1_strong_pair_visual", 0)})
    if all_tiers.get("T2_individual_visual_plus_mesh", 0) != 167:
        errors.append({"error_type": "unexpected_t2_rows", "expected": 167, "actual": all_tiers.get("T2_individual_visual_plus_mesh", 0)})
...
def report_text(summary: dict[str, Any]) -> str:
    ...
    return f"""# H002 V22 Hanging-On Strict Audit Packet Materialization
...
instance ids, construction metadata, rank band, geometry bucket, GT match fields, strict-group ids,
and planned proxy roles remain hidden. This stage does not fill labels, train posterior, use
validation/test rows, or promote multi-view as deployable model input.
"""
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    next_todo = NEXT_TODO_READY if not validation_errors else EXPECTED_PLAN_NEXT
    summary = {
        "schema_version": "h002_reliability_target_v22_hanging_on_audit_packet_materialization_summary_v1",
