#!/usr/bin/env python3
"""Audit true-user-review support/vertical target independence."""
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "independent_support_vertical_v2_true_user_review_target_independence_audit_rank_band70_codex_proxy_pending_confirmation"

GEOMETRY_TARGET = "geometry_validity_true_user_review_target"
RELIABILITY_TARGET = "relation_reliability_true_user_review_target"
TARGET_INPUTS = {
    GEOMETRY_TARGET: "geometry_validity_true_user_review_posterior_rows.jsonl",
    RELIABILITY_TARGET: "relation_reliability_true_user_review_posterior_rows.jsonl",
}

LABEL_SOURCE = "codex_proxy_true_user_review_pending_confirmation"

SLICE_SPECS = {
    "original_true_user_review": {
        "balanced_keys": [],
        "reason": "full Codex-proxy pending-confirmation true-user-review binary target",
        "priority": 99,
    },
    "prior_relation_validity_balanced_true_user_review": {
    ...
    "queue_family_balanced_true_user_review": {
    ...
    "role_family_balanced_true_user_review": {
    ...
    "prior_label_rank_balanced_true_user_review": {
...
def validate_rows(target_name: str, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if row.get("filled_by") != "codex_proxy":
            errors.append({"target_name": target_name, "error_type": "unexpected_filled_by", "row_number": index, "blind_review_id": blind_id, "filled_by": row.get("filled_by")})
        if row.get("actual_true_user_reviewer") is not False:
            errors.append(
                {
                    "target_name": target_name,
                    "error_type": "unexpected_actual_true_user_reviewer_flag",
                    "row_number": index,
                    "blind_review_id": blind_id,
                    "actual_true_user_reviewer": row.get("actual_true_user_reviewer"),
                }
            )
        if row.get("workflow_treat_as_user_review_for_next_hypothesis_steps") is not True:
            errors.append({"target_name": target_name, "error_type": "missing_workflow_user_review_flag", "row_number": index, "blind_review_id": blind_id})
        if row.get("user_confirmation_pending") is not True:
            errors.append({"target_name": target_name, "error_type": "missing_user_confirmation_pending_flag", "row_number": index, "blind_review_id": blind_id})
        ...
        if "audit_only_true_user_review_fields" not in row:
            errors.append({"target_name": target_name, "error_type": "missing_audit_only_true_user_review_fields", "row_number": index, "blind_review_id": blind_id})
        ...
        forbidden = evidence.get("forbidden_as_posterior_input", {})
        expected_forbidden = [
...
def per_target_decision(target_name: str, summaries: list[dict[str, Any]], errors: list[dict[str, Any]]) -> dict[str, Any]:
    original = next(item for item in summaries if item["slice_name"] == "original_true_user_review")
    ...
    if errors:
        ...
        decision = "Fix row validation errors before using any true-user-review target slice."
        next_step = "fix_true_user_review_target_independence_audit_errors"
    elif strict:
        status = "strict_controlled_slice_ready"
        decision = (
            "A strict true-user-review controlled slice clears harmful prior-label carryover and "
            "construction-risk checks. Source-score feature join is still required before posterior smoke."
        )
        next_step = "true_user_review_source_feature_join_then_controlled_posterior_smoke"
    elif construction:
        ...
        next_step = "revise_true_user_review_target_or_collect_real_user_labels"
    else:
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# H002 True User Review Target Independence Audit",
        "",
        ...
        "- They are not paper-level true user/external annotation before user confirmation.",
        "- Hidden metadata is used only after label lock for audit and controlled-slice construction.",
        "- True-user review fields, hidden strata, previous proxy labels, and audit packet paths are not posterior inputs.",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if all_validation_errors:
        status = "full_train_independent_support_vertical_v2_true_user_review_target_independence_audit_errors"
        decision = "Fix validation errors before any true-user-review target slice can be used."
        next_todo = "fix_true_user_review_rank_band70_target_independence_audit_errors"
    elif relation_strict:
        status = "full_train_independent_support_vertical_v2_true_user_review_target_independence_audit_relation_strict_slice_ready"
        decision = (
            "A strict relation-reliability slice exists under the Codex-proxy pending-confirmation true-user-review target. "
            "Before posterior smoke, join source rank/score and p_geom_valid as deployable features."
        )
        next_todo = "true_user_review_rank_band70_source_feature_join"
        ...
        decision = (
            "No strict relation-reliability slice clears harmful prior-label carryover. Construction-only diagnostic slices "
            "may be used for plumbing/error analysis, but posterior method validation remains blocked."
        )
        next_todo = "revise_true_user_review_target_or_collect_real_user_labels"
    else:
        status = "full_train_independent_support_vertical_v2_true_user_review_target_independence_audit_blocked"
        decision = "No size-ready strict or construction-only controlled slice exists for true-user-review relation reliability."
        next_todo = "revise_true_user_review_target_or_collect_real_user_labels"
    ...
    summary = {
        "schema_version": "h002_support_vertical_v2_true_user_review_target_independence_audit_summary_v1",
        "status": status,
        ...
        "boundary": {
            ...
            "true_user_review_fields_used_for_target_or_audit_only": True,
            "previous_proxy_labels_used_for_audit_only": True,
