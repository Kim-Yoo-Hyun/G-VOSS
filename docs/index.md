...
- `attachment_deferred` is now the preferred future relation-family upgrade if H001 is expanded beyond the current AAAI claim. It is not current metric evidence. Docker G0 scope/schema audit and G1 extractor contract are complete with status `attachment_deferred_extractor_contract_ready_no_extraction`: 967 GT rows (`attached to` 808, `hanging on` 126, `connected to` 33), candidate denominator 3,512 if validated, VL-SAT 77,748 candidate prediction rows, Open3DSG 57,300 candidate prediction rows, and existing verification status `unsupported` for both sources. This path is more H001-aligned than `relative_horizontal` because attachment/hanging/connection have physical consistency constraints. The frozen extractor contract requires identity, OBB, local point contact, surface candidate/normal, gravity/hanging, contradictory support, and affordance-as-context fields while forbidding `verification_status`, `p_geom_valid`, recall credit, and reranking scores. Owner: `experiments/H001_geom_reliability/sources/attachment_deferred/README.md`; contract outputs: `experiments/H001_geom_reliability/sources/attachment_deferred/evidence_extractor/`.
...
4. `attachment_deferred`를 current H001 evidence level까지 확장하려면 frozen G1 contract를 만족하는 G1b evidence-only dry run을 어떻게 구현하고 검증할 것인가?
