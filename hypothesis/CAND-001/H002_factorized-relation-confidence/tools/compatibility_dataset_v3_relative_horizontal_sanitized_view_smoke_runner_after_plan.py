#!/usr/bin/env python3
"""Run train-only grouped-CV smoke for relative-horizontal compatibility."""
...
DEFAULT_PLAN_DIR = H2_ROOT / "artifacts/compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_plan_after_schema_audit"
DEFAULT_OUTPUT_DIR = H2_ROOT / "artifacts/compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_runner_after_plan"

EXPECTED_PLAN_STATUS = "h002_compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_plan_after_schema_audit_ready"
EXPECTED_PLAN_NEXT = "compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_runner_after_plan"
EXPECTED_ROW_SCHEMA = "h002_relative_horizontal_runner_ready_view_v1"
EXPECTED_BLOCKS = {"G_e_horizontal", "T_e"}

SCHEMA_VERSION = "h002_compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_runner_after_plan_v1"
STATUS_ERRORS = "h002_compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_runner_after_plan_input_errors"
STATUS_PASSED = "h002_compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_runner_after_plan_passed_controls"
STATUS_CONCAT_CAVEAT = "h002_compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_runner_after_plan_passed_but_concat_solves"
STATUS_DIAGNOSTIC = "h002_compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_runner_after_plan_diagnostic_only_failed_controls"

NEXT_TODO_PASSED = "compatibility_dataset_v3_relative_horizontal_smoke_result_review_after_runner"
NEXT_TODO_DIAGNOSTIC = "compatibility_dataset_v3_relative_horizontal_smoke_failure_analysis_after_runner"
...
STRONG_INVERSION_AUROC_MAX = 0.10
...
def normalize_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        normalized.append(
            {
                "G_e_horizontal": block(row, "G_e_horizontal"),
                "T_e": block(row, "T_e"),
...
def predicate_sign(predicate: Any) -> float:
    if predicate in {"right", "behind"}:
        return 1.0
    if predicate in {"left", "front"}:
        return -1.0
    return 0.0


def selected_projection(t: dict[str, Any], g: dict[str, Any]) -> float:
    predicate = t.get("predicate_label")
    sign = predicate_sign(predicate)
    axis = predicate_axis(predicate)
    if axis == "dx":
        value = safe_float(g.get("delta_x_subject_minus_object"), 0.0)
    elif axis == "dy":
        value = safe_float(g.get("delta_y_subject_minus_object"), 0.0)
    else:
        value = 0.0
    return sign * value
...
def compatibility_interactions_from_dict(t: dict[str, Any], g: dict[str, Any]) -> dict[str, float]:
    predicate = t.get("predicate_label")
    dx = safe_float(g.get("delta_x_subject_minus_object"), 0.0)
    dy = safe_float(g.get("delta_y_subject_minus_object"), 0.0)
    dist = safe_float(g.get("horizontal_distance"), math.sqrt(dx * dx + dy * dy))
    signed = selected_projection(t, g)
    out: dict[str, float] = {
        "C.expected_axis_is_dx": 1.0 if predicate_axis(predicate) == "dx" else 0.0,
        "C.expected_axis_is_dy": 1.0 if predicate_axis(predicate) == "dy" else 0.0,
        "C.expected_axis_sign": predicate_sign(predicate),
        "C.signed_projection": signed,
        "C.normalized_signed_projection": signed / max(dist, 1e-6),
        "C.orthogonal_projection": dy if predicate_axis(predicate) == "dx" else dx,
    }
    out.update(one_hot("C.axis", predicate_axis(predicate)))
    return out


def compatibility_interaction_features(row: dict[str, Any]) -> dict[str, float]:
    return compatibility_interactions_from_dict(row.get("T_e", {}), row.get("G_e_horizontal", {}))
...
def selected_axis_sign_flipped_g(row: dict[str, Any]) -> dict[str, Any]:
    predicate = row.get("T_e", {}).get("predicate_label")
    g = dict(row.get("G_e_horizontal", {}))
    if predicate_axis(predicate) == "dx":
        g["delta_x_subject_minus_object"] = -safe_float(g.get("delta_x_subject_minus_object"), 0.0)
    elif predicate_axis(predicate) == "dy":
        g["delta_y_subject_minus_object"] = -safe_float(g.get("delta_y_subject_minus_object"), 0.0)
    dx = safe_float(g.get("delta_x_subject_minus_object"), 0.0)
    dy = safe_float(g.get("delta_y_subject_minus_object"), 0.0)
    g["horizontal_distance"] = math.sqrt(dx * dx + dy * dy)
    return g


def axis_sign_flipped_g_features(row: dict[str, Any]) -> dict[str, float]:
    t = row.get("T_e", {})
    g = selected_axis_sign_flipped_g(row)
    return merge_features(t_features(row), g_features_from_dict(g), compatibility_interactions_from_dict(t, g))


def wrong_frame_xy_swapped_g(row: dict[str, Any]) -> dict[str, Any]:
    g = dict(row.get("G_e_horizontal", {}))
    dx = safe_float(g.get("delta_x_subject_minus_object"), 0.0)
    dy = safe_float(g.get("delta_y_subject_minus_object"), 0.0)
    g["delta_x_subject_minus_object"] = dy
    g["delta_y_subject_minus_object"] = dx
    g["horizontal_distance"] = math.sqrt(dx * dx + dy * dy)
    return g


def wrong_frame_xy_swap_features(row: dict[str, Any]) -> dict[str, float]:
    t = row.get("T_e", {})
    g = wrong_frame_xy_swapped_g(row)
    return merge_features(t_features(row), g_features_from_dict(g), compatibility_interactions_from_dict(t, g))


def subject_object_swapped_g(row: dict[str, Any]) -> dict[str, Any]:
    g = dict(row.get("G_e_horizontal", {}))
    dx = -safe_float(g.get("delta_x_subject_minus_object"), 0.0)
    dy = -safe_float(g.get("delta_y_subject_minus_object"), 0.0)
    g["delta_x_subject_minus_object"] = dx
    g["delta_y_subject_minus_object"] = dy
    g["horizontal_distance"] = math.sqrt(dx * dx + dy * dy)
    return g
...
def subject_object_swap_features(row: dict[str, Any]) -> dict[str, float]:
    t = row.get("T_e", {})
    g = subject_object_swapped_g(row)
    return merge_features(t_features(row), g_features_from_dict(g), compatibility_interactions_from_dict(t, g))
...
def shifted_geometry_map(rows: list[dict[str, Any]], group_fn: Callable[[dict[str, Any]], str] | None = None) -> dict[str, dict[str, Any]]:
    ...
    for group_rows in groups.values():
        ...
        if len(ordered) <= 1:
            for row in ordered:
                out[str(row["row_id"])] = dict(row.get("G_e_horizontal", {}))
            continue
        ...
        for idx, row in enumerate(ordered):
            ...
            out[str(row["row_id"])] = dict(donor.get("G_e_horizontal", {}))
    return out
...
def model_feature_fns(rows: list[dict[str, Any]]) -> dict[str, tuple[FeatureFn, FeatureFn]]:
    ...
    return {
        ...
        "M2_geometry_only_G_horizontal": (g_features, g_features),
        ...
        "C4_axis_sign_flipped_G": (primary, axis_sign_flipped_g_features),
        "C5_wrong_frame_xy_swap": (primary, wrong_frame_xy_swap_features),
        "C6_subject_object_swap": (primary, subject_object_swap_features),
    }
...
def validate(
    ...
    valid_pairs = {("behind", "front"), ("left", "right")}
    for group_id, group_rows in by_group.items():
        ...
        predicates = tuple(sorted(str(row.get("T_e", {}).get("predicate_label")) for row in group_rows))
        if predicates not in valid_pairs:
            errors.append({"error_type": "group_predicates_not_opposites", "group_id": group_id, "predicates": predicates})
        g_values = {json.dumps(row.get("G_e_horizontal", {}), sort_keys=True) for row in group_rows}
        if len(g_values) != 1:
...
def gate_summary(metrics: dict[str, Any], margins: dict[str, Any], errors: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    m2 = value_or_zero(metrics, "M2_geometry_only_G_horizontal")
    ...
    c4 = value_or_zero(metrics, "C4_axis_sign_flipped_G")
    c5 = value_or_zero(metrics, "C5_wrong_frame_xy_swap")
    c6 = value_or_zero(metrics, "C6_subject_object_swap")
    ...
    gate_wrong_t = c1 <= CONTROL_AUROC_MAX or c1 <= STRONG_INVERSION_AUROC_MAX
    gate_shuffle = best_shuffle <= CONTROL_AUROC_MAX
    gate_axis_flip = c4 <= CONTROL_AUROC_MAX or c4 <= STRONG_INVERSION_AUROC_MAX
    gate_wrong_frame = c5 <= CONTROL_AUROC_MAX
    gate_subject_object = c6 <= CONTROL_AUROC_MAX or c6 <= STRONG_INVERSION_AUROC_MAX
    ...
    overall = (
        gate_data
        and gate_single_factor
        and gate_primary
        and gate_gain
        and gate_wrong_t
        and gate_shuffle
        and gate_axis_flip
        and gate_wrong_frame
        and gate_subject_object
        and gate_margin
    )
    ...
    return {
        ...
        "gate_single_factor_shortcuts_near_chance": {
            ...
            "auroc": {
                ...
                "M2_geometry_only_G_horizontal": m2,
        ...
        "gate_plain_concat_boundary": {
            ...
            "interpretation": "If true, explicit signed interaction is less necessary for this family than expected.",
        },
        ...
        "gate_axis_sign_flipped_G_control": {"pass": gate_axis_flip, "C4_axis_sign_flipped_G": c4, "max_allowed": CONTROL_AUROC_MAX},
        "gate_wrong_frame_xy_swap": {"pass": gate_wrong_frame, "C5_wrong_frame_xy_swap": c5, "max_allowed": CONTROL_AUROC_MAX},
        "gate_subject_object_swap": {"pass": gate_subject_object, "C6_subject_object_swap": c6, "max_allowed": CONTROL_AUROC_MAX},
        ...
        "overall_interpretation": (
            "relative_horizontal_smoke_passed_but_concat_solves"
            if overall and concat_caveat
            else "relative_horizontal_smoke_passed_controls"
            if overall
            else "relative_horizontal_smoke_diagnostic_only_failed_controls"
        ),
    }
...
def prediction_rows(rows: list[dict[str, Any]], predictions: dict[str, list[float]]) -> list[dict[str, Any]]:
    ...
    for idx, row in enumerate(rows):
        ...
        item = {
            ...
            "predicate": row.get("T_e", {}).get("predicate_label"),
            "delta_x_subject_minus_object": g.get("delta_x_subject_minus_object"),
            "delta_y_subject_minus_object": g.get("delta_y_subject_minus_object"),
            "horizontal_distance": g.get("horizontal_distance"),
        }
        ...
        cases.append(
            {
                ...
                "prediction": pred,
                "score": round(score, 6),
                "G_e_horizontal": row.get("G_e_horizontal", {}),
            }
        )
...
def write_report(path: Path, summary: dict[str, Any], metrics: dict[str, Any], gates: dict[str, Any]) -> None:
    ...
    lines = [
        "# H002 Relative-Horizontal Sanitized View Smoke Runner",
        "",
        "## Status",
        "",
        "```text",
    ...
    lines.extend(
        [
            ...
            f"- axis sign flip control: `{gates['gate_axis_sign_flipped_G_control']['pass']}`",
            f"- wrong-frame xy swap: `{gates['gate_wrong_frame_xy_swap']['pass']}`",
            f"- subject/object swap: `{gates['gate_subject_object_swap']['pass']}`",
            ...
            "supports the `relative_horizontal` predicate-geometry compatibility route under",
            "the frozen reference-frame assumption. It is not paper evidence until reproduced",
            "under the experiment workflow.",
            "",
        ]
    )
