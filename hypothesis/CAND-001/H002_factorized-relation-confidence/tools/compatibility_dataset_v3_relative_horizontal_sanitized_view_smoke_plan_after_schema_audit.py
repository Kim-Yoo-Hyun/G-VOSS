#!/usr/bin/env python3
"""Write the relative-horizontal train-only sanitized-view smoke plan."""
...
DEFAULT_AUDIT_DIR = (
    H2_ROOT / "artifacts/compatibility_dataset_v3_relative_horizontal_schema_shortcut_audit_after_materialization"
)
DEFAULT_OUTPUT_DIR = (
    H2_ROOT / "artifacts/compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_plan_after_schema_audit"
)

EXPECTED_AUDIT_STATUS = (
    "h002_compatibility_dataset_v3_relative_horizontal_schema_shortcut_audit_after_materialization_ready_for_smoke_plan"
)
EXPECTED_AUDIT_NEXT = "compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_plan_after_schema_audit"
EXPECTED_INPUT_SCHEMA = "h002_relative_horizontal_smoke_ready_view_v1"

SCHEMA_VERSION = "h002_compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_plan_after_schema_audit_v1"
SMOKE_READY_SCHEMA = "h002_relative_horizontal_runner_ready_view_v1"
STATUS_READY = "h002_compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_plan_after_schema_audit_ready"
STATUS_ERROR = "h002_compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_plan_after_schema_audit_input_errors"
...
EXPECTED_PREDICATES = {"behind": 600, "front": 600, "left": 600, "right": 600}
EXPECTED_FEATURE_BLOCKS = {"G_e_horizontal", "T_e"}
        ...
        normalized.append(
            {
                ...
                "row_id": row_id,
                "cv_group_id": str(row.get("cv_group") or row.get("group_id")),
                "split": "train",
                "target_name": "C_e",
...
def count_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    group_sizes = Counter(len(values) for values in by_group.values())
    return {
        "rows": len(rows),
        "positive": labels[1],
        "negative": labels[0],
...
def validate_inputs(
    ...
    for key in ["validation_usage", "test_usage", "h001_artifacts_modified", "runs_new_learned_smoke", "trains_new_model"]:
        if boundary.get(key) is not False:
            errors.append({"error_type": "audit_boundary_not_false", "key": key, "actual": boundary.get(key)})
    for name in ["validation_errors.jsonl", "feature_path_violations.jsonl", "group_integrity_errors.jsonl"]:
        path = audit_dir / name
        if path.exists() and path.read_text(encoding="utf-8").strip():
            errors.append({"error_type": f"nonempty_audit_{name}", "path": rel_path(path)})
    ...
    for row in smoke_rows:
        ...
        for field in [
            "feature_blocks.G_e_horizontal.delta_x_subject_minus_object",
            "feature_blocks.G_e_horizontal.delta_y_subject_minus_object",
            "feature_blocks.G_e_horizontal.horizontal_distance",
        ]:
...
def input_profile_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for path in paths:
        ...
        finite = sum(1 for value in values if finite_number(value))
        out.append(
            {
                "feature_path": path,
                "rows": len(rows),
...
def model_view_rows() -> list[dict[str, Any]]:
    return [
        ...
        {"model": "M2_geometry_only_G_horizontal", "input_blocks": "G_e_horizontal", "primary": False, "role": "predicate-independent continuous horizontal geometry baseline; should be near chance"},
        {"model": "M3_TG_concat_no_interaction", "input_blocks": "T_e + G_e_horizontal", "primary": False, "role": "plain concatenation without explicit signed predicate-geometry products"},
        {"model": "M4_TG_horizontal_interaction", "input_blocks": "T_e + G_e_horizontal", "primary": True, "role": "primary predicate-horizontal-geometry compatibility smoke"},
        ...
        {"model": "S2_geometry_exact_tuple_shortcut", "input_blocks": "G_e_horizontal exact tuple", "primary": False, "role": "same-G paired construction should make exact geometry tuple uninformative"},
        {"model": "C1_wrong_T_same_G", "input_blocks": "wrong T_e + same G_e_horizontal", "primary": False, "role": "predicate-conditioning negative control"},
        {"model": "C2_shuffled_G_global", "input_blocks": "T_e + globally shuffled G_e_horizontal", "primary": False, "role": "geometry-alignment negative control"},
        {"model": "C3_shuffled_G_within_predicate", "input_blocks": "T_e + within-predicate shuffled G_e_horizontal", "primary": False, "role": "harder geometry-alignment control"},
        {"model": "C4_axis_sign_flipped_G", "input_blocks": "T_e + sign-flipped selected G_e_horizontal", "primary": False, "role": "directional geometry inversion control"},
        {"model": "C5_wrong_frame_xy_swap", "input_blocks": "T_e + x/y swapped G_e_horizontal", "primary": False, "role": "wrong-frame control for horizontal relations"},
        {"model": "C6_subject_object_swap", "input_blocks": "T_e + endpoint-swapped G_e_horizontal", "primary": False, "role": "directional relation endpoint-swap control"},
    ]
...
def gate_rows() -> list[dict[str, Any]]:
    return [
        ...
        {"gate": "plain_concat_boundary", "criterion": "M3 is diagnostic; if M3 solves the task, report that explicit signed interaction may be unnecessary for this family", "blocks_paper_claim_if_fail": True},
        ...
        {"gate": "axis_sign_flip_control", "criterion": "C4 sign-flipped selected axis should invert or strongly degrade compatibility scores", "blocks_promotion_if_fail": True},
        {"gate": "wrong_frame_control", "criterion": f"C5 x/y swapped frame should degrade to <= {CONTROL_AUROC_MAX:.2f} unless data shows frame ambiguity", "blocks_promotion_if_fail": True},
        {"gate": "subject_object_swap_control", "criterion": "C6 endpoint swap should invert or strongly degrade directional compatibility scores", "blocks_promotion_if_fail": True},
...
def control_rows() -> list[dict[str, Any]]:
    return [
        {"control": "wrong_T_same_G", "construction": "swap left<->right and front<->behind in T_e while keeping G_e_horizontal and target", "expected_result": "predicate-conditioned compatibility should degrade or invert"},
        {"control": "shuffled_G_global", "construction": "permute G_e_horizontal across all rows with a deterministic seed", "expected_result": "breaks aligned object-pair horizontal evidence"},
        {"control": "shuffled_G_within_predicate", "construction": "permute G_e_horizontal within each predicate label", "expected_result": "preserves predicate distribution while breaking paired geometry"},
        {"control": "axis_sign_flipped_G", "construction": "flip the signed axis relevant to the predicate family: dx for left/right, dy for front/behind", "expected_result": "inverts left/right or front/behind compatibility"},
        {"control": "wrong_frame_xy_swap", "construction": "swap dx and dy before scoring", "expected_result": "tests whether the model depends on the frozen reference-frame route"},
        {"control": "subject_object_swap", "construction": "multiply both dx and dy by -1 to mimic endpoint swap", "expected_result": "directional relation compatibility should invert or degrade"},
        {"control": "no_interaction_concat", "construction": "compare M3 plain concat with M4 signed predicate-conditioned interaction", "expected_result": "tests whether explicit compatibility products are necessary"},
    ]
...
def smoke_plan(input_path: Path) -> dict[str, Any]:
    return {
        ...
        "task": {
            "name": "relative-horizontal predicate-geometry compatibility C_e smoke",
            "target": "target_y",
            "target_semantics": "1 if left/right/front/behind predicate is compatible with signed horizontal displacement under the frozen frame",
            "not_in_scope": ["Z_e posterior", "p_rel final reliability", "p_obs abstention", "validation/test performance", "paper-level claim", "`in front of` alias"],
        },
        "input_contract": {
        ...
        "split_policy": {
            ...
            "folds": 5,
            "group_key": "cv_group_id",
            "group_rule": "the compatible and incompatible row with identical G_e_horizontal must remain in the same fold",
            "validation_usage": False,
            "test_usage": False,
        },
        "feature_engineering": {
            "T_e": "predicate label/text and relation family only",
            "G_e_horizontal": "predicate-independent continuous dx, dy, and horizontal distance",
            "primary_interaction": "predicate-specific signed projection: left=-dx, right=+dx, front=-dy, behind=+dy",
            "expected_axis_sign": {"left": {"axis": "dx", "sign": -1}, "right": {"axis": "dx", "sign": 1}, "front": {"axis": "dy", "sign": -1}, "behind": {"axis": "dy", "sign": 1}},
            "Z_e_policy": "excluded from this C_e smoke; source confidence remains outside compatibility",
            "Q_e_policy": "axis-boundary and frame-disagreement rows are diagnostic and outside this primary smoke",
            "in_front_of_policy": "not observed in current train/full source and excluded",
        },
        "models": model_view_rows(),
        "gates": gate_rows(),
        "controls": control_rows(),
        "metrics": ["AUROC", "AUPRC", "accuracy", "balanced_accuracy", "Brier", "ECE", "fold mean/std", "paired margin pass rate", "axis-pair slice AUROC"],
        "paper_boundary": {
...
def render_report(summary: dict[str, Any]) -> str:
    ...
    return f"""# H002 Relative-Horizontal Sanitized View Smoke Plan After Schema Audit
...
- `M4_TG_horizontal_interaction` as the primary compatibility smoke
...
and the comparison contract for `left/right/front/behind`. The target is not
whether horizontal geometry is useful by itself. The target is whether the same
predicate-independent `G_e_horizontal` evidence changes meaning under different
`T_e` predicates and under a frozen reference-frame route.
"""
...
def main() -> int:
    ...
    next_todo = NEXT_TODO if not validation_errors else "compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_plan_repair"
    ...
    summary = {
        ...
        "primary_model": "M4_TG_horizontal_interaction",
    ...
    input_manifest = {
        "schema_version": "h002_relative_horizontal_smoke_input_manifest_v1",
        "input_sha256": sha256_file(input_path),
