#!/usr/bin/env python3
"""Formal leakage review for H002 v22 hanging-on strict audit packets."""
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v22_hanging_on_strict_conditional_contrast_audit_packet_leakage_review"

EXPECTED_MATERIALIZATION_STATUS = "h002_reliability_target_v22_hanging_on_strict_conditional_contrast_audit_packet_materialization_ready_for_leakage_review"
EXPECTED_MATERIALIZATION_NEXT = "reliability_target_v22_hanging_on_strict_conditional_contrast_audit_packet_leakage_review"

STATUS_READY = "h002_reliability_target_v22_hanging_on_strict_conditional_contrast_audit_packet_leakage_review_passed_ready_for_label_fill"
STATUS_BLOCKED = "h002_reliability_target_v22_hanging_on_strict_conditional_contrast_audit_packet_leakage_review_blocked"
NEXT_TODO_READY = "reliability_target_v22_hanging_on_strict_conditional_contrast_audit_packet_label_fill"
NEXT_TODO_BLOCKED = "repair_v22_hanging_on_strict_audit_packet_visible_leakage"
...
PACKET_ID_RE = re.compile(r"^apv22_[0-9]{4}_v22hang_[0-9a-f]{14}$")
...
def validate_materialization_summary(summary: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if summary.get("status") != EXPECTED_MATERIALIZATION_STATUS:
        errors.append({"error_type": "unexpected_materialization_status", "expected": EXPECTED_MATERIALIZATION_STATUS, "actual": summary.get("status")})
    if summary.get("next_todo") != EXPECTED_MATERIALIZATION_NEXT:
        errors.append({"error_type": "unexpected_materialization_next", "expected": EXPECTED_MATERIALIZATION_NEXT, "actual": summary.get("next_todo")})
    if summary.get("validation_errors") != 0:
        errors.append({"error_type": "materialization_validation_errors_present", "actual": summary.get("validation_errors")})
    counts = summary.get("counts", {})
    expected_counts = {
        "visible_review_rows": TARGET_ROWS,
        "packet_dirs": TARGET_ROWS,
        "materialized_hidden_manifest_rows": TARGET_ROWS,
        "visible_leakage_hits": 0,
        "gt_match_axis_joined_rows": TARGET_ROWS,
    }
...
def validate_visible_sheet(header: list[str], rows: list[dict[str, str]], source: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    if len(rows) != TARGET_ROWS:
        errors.append({"error_type": "unexpected_visible_sheet_rows", "expected": TARGET_ROWS, "actual": len(rows)})
    packet_ids = []
    ...
    duplicate_ids = [pid for pid, count in Counter(packet_ids).items() if count > 1]
    if duplicate_ids:
        errors.append({"error_type": "duplicate_visible_packet_id", "duplicates": duplicate_ids[:20]})
    return errors, hits
    ...
    for row in packet_index:
        ...
        for image_path in image_files:
            ...
            if not IMAGE_NAME_RE.match(image_path.name):
                hits.append({"source": rel_path(image_path), "hit_type": "non_neutral_image_filename", "filename": image_path.name})
...
def validate_hidden_manifest(hidden_rows: list[dict[str, Any]], visible_packet_ids: set[str]) -> tuple[list[dict[str, Any]], dict[str, int]]:
    ...
    if len(hidden_rows) != TARGET_ROWS:
        errors.append({"error_type": "unexpected_hidden_manifest_rows", "expected": TARGET_ROWS, "actual": len(hidden_rows)})
    ...
    if counts["hidden_rows_with_source_paths"] != TARGET_ROWS:
        errors.append({"error_type": "hidden_manifest_source_paths_incomplete", "expected": TARGET_ROWS, "actual": counts["hidden_rows_with_source_paths"]})
    if counts["hidden_rows_with_scan_ids"] != TARGET_ROWS:
        errors.append({"error_type": "hidden_manifest_scan_ids_incomplete", "expected": TARGET_ROWS, "actual": counts["hidden_rows_with_scan_ids"]})
    if counts["hidden_rows_with_gt_match_axis"] != TARGET_ROWS:
        errors.append({"error_type": "hidden_manifest_gt_match_axis_incomplete", "expected": TARGET_ROWS, "actual": counts["hidden_rows_with_gt_match_axis"]})
    return errors, dict(counts)


def report_text(summary: dict[str, Any]) -> str:
    counts = summary["counts"]
    return f"""# H002 V22 Hanging-On Strict Audit Packet Leakage Review
...
rank bands, geometry buckets, planned proxy roles, strict-group ids, or GT-match fields.

The hidden manifest retains source paths, scan/instance identifiers, construction metadata, and
existing GT relation match fields for provenance and future mismatch analysis only.
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    materialization_summary = read_json(materialization_dir / "summary.json")
    ...
    validation_errors = validate_materialization_summary(materialization_summary)
    ...
    status = STATUS_READY if counts["formal_leakage_review_pass"] else STATUS_BLOCKED
    next_todo = NEXT_TODO_READY if counts["formal_leakage_review_pass"] else NEXT_TODO_BLOCKED
    ...
    output_summary = {
        "schema_version": "h002_reliability_target_v22_hanging_on_audit_packet_leakage_review_summary_v1",
