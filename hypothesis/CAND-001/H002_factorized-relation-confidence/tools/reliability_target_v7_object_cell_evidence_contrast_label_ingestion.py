#!/usr/bin/env python3
"""Ingest v7 object-cell evidence-contrast labels after proxy fill."""
...
FILL_DIR = RGA_ROOT / "reliability_target_v7_object_cell_evidence_contrast_label_fill_codex_proxy_user_requested"
READINESS_DIR = RGA_ROOT / "reliability_target_v7_object_cell_evidence_contrast_label_readiness_codex_proxy_user_requested"

DEFAULT_COMPLETED_SHEET = FILL_DIR / "completed_v7_object_cell_evidence_contrast_label_sheet_codex_proxy_user_requested.tsv"
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v7_object_cell_evidence_contrast_label_ingestion_codex_proxy_user_requested"

LABEL_SOURCE = "codex_proxy_reliability_target_v7_object_cell_evidence_contrast_user_requested"
...
def validate_fill_summary(fill_summary: dict[str, Any]) -> list[dict[str, Any]]:
    errors: list[dict[str, Any]] = []
    expected_status = "h002_reliability_target_v7_object_cell_evidence_contrast_label_filled_codex_proxy_user_requested"
    ...
    expected_next = "reliability_target_v7_object_cell_evidence_contrast_label_ingestion"
...
def hidden_metadata(manifest: dict[str, Any]) -> dict[str, Any]:
    keys = [
        ...
        "source_queue_hidden",
        "strict_group_b2_count_hidden",
        "strict_group_b3_count_hidden",
        "strict_group_capacity_after_cap_hidden",
        "strict_group_key_hidden",
        "structural_pair_hidden",
        "subject_object_family_cell_hidden",
        "subject_object_label_pair_hidden",
...
def make_label(row: dict[str, str], manifest: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "h002_reliability_target_v7_object_cell_evidence_contrast_ingested_label_v1",
    ...
    return {
        **row,
        ...
        "audit_note": (
            ...
            "label_match_status, object-cell metadata, rank band, endpoint pattern, packet source, "
...
def excluded_target_row(label: dict[str, Any], target_key: str) -> dict[str, Any] | None:
    ...
    if target["target_y"] is not None:
        return None
    return {
        "schema_version": "h002_reliability_target_v7_object_cell_evidence_contrast_excluded_target_v1",
        "target_name": target["target_name"],
        "target_y": None,
        "target_use": target["target_use"],
        "target_reason": target["reason"],
...
def target_independence_probe(target_rows: list[dict[str, Any]], target_name: str) -> dict[str, Any]:
    ...
    if not target_rows:
        return {
            "schema_version": "h002_reliability_target_v7_object_cell_evidence_contrast_independence_probe_v1",
            "target_name": target_name,
...
def build_summary_status(
    ...
    if errors:
        return (
            "h002_reliability_target_v7_object_cell_evidence_contrast_label_ingestion_errors",
            "Fix v7 object-cell evidence-contrast label ingestion errors before target audit.",
            "fix_reliability_target_v7_object_cell_evidence_contrast_label_ingestion_errors",
        )
    if not enough_multiclass_mass:
        return (
            "h002_reliability_target_v7_object_cell_evidence_contrast_label_ingested_multiclass_sparse",
            "V7 labels are ingested, but at least one multiclass reliability state is sparse.",
            "reliability_target_v7_object_cell_evidence_contrast_target_independence_audit",
        )
    if not enough_binary_mass:
        return (
            "h002_reliability_target_v7_object_cell_evidence_contrast_label_ingested_binary_sparse",
            "V7 labels are ingested, but binary reliable/unreliable diagnostic mass is sparse.",
            "reliability_target_v7_object_cell_evidence_contrast_target_independence_audit",
        )
    if any_probe_risk:
        return (
            "h002_reliability_target_v7_object_cell_evidence_contrast_label_ingested_with_probe_risk",
            "V7 labels are ingested, but hidden/visible shortcut probes flag target-construction risk. Run target-independence audit before any posterior smoke.",
            "reliability_target_v7_object_cell_evidence_contrast_target_independence_audit",
        )
    return (
        "h002_reliability_target_v7_object_cell_evidence_contrast_label_ingested_ready_for_target_independence_audit",
        "V7 labels are ingested. Run dedicated target-independence audit before posterior smoke.",
        "reliability_target_v7_object_cell_evidence_contrast_target_independence_audit",
    )
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    multiclass_targets = [
        target_row(label, "relation_reliability_state_v6_multiclass_target", "h002_reliability_target_v7_object_cell_evidence_contrast_multiclass_row_v1")
        for label in labels
    ]
    reliability_targets = [
        row
        for row in (
            target_row(label, "relation_reliability_v6_binary_target", "h002_reliability_target_v7_object_cell_evidence_contrast_binary_row_v1")
            for label in labels
        )
        if row is not None
    ]
    geometry_targets = [
        row
        for row in (
            target_row(label, "geometry_support_v6_binary_target", "h002_geometry_support_v6_object_cell_evidence_contrast_binary_row_v1")
            for label in labels
        )
        if row is not None
    ]
    usefulness_targets = [
        row
        for row in (
            target_row(label, "relation_usefulness_v6_binary_target", "h002_relation_usefulness_v6_object_cell_evidence_contrast_binary_row_v1")
            for label in labels
        )
        if row is not None
    ]
    ...
    multiclass_posterior = [
        row
        for row in (
            posterior_candidate_row(label, "relation_reliability_state_v6_multiclass_target", "h002_reliability_target_v7_object_cell_evidence_contrast_multiclass_posterior_candidate_row_v1")
            for label in labels
        )
        if row is not None
    ]
    reliability_posterior = [
        row
        for row in (
            posterior_candidate_row(label, "relation_reliability_v6_binary_target", "h002_reliability_target_v7_object_cell_evidence_contrast_binary_posterior_candidate_row_v1")
            for label in labels
        )
        if row is not None
    ]
    geometry_posterior = [
        row
        for row in (
            posterior_candidate_row(label, "geometry_support_v6_binary_target", "h002_geometry_support_v6_object_cell_evidence_contrast_posterior_candidate_row_v1")
            for label in labels
        )
        if row is not None
    ]
    usefulness_posterior = [
        row
        for row in (
            posterior_candidate_row(label, "relation_usefulness_v6_binary_target", "h002_relation_usefulness_v6_object_cell_evidence_contrast_posterior_candidate_row_v1")
            for label in labels
        )
        if row is not None
    ]
    ...
    output_paths = {
        ...
        "validated_v7_labels": output_dir / "validated_v7_labels.jsonl",
        ...
        "excluded_targets": output_dir / "excluded_v7_targets.jsonl",
    ...
    summary = {
        "schema_version": "h002_reliability_target_v7_object_cell_evidence_contrast_label_ingestion_summary_v1",
        "status": status,
        ...
        "input_paths": {
            ...
            "v7_label_schema": rel_path(schema_path),
    ...
    write_jsonl(output_paths["validated_v7_labels"], labels)
    ...
    write_json(
        ...
        {
            "schema_version": "h002_reliability_target_v7_object_cell_evidence_contrast_target_independence_probe_bundle_v1",
            "probes": probes,
        },
    )
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# H002 Reliability Target V7 Object-Cell Evidence-Contrast Label Ingestion",
        "",
