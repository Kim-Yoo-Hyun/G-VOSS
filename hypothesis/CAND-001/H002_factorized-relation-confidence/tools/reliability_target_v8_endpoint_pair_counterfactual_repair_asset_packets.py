#!/usr/bin/env python3
"""Generate packets for the H002 v8 repair counterfactual batch."""
...
REPAIR_DIR = RGA_ROOT / "reliability_target_v8_endpoint_pair_counterfactual_target_repair_and_additional_mining_codex_proxy_user_requested"
DEFAULT_REPAIR_SUMMARY = REPAIR_DIR / "summary.json"
DEFAULT_SELECTED_CANDIDATES = REPAIR_DIR / "selected_primary_candidates.jsonl"
DEFAULT_ASSET_REQUESTS = REPAIR_DIR / "asset_request_manifest.jsonl"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v8_endpoint_pair_counterfactual_repair_asset_packets_codex_proxy_user_requested"
DEFAULT_SCAN_ROOT = REPO_ROOT / "local_dataset/3RScan/scans"

EXPECTED_REPAIR_STATUS = "h002_reliability_target_v8_current_repair_insufficient_additional_mining_ready"
EXPECTED_NEXT_TODO = "reliability_target_v8_endpoint_pair_counterfactual_repair_asset_packets"

SCHEMA_VERSION = "h002_reliability_target_v8_endpoint_pair_counterfactual_repair_asset_packets_v1"
REVIEW_SCOPE = "h002_reliability_v8_endpoint_pair_counterfactual_repair_packeted_review"
GENERATED_PACKET_SOURCE = "generated_v8_endpoint_pair_counterfactual_repair_asset_packet"
EXISTING_PACKET_SOURCE = "existing_prior_asset_packet"
MISSING_PACKET_SOURCE = "missing_v8_endpoint_pair_counterfactual_repair_asset_packet"
...
def validate_inputs(repair_summary: dict[str, Any], seeds: list[dict[str, Any]], asset_requests: list[dict[str, Any]]) -> list[dict[str, Any]]:
    errors: list[dict[str, Any]] = []
    if repair_summary.get("status") != EXPECTED_REPAIR_STATUS:
        errors.append({"error_type": "unexpected_repair_status", "value": repair_summary.get("status")})
    if repair_summary.get("next_todo") != EXPECTED_NEXT_TODO:
        errors.append({"error_type": "unexpected_repair_next_todo", "value": repair_summary.get("next_todo")})
    boundary = repair_summary.get("boundary", {})
    for key in ["validation_usage", "test_usage", "trains_new_posterior", "fills_new_labels", "posterior_allowed", "h001_artifacts_modified", "multi_view_as_model_input"]:
        if boundary.get(key) is not False:
            errors.append({"error_type": "repair_boundary_not_false", "field": key, "value": boundary.get(key)})
    selection = repair_summary.get("selection_summary", {})
    if len(seeds) != selection.get("selected_primary_rows"):
        errors.append({"error_type": "selected_seed_count_mismatch", "seed_rows": len(seeds), "summary_rows": selection.get("selected_primary_rows")})
    if len(asset_requests) != selection.get("asset_needed_primary_rows"):
        errors.append({"error_type": "asset_request_count_mismatch", "asset_request_rows": len(asset_requests), "summary_rows": selection.get("asset_needed_primary_rows")})
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "- Hidden semantic/geometry/endpoint metadata remains hidden.",
        "",
        "## Status",
        "",
        "```text",
        ...
        "| Item | Count |",
        "| --- | ---: |",
        f"| selected primary rows | {counts['input_selected_rows']} |",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    repair_summary = read_json(args.repair_summary)
    seeds = read_jsonl(args.selected_candidates)
    asset_requests = read_jsonl(args.asset_requests)
    validation_errors = validate_inputs(repair_summary, seeds, asset_requests)
    ...
    if validation_errors or path_errors or leakage_hits:
        status = "h002_reliability_target_v8_repair_asset_packets_blocked"
        next_todo = "fix_reliability_target_v8_repair_asset_packets"
        decision = "Repair packet generation is blocked by validation, leakage, or path errors."
    elif status_counts.get("ready", 0) == len(label_rows):
        status = "h002_reliability_target_v8_repair_asset_packets_ready_for_label_readiness"
        next_todo = "reliability_target_v8_endpoint_pair_counterfactual_repair_label_readiness"
        decision = "The full 200-row repair batch is packet-complete and can proceed to label readiness."
    else:
        status = "h002_reliability_target_v8_repair_asset_packets_partial_needs_gap_audit"
        next_todo = "reliability_target_v8_endpoint_pair_counterfactual_repair_asset_packet_gap_audit"
        decision = "Repair packets are partial. Inspect non-ready rows before label readiness/fill."

    output_paths = {
        "summary": output_dir / "summary.json",
        "report": output_dir / "report.md",
        "full_label_sheet": output_dir / "repair_full_label_sheet.tsv",
        "label_ready_sheet": output_dir / "repair_label_ready_sheet.tsv",
        "full_manifest_post_label_only": output_dir / "repair_full_manifest_post_label_only.jsonl",
        "label_ready_manifest_post_label_only": output_dir / "repair_label_ready_manifest_post_label_only.jsonl",
    ...
    summary = {
        ...
        "next_todo": next_todo,
        "input_paths": {
            "repair_summary": rel_path(args.repair_summary),
        ...
        "source_repair_status": repair_summary.get("status"),
        "counts": {
            "input_selected_rows": len(seeds),
