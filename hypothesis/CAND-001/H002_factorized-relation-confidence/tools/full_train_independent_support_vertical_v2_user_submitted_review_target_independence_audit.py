#!/usr/bin/env python3
"""Audit user-submitted rank-band review target independence."""
...
DEFAULT_INGESTION_DIR = RGA_ROOT / "independent_support_vertical_v2_user_submitted_review_ingestion_rank_band70"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "independent_support_vertical_v2_user_submitted_review_target_independence_audit_rank_band70"

GEOMETRY_TARGET = "geometry_validity_user_submitted_review_target"
RELIABILITY_TARGET = "relation_reliability_user_submitted_review_target"
TARGET_INPUTS = {
    GEOMETRY_TARGET: "geometry_validity_user_submitted_review_posterior_rows.jsonl",
    RELIABILITY_TARGET: "relation_reliability_user_submitted_review_posterior_rows.jsonl",
}

SLICE_SPECS = {
    "original_user_submitted_review": {
        "balanced_keys": [],
        "reason": "full user-submitted rank-band review binary target",
        "priority": 99,
    },
    "prior_relation_validity_balanced_user_submitted_review": {
    ...
    "queue_family_balanced_user_submitted_review": {
    ...
    "role_family_balanced_user_submitted_review": {
    ...
    "prior_label_rank_balanced_user_submitted_review": {
...
def validate_rows(target_name: str, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if row.get("user_reported_completed_by_user") is not True:
            errors.append({"target_name": target_name, "error_type": "missing_user_reported_completed_flag", "row_number": index, "blind_review_id": blind_id})
        if row.get("user_submitted_completed_sheet") is not True:
            errors.append({"target_name": target_name, "error_type": "missing_user_submitted_completed_sheet_flag", "row_number": index, "blind_review_id": blind_id})
        if row.get("actual_independent_reviewer_verified") is not False:
            errors.append({"target_name": target_name, "error_type": "unexpected_actual_independent_reviewer_verified_flag", "row_number": index, "blind_review_id": blind_id, "value": row.get("actual_independent_reviewer_verified")})
        ...
        if "audit_only_user_submitted_review_fields" not in row:
            errors.append({"target_name": target_name, "error_type": "missing_audit_only_user_submitted_review_fields", "row_number": index, "blind_review_id": blind_id})
        ...
        for key in ["true_user_review_fields", "hidden_strata", "previous_proxy_labels", "audit_packet_paths", "multi_view_as_model_input"]:
...
def per_target_decision(target_name: str, summaries: list[dict[str, Any]], errors: list[dict[str, Any]]) -> dict[str, Any]:
    original = next(item for item in summaries if item["slice_name"] == "original_user_submitted_review")
    ...
    if errors:
        ...
        decision = "Fix row validation errors before using any user-submitted review target slice."
        next_step = "fix_user_submitted_review_target_independence_audit_errors"
    elif strict:
        status = "strict_controlled_slice_ready"
        decision = "A strict user-submitted review slice clears harmful prior carryover and construction-risk checks."
        next_step = "user_submitted_review_source_feature_join_then_controlled_posterior_smoke"
    elif construction:
        ...
        next_step = "confirm_reviewer_independence_or_collect_external_labels"
    else:
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# H002 User-Submitted Review Target Independence Audit",
        "",
        ...
        "- Reviewer id indicates `codex_packet_only_diagnostic`; do not over-claim verified external independence.",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if all_validation_errors:
        status = "full_train_independent_support_vertical_v2_user_submitted_review_target_independence_audit_errors"
        decision = "Fix validation errors before any user-submitted target slice can be used."
        next_todo = "fix_user_submitted_rank_band70_target_independence_audit_errors"
    elif relation_strict:
        status = "full_train_independent_support_vertical_v2_user_submitted_review_target_independence_audit_relation_strict_slice_ready"
        decision = "A strict relation-reliability slice exists. Reviewer independence caveat must still be resolved before paper-level claims."
        next_todo = "user_submitted_rank_band70_source_feature_join"
    elif relation_construction:
        status = "full_train_independent_support_vertical_v2_user_submitted_review_target_independence_audit_strict_blocked_construction_slice_available"
        decision = "No strict relation-reliability slice clears harmful prior carryover. Construction-only diagnostic slice exists."
        next_todo = "confirm_reviewer_independence_or_collect_external_labels"
    else:
        status = "full_train_independent_support_vertical_v2_user_submitted_review_target_independence_audit_blocked"
        decision = "No size-ready strict or construction-only controlled slice exists for user-submitted relation reliability."
        next_todo = "confirm_reviewer_independence_or_collect_external_labels"
    ...
    summary = {
        "schema_version": "h002_support_vertical_v2_user_submitted_review_target_independence_audit_summary_v1",
        "status": status,
        ...
        "boundary": {
            ...
            "reviewer_id_indicates_codex_diagnostic": ingestion_summary.get("boundary", {}).get("reviewer_id_indicates_codex_diagnostic"),
...
def main() -> int:
    ...
    print(
        ...
        f"reviewer_id_caveat={summary['boundary']['reviewer_id_indicates_codex_diagnostic']} "
