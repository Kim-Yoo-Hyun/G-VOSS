#!/usr/bin/env python3
"""Audit v4 matched-contrast partial asset packets before label fill."""
...
PACKET_DIR = RGA_ROOT / "reliability_target_v4_matched_contrast_asset_packets"
DEFAULT_PACKET_SUMMARY = PACKET_DIR / "summary.json"
DEFAULT_FULL_LABEL_SHEET = PACKET_DIR / "matched_contrast_full_label_sheet.tsv"
DEFAULT_FULL_MANIFEST = PACKET_DIR / "matched_contrast_full_manifest_post_label_only.jsonl"
DEFAULT_GENERATED_PACKET_MANIFEST = PACKET_DIR / "generated_packet_manifest.jsonl"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v4_matched_contrast_asset_packet_gap_audit"

SCHEMA_VERSION = "h002_reliability_target_v4_matched_contrast_asset_packet_gap_audit_v1"
...
def decide_row(packet_row: dict[str, Any], manifest_row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "matched_contrast_pair_id_hidden": manifest_row.get("matched_contrast_pair_id_hidden"),
        "matched_contrast_role_hidden": manifest_row.get("matched_contrast_role_hidden"),
...
def pair_decisions(row_decisions: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    for row in row_decisions:
        by_pair[str(row["matched_contrast_pair_id_hidden"])].append(row)
    ...
    for pair_id, rows in by_pair.items():
        ...
        if "replacement_needed" in row_decision_set:
            ...
            reason = "at least one row in the matched pair needs replacement"
...
def label_sheet_rows(
    ...
    for row in label_rows:
        ...
        pair_decision = pair_decisions_by_id[str(row_decision["matched_contrast_pair_id_hidden"])]
        updated = dict(row)
...
def validate_inputs(packet_summary: dict[str, Any], label_rows: list[dict[str, Any]], manifests: list[dict[str, Any]]) -> list[dict[str, Any]]:
    errors: list[dict[str, Any]] = []
    if packet_summary.get("next_todo") != "reliability_target_v4_matched_contrast_asset_packet_gap_audit":
        errors.append({"error": "unexpected_next_todo", "value": packet_summary.get("next_todo")})
    if packet_summary.get("status") != "h002_reliability_target_v4_matched_contrast_asset_packets_partial":
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        ...
        "- This decides packet usability before v4 label fill.",
        "- Pair integrity is preserved: if one row needs replacement, the whole matched pair is excluded.",
        "",
        "## Status",
        "",
        f"`{summary['status']}`",
    ...
    for row in summary["excluded_pair_decisions"]:
        lines.append(f"| `{row['matched_contrast_pair_id_hidden']}` | {row['pair_gap_reason']} |")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    ready_label_rows, excluded_label_rows = label_sheet_rows(
        label_rows,
        row_by_id,
        pair_by_id,
        output_dir,
    ...
    replacement_requests = [
        {
            "replacement_request_reason": "v4_matched_contrast_pair_excluded_by_asset_gap_audit",
            "matched_contrast_pair_id_hidden": pair["matched_contrast_pair_id_hidden"],
    ...
    role_counts = Counter(
        str(manifest_by_id[str(row["blind_review_id"])].get("matched_contrast_role_hidden"))
    ...
    status = (
        ...
        else "h002_reliability_target_v4_matched_contrast_asset_packet_gap_audit_needs_replacement"
    )
    next_todo = (
        "reliability_target_v4_matched_contrast_label_readiness"
        if status == "h002_reliability_target_v4_matched_contrast_asset_packet_gap_audit_ready_for_label_readiness"
        else "reliability_target_v4_matched_contrast_replacement_mining"
    )
    decision = (
        "Proceed with 79 label-ready matched pairs and keep one replacement-needed pair out of label fill."
        if status == "h002_reliability_target_v4_matched_contrast_asset_packet_gap_audit_ready_for_label_readiness"
    ...
    output_paths["excluded_pair_ids"].write_text(
        "\n".join(row["matched_contrast_pair_id_hidden"] for row in excluded_pairs) + ("\n" if excluded_pairs else ""),
        encoding="utf-8",
    )
