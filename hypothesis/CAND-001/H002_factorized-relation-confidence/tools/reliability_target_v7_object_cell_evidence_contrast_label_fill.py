#!/usr/bin/env python3
"""Fill the v7 object-cell evidence-contrast reliability label sheet as a Codex proxy."""
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v7_object_cell_evidence_contrast_label_fill_codex_proxy_user_requested"

SCHEMA_VERSION = "h002_reliability_target_v7_object_cell_evidence_contrast_label_fill_summary_v1"
STATUS_READY = "h002_reliability_target_v7_object_cell_evidence_contrast_label_filled_codex_proxy_user_requested"
STATUS_ERROR = "h002_reliability_target_v7_object_cell_evidence_contrast_label_fill_errors"
REVIEWER_ID = "(codex_proxy_v7_user_requested_visible_only)"
REVIEW_ROUND = "1"
...
def validate_inputs(
    ...
    if readiness_summary.get("next_todo") != "reliability_target_v7_object_cell_evidence_contrast_label_fill":
        errors.append({"error_type": "unexpected_readiness_next_todo", "value": readiness_summary.get("next_todo")})
    if readiness_summary.get("status") != "h002_reliability_target_v7_object_cell_evidence_contrast_label_readiness_ready_for_label_fill":
...
def fill_row(row: dict[str, str]) -> dict[str, Any]:
    ...
    filled["label_notes_v6"] = (
        "codex proxy v7 visible-only fill; used visible subject/object/predicate labels, "
...
def label_record(row: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "h002_reliability_target_v7_object_cell_evidence_contrast_proxy_label_v1",
        ...
        "provenance": {
            "batch_name": "reliability_target_v7_object_cell_evidence_contrast_label_fill",
...
def multiclass_target_rows(filled_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in filled_rows:
        ...
        rows.append(
            {
                "schema_version": "h002_reliability_target_v7_object_cell_evidence_contrast_multiclass_target_v1",
                "blind_review_id": row["blind_review_id"],
                "scan_id": row["scan_id"],
                "scene_context_id": row["scene_context_id"],
                "subject_id": row["subject_id"],
                ...
                "target_source": "codex_proxy_user_requested_visible_only_v7_object_cell_evidence_contrast_label_fill",
                "split": "train_only",
                "paper_evidence_allowed": False,
            }
        )
...
def binary_target_rows(filled_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in filled_rows:
        ...
        rows.append(
            {
                "schema_version": "h002_reliability_target_v7_object_cell_evidence_contrast_binary_target_v1",
                "blind_review_id": row["blind_review_id"],
                "scan_id": row["scan_id"],
                "scene_context_id": row["scene_context_id"],
                "subject_id": row["subject_id"],
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# H002 Reliability Target V7 Object-Cell Evidence-Contrast Label Fill",
        "",
        ...
        "- Hidden semantic-geometry bucket, semantic score/rank, `p_geom_valid`, geometry status, source queue, object-cell metadata, label-match status, and target-construction metadata are not used before label lock.",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    output_paths = {
        ...
        "completed_sheet": output_dir / "completed_v7_object_cell_evidence_contrast_label_sheet_codex_proxy_user_requested.tsv",
        "v7_proxy_labels": output_dir / "v7_proxy_labels.jsonl",
    ...
    next_todo = "fix_reliability_target_v7_object_cell_evidence_contrast_label_fill" if errors else "reliability_target_v7_object_cell_evidence_contrast_label_ingestion"
    decision = (
        "Input or fill validation errors block ingestion."
        if errors
        else (
            "Filled the 240-row v7 object-cell evidence-contrast sheet as a user-requested Codex proxy. "
    ...
    write_jsonl(output_paths["v7_proxy_labels"], label_rows)
