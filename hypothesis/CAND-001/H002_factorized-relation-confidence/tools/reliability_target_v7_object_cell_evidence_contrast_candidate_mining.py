#!/usr/bin/env python3
"""Mine H002 v7 object-cell evidence-contrast candidate rows."""
...
READY_STATUS = "h002_reliability_target_v7_object_cell_evidence_contrast_feasibility_ready"
READY_CAVEAT_STATUS = "h002_reliability_target_v7_object_cell_evidence_contrast_feasibility_ready_with_structural_caveat"
EXPECTED_NEXT_TODO = "reliability_target_v7_object_cell_evidence_contrast_candidate_mining"
...
MAX_ROWS_PER_STRICT_GROUP = 8
MAX_ROWS_PER_SCAN = 16
MAX_OBJECT_PAIR_SHARE = 0.10
MAX_OBJECT_CELL_SHARE = 0.10
MAX_STRUCTURAL_ROWS = 24
MAX_HARD_ROOM_SURFACE_ROWS = 24

REVIEW_SCOPE = "h002_reliability_v7_object_cell_evidence_contrast_review"
SCHEMA_VERSION = "h002_reliability_target_v7_object_cell_evidence_contrast_candidate_mining_v1"
...
def blind_review_id(row: dict[str, Any]) -> str:
    return "ftv7oc_" + stable_hash("h002_v7_object_cell:" + str(row["prediction_id"]))[:12]
...
def validate_upstream(summary: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if summary.get("status") not in {READY_STATUS, READY_CAVEAT_STATUS}:
        errors.append({"error_type": "unexpected_feasibility_status", "actual": summary.get("status")})
    if summary.get("next_todo") != EXPECTED_NEXT_TODO:
        errors.append({"error_type": "unexpected_feasibility_next_todo", "actual": summary.get("next_todo")})
    if summary.get("candidate_mining_allowed") is not True:
        errors.append({"error_type": "candidate_mining_not_allowed_by_feasibility"})
    for field in ["posterior_allowed", "label_fill_allowed", "validation_used", "test_used", "h001_artifacts_modified"]:
        expected = False
        if summary.get(field) is not expected:
            errors.append({"error_type": "unexpected_feasibility_boundary", "field": field, "actual": summary.get(field)})
    gates = summary.get("feasibility_gates") or {}
    if gates.get("core_feasibility_pass") is not True:
        errors.append({"error_type": "core_feasibility_gate_not_passed"})
...
def internal_row(row: dict[str, Any], packet: dict[str, Any] | None) -> dict[str, Any]:
    ...
    output = {
        "schema_version": "h002_reliability_target_v7_object_cell_evidence_contrast_internal_candidate_v1",
        "batch_name": "reliability_target_v7_object_cell_evidence_contrast_candidate_mining",
        ...
        "strict_group_key_hidden": row.get("strict_group_key"),
        "strict_group_capacity_after_cap_hidden": row.get("strict_group_capacity_after_cap"),
        "strict_group_b2_count_hidden": row.get("strict_group_b2_count"),
        "strict_group_b3_count_hidden": row.get("strict_group_b3_count"),
        "structural_pair_hidden": bool(row.get("structural_pair")),
        "hard_room_surface_pair_hidden": bool(row.get("hard_room_surface_pair")),
        "same_label_endpoint_hidden": bool(row.get("same_label_endpoint")),
...
def asset_request_row(row: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "h002_reliability_target_v7_object_cell_evidence_contrast_asset_request_v1",
        "asset_request_reason": "v7_object_cell_evidence_contrast_candidate_needs_multiview_pointcloud_context_packet",
        "blind_review_id": row["blind_review_id"],
        "prediction_id": row.get("prediction_id"),
        "scan_id": row.get("scan_id"),
        "scene_context_id": row.get("scene_context_id"),
        ...
        "hidden_strata": {
            ...
            "strict_group_key": row.get("strict_group_key_hidden"),
...
def count_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    group_counts = Counter(row["strict_group_key_hidden"] for row in rows)
    ...
    return {
        ...
        "max_single_object_label_pair_share": max(pair_counts.values()) / len(rows) if rows else 0.0,
            ...
            output.append(
                {
                    ...
                    "strict_groups": len({row["strict_group_key_hidden"] for row in subset}),
...
def cap_audit_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    counters = {
        "strict_group": Counter(row["strict_group_key_hidden"] for row in rows),
    ...
    limits = {
        "strict_group": MAX_ROWS_PER_STRICT_GROUP,
        "scan_id": MAX_ROWS_PER_SCAN,
        "subject_object_label_pair": int(TARGET_ROWS * MAX_OBJECT_PAIR_SHARE),
        "subject_object_family_cell": int(TARGET_ROWS * MAX_OBJECT_CELL_SHARE),
    }
    output: list[dict[str, Any]] = []
        ...
        [
            {
                ...
                "limit": MAX_STRUCTURAL_ROWS,
                "max_observed": structural_count,
                "max_key": "structural_pair",
                "unique_values": 1,
                "violates_cap": structural_count > MAX_STRUCTURAL_ROWS,
            },
            {
                "cap_name": "hard_room_surface_pair_rows",
                "limit": MAX_HARD_ROOM_SURFACE_ROWS,
                "max_observed": hard_surface_count,
                "max_key": "hard_room_surface_pair",
                "unique_values": 1,
                "violates_cap": hard_surface_count > MAX_HARD_ROOM_SURFACE_ROWS,
            },
        ]
    )
    return output
...
def validate_candidate_counts(counts: dict[str, Any], bucket_rows: list[dict[str, Any]], cap_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if counts["max_single_object_label_pair_share"] > MAX_OBJECT_PAIR_SHARE:
        errors.append({"error_type": "object_pair_share_cap_violation", "value": counts["max_single_object_label_pair_share"]})
    if counts["max_single_subject_object_family_cell_share"] > MAX_OBJECT_CELL_SHARE:
        errors.append({"error_type": "object_cell_share_cap_violation", "value": counts["max_single_subject_object_family_cell_share"]})
    for row in cap_rows:
        if row["violates_cap"]:
...
def report_text(summary: dict[str, Any]) -> str:
    ...
    return f"""# H002 V7 Object-Cell Evidence Contrast Candidate Mining
...
- The v6 reliability label schema is kept, but the sampling unit is v7 object-cell evidence contrast.
- Semantic score, rank, geometry status, `p_geom_valid`, queue kind, strict group key, and object-cell metadata are hidden from the visible label surface.
...
strict_groups_used = {counts["strict_groups_used"]}
max_rows_per_strict_group = {counts["max_rows_per_strict_group"]}
max_single_object_label_pair_share = {counts["max_single_object_label_pair_share"]:.4f}
...
The candidate queue is fixed with object/category concentration caps and B2/B3 balance. This is still not posterior evidence.
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    packet_ready_visible_rows = [visible_row(row) for row in selected if row["packet_status_hidden"] == "ready"]
    ...
    if validation_errors:
        status = "h002_reliability_target_v7_object_cell_evidence_contrast_candidate_mining_errors"
        next_todo = "fix_reliability_target_v7_object_cell_evidence_contrast_candidate_mining_errors"
    elif counts["asset_needed_rows"] > 0:
        status = "h002_reliability_target_v7_object_cell_evidence_contrast_candidate_mining_ready_needs_asset_packets"
        next_todo = "reliability_target_v7_object_cell_evidence_contrast_asset_packets"
    else:
        status = "h002_reliability_target_v7_object_cell_evidence_contrast_candidate_mining_ready_for_label_readiness"
    ...
    output_paths = {
        ...
        "label_sheet": rel_path(output_dir / "v7_object_cell_evidence_contrast_label_sheet.tsv"),
        "packet_ready_label_sheet": rel_path(output_dir / "v7_object_cell_evidence_contrast_packet_ready_label_sheet.tsv"),
    ...
    summary = {
        "schema_version": "h002_reliability_target_v7_object_cell_evidence_contrast_candidate_mining_summary_v1",
        ...
        "structural_caveat_action": "candidate rows are capped at <=24 structural and <=24 hard room-surface rows; selected rows currently have zero for both.",
        "input_paths": {
    ...
    write_tsv(output_dir / "v7_object_cell_evidence_contrast_label_sheet.tsv", visible_rows, VISIBLE_FIELDS)
    write_tsv(output_dir / "v7_object_cell_evidence_contrast_packet_ready_label_sheet.tsv", packet_ready_visible_rows, VISIBLE_FIELDS)
...
def main() -> None:
    ...
    print(
        "status={status} rows={rows} ready={ready} asset_needed={asset_needed} errors={errors} next={next_todo}".format(
            status=summary["status"],
