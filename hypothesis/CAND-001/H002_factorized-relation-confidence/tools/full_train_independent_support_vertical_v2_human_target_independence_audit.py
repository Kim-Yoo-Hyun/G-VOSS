#!/usr/bin/env python3
"""Audit Codex-proxy human support/vertical target independence."""
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "independent_support_vertical_v2_human_target_independence_audit_codex_proxy_user_review_pending"

GEOMETRY_TARGET = "geometry_validity_human_target"
RELIABILITY_TARGET = "relation_reliability_human_target"
TARGET_INPUTS = {
    GEOMETRY_TARGET: "geometry_validity_human_posterior_rows.jsonl",
    RELIABILITY_TARGET: "relation_reliability_human_posterior_rows.jsonl",
}
...
SLICE_SPECS = {
    "original_human": {
        "balanced_keys": [],
        "reason": "full Codex-proxy human binary target",
        "priority": 99,
    },
    "prior_relation_validity_balanced_human": {
    ...
    "prior_label_rank_balanced_human": {
...
def validate_rows(target_name: str, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if row.get("human_confirmed_by_user_request") is not True:
            errors.append(
                {
                    "target_name": target_name,
                    "error_type": "missing_human_confirmed_by_user_request_flag",
                    "row_number": index,
                    "blind_review_id": blind_id,
                }
            )
        if row.get("actual_human_reviewer") is not False:
            errors.append(
                {
                    "target_name": target_name,
                    "error_type": "unexpected_actual_human_reviewer_flag",
                    "row_number": index,
                    "blind_review_id": blind_id,
                    "actual_human_reviewer": row.get("actual_human_reviewer"),
                }
            )
        if row.get("user_review_pending") is not True:
            errors.append(
                {
                    "target_name": target_name,
                    "error_type": "missing_user_review_pending_flag",
                    "row_number": index,
                    "blind_review_id": blind_id,
                }
            )
        if row.get("paper_locked") is not False:
            errors.append(
                {
                    "target_name": target_name,
                    "error_type": "unexpected_paper_locked_flag",
                    "row_number": index,
                    "blind_review_id": blind_id,
                    "paper_locked": row.get("paper_locked"),
                }
            )
        if "audit_only_human_label_fields" not in row:
            errors.append({"target_name": target_name, "error_type": "missing_audit_only_human_label_fields", "row_number": index, "blind_review_id": blind_id})
...
def per_target_decision(target_name: str, summaries: list[dict[str, Any]], errors: list[dict[str, Any]]) -> dict[str, Any]:
    original = next(item for item in summaries if item["slice_name"] == "original_human")
    ...
    if errors:
        ...
        decision = "Fix row validation errors before using any human-target slice."
        next_step = "fix_human_target_independence_audit_errors"
    elif strict:
        status = "strict_controlled_slice_ready"
        decision = (
            "A strict human-proxy controlled slice clears harmful prior-label carryover and "
            "construction-risk checks. Source-score feature join is still required before posterior smoke."
        )
        next_step = "human_source_feature_join_then_controlled_posterior_smoke"
    elif construction:
        ...
        next_step = "revise_human_label_protocol_or_add_external_review_evidence"
    else:
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# H002 Human Support/Vertical V2 Target Independence Audit",
        "",
        ...
        "- Labels are Codex proxy human fields that the workflow treats as human-confirmed by user request.",
        "- They remain user-review-pending and are not external human annotation before user confirmation.",
        "- Hidden metadata is used only after label lock for audit and controlled-slice construction.",
        "- Human label fields, hidden strata, and v2 reference axes are not posterior inputs.",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if all_validation_errors:
        status = "full_train_independent_support_vertical_v2_human_target_independence_audit_errors"
        decision = "Fix validation errors before any human-target slice can be used."
        next_todo = "fix_full_train_independent_support_vertical_v2_human_target_independence_audit_errors"
    elif relation_strict:
        status = "full_train_independent_support_vertical_v2_human_target_independence_audit_relation_strict_slice_ready"
        decision = (
            "A strict relation-reliability slice exists under the user-requested human-proxy target. "
            "Before posterior smoke, join source rank/score and p_geom_valid as deployable features."
        )
        next_todo = "full_train_independent_support_vertical_v2_human_source_feature_join"
        ...
        next_todo = "revise_human_label_protocol_or_add_external_review_evidence"
    else:
        status = "full_train_independent_support_vertical_v2_human_target_independence_audit_blocked"
        decision = "No size-ready strict or construction-only controlled slice exists for human-proxy relation reliability."
        next_todo = "revise_human_label_protocol_or_add_external_review_evidence"
    ...
    summary = {
        "schema_version": "h002_support_vertical_v2_human_target_independence_audit_summary_v1",
        "status": status,
        ...
        "boundary": {
            ...
            "human_label_fields_used_for_target_or_audit_only": True,
