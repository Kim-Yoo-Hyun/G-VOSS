#!/usr/bin/env python3
"""Audit informative-anchor H002 reliability target v3 independence."""
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v3_informative_anchor_target_independence_audit_codex_proxy_user_requested"
...
SLICE_SPECS = {
    "original_informative_anchor_v3": {
        "balanced_keys": [],
        "reason": "full informative-anchor v3 binary target",
        "priority": 99,
    },
    "anchor_category_balanced_v3": {
        "balanced_keys": ["anchor_category_hidden"],
        "reason": "matched positives/negatives within informative-anchor sampling category",
        "priority": 1,
    },
    ...
    "object_family_cell_balanced_v3": {
        "balanced_keys": ["object_family_cell_hidden"],
        "reason": "matched positives/negatives within object family cell",
        "priority": 4,
    },
    "endpoint_family_cell_balanced_v3": {
        "balanced_keys": ["endpoint_family_cell_hidden"],
        "reason": "matched positives/negatives within endpoint-family cell",
        "priority": 5,
    },
    ...
    "label_geometry_bucket_balanced_v3": {
        "balanced_keys": ["label_geometry_bucket_hidden"],
        "reason": "matched positives/negatives within label/geometry bucket",
        "priority": 9,
    },
    "family_balanced_v3": {
    ...
    "anchor_endpoint_balanced_v3": {
        "balanced_keys": ["anchor_category_hidden", "endpoint_flag_pattern_hidden"],
        "reason": "matched positives/negatives within anchor category and endpoint pattern",
        "priority": 14,
    },
    ...
    "rank_endpoint_balanced_v3": {
        "balanced_keys": ["rank_band_hidden", "endpoint_flag_pattern_hidden"],
        "reason": "matched positives/negatives within rank band and endpoint pattern",
        "priority": 16,
    },
    "anchor_rank_balanced_v3": {
        "balanced_keys": ["anchor_category_hidden", "rank_band_hidden"],
        "reason": "matched positives/negatives within anchor category and semantic rank band",
        "priority": 17,
    },
}
...
def all_group_summaries(
    ...
    key_groups = [
        ("anchor_sampling", "hidden_post_label_audit", ANCHOR_KEYS),
...
def counts_for(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "by_anchor_category": dict(sorted(Counter(group_value(row, "anchor_category_hidden") for row in rows).items())),
...
def slice_summary(
    ...
    anchor = risk_summaries(summaries, "anchor_sampling")
    ...
    return {
        ...
        "anchor_sampling_risk_count": len(anchor),
        ...
        "top_anchor_sampling_risks": top_risks(anchor),
...
def per_target_decision(target_name: str, summaries: list[dict[str, Any]], errors: list[dict[str, Any]]) -> dict[str, Any]:
    original = next(item for item in summaries if item["slice_name"] == "original_informative_anchor_v3")
    ...
    if errors:
        ...
        decision = "Fix row validation errors before using informative-anchor v3 target slices."
        next_step = "fix_reliability_target_v3_informative_anchor_target_independence_errors"
        ...
        next_step = "reliability_target_v3_informative_anchor_path_decision"
    elif strict:
        status = "strict_controlled_slice_ready"
        decision = "A strict informative-anchor controlled slice clears anchor, endpoint/object, construction, and object-label shortcut checks."
        next_step = "reliability_target_v3_informative_anchor_source_feature_join"
    elif diagnostic:
        status = "diagnostic_slice_only"
        decision = "A diagnostic informative-anchor slice clears anchor, endpoint/object, and construction checks but still has object-label risk or insufficient strict mass."
        next_step = "reliability_target_v3_informative_anchor_path_decision"
    elif geometry_only:
...
def global_decision(target_decisions: dict[str, Any], errors: list[dict[str, Any]]) -> tuple[str, str, str]:
    if errors:
        return (
            "h002_reliability_target_v3_informative_anchor_target_independence_audit_errors",
            "Fix informative-anchor target audit validation errors before any target decision.",
            "fix_reliability_target_v3_informative_anchor_target_independence_errors",
        )
    ...
    if reliability["status"] == "strict_controlled_slice_ready":
        return (
            "h002_reliability_target_v3_informative_anchor_target_independence_audit_reliability_ready",
            "The main relation reliability target has a strict controlled slice; source feature join may be planned next.",
            "reliability_target_v3_informative_anchor_source_feature_join",
        )
    if reliability["status"] == "diagnostic_slice_only":
        return (
            "h002_reliability_target_v3_informative_anchor_target_independence_audit_diagnostic_only",
            "The main reliability target has only a diagnostic slice; decide whether to accept a scoped diagnostic smoke or resample.",
            "reliability_target_v3_informative_anchor_path_decision",
        )
    if reliability["status"] == "blocked_no_controlled_slice" and geometry["original"]["min_class"] >= MIN_POSITIVES_FOR_POSTERIOR:
        return (
            "h002_reliability_target_v3_informative_anchor_target_independence_audit_blocked_geometry_support_available",
            (
                "Informative-anchor labels fixed positive sparsity for reliability, but no independent controlled reliability "
                "slice survives anchor/category, endpoint/object, construction, and visible object checks. Geometry-support "
                "still has mass, but it should remain an evidence axis rather than the main relation reliability target."
            ),
            "reliability_target_v3_informative_anchor_path_decision",
        )
    return (
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        ...
        "- Majority-baseline excess is reported to avoid mistaking target imbalance for real shortcut signal.",
        "",
        "## Status",
        "",
        f"`{summary['status']}`",
    ...
    risk_fields = [
        ("anchor_sampling", "top_anchor_sampling_risks"),
    ...
    lines.extend(
        [
            ...
            "## Controlled Slices",
            "",
            "| Target | Slice | Rows | Pos | Neg | Positive Sparse | Anchor | Endpoint/Object | Construction | Geometry Align | Object Risk | Strict | Diagnostic |",
            "| --- | --- | ---: | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | --- | --- |",
        ]
    )
    for item in sorted(
        ...
        lines.append(
            ...
            f"`{item['positive_sparse']}` | {item['anchor_sampling_risk_count']} | {item['endpoint_object_structure_risk_count']} | "
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary = {
        "schema_version": "h002_reliability_target_v3_informative_anchor_target_independence_audit_summary_v1",
        "status": status,
