#!/usr/bin/env python3
"""Audit target independence for H002 v14 physical relation-family targets."""
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v14_physical_relation_family_target_independence_audit"
...
EXPECTED_NEXT_TODO = "reliability_target_v14_physical_relation_family_target_independence_audit"
NEXT_TODO = "reliability_target_v14_physical_relation_family_path_decision_after_audit"

SCHEMA_VERSION = "h002_reliability_target_v14_physical_relation_family_target_independence_audit_v1"
...
TARGET_SPECS = {
    ...
    "endpoint_multiclass": {
        "field": "endpoint_identity_state_v14",
        "usable": None,
        "role": "diagnostic",
    },
    "coverage_multiclass": {
        "field": "coverage_state_v14",
        "usable": None,
        "role": "provenance",
    },
}
...
SLICE_SPECS = {
    "full": [],
    "same_family": ["predicate_family"],
    "same_predicate": ["predicate_label"],
    "same_source_queue": ["source_queue_hidden"],
    "same_queue_kind": ["queue_kind_hidden"],
    "same_quota_cell": ["quota_cell_id_hidden"],
    "same_quota_role": ["quota_cell_role_hidden"],
    "same_rank_band": ["rank_band_hidden"],
    "same_semantic_rank_coarse": ["semantic_rank_band_coarse"],
    ...
    "same_geometry_witness": ["geometry_witness_summary_v14"],
    "same_support_or_vertical_witness": ["support_or_vertical_witness_summary_v14"],
    "same_endpoint_summary": ["endpoint_identity_summary_v14"],
    "same_family_queue": ["predicate_family", "source_queue_hidden"],
    "same_family_rank": ["predicate_family", "rank_band_hidden"],
    "same_family_geometry": ["predicate_family", "geometry_status_hidden"],
    "same_predicate_queue": ["predicate_label", "source_queue_hidden"],
    "same_predicate_rank": ["predicate_label", "rank_band_hidden"],
    "same_predicate_geometry": ["predicate_label", "geometry_status_hidden"],
    "same_quota_rank": ["quota_cell_id_hidden", "rank_band_hidden"],
    "same_quota_geometry": ["quota_cell_id_hidden", "geometry_status_hidden"],
...
CONTROL_EQUIVALENCE = {
    ...
    "source_queue_hidden": {"source_queue_hidden", "queue_kind_hidden"},
    "queue_kind_hidden": {"source_queue_hidden", "queue_kind_hidden"},
    "geometry_status_hidden": {"geometry_status_hidden", "h001_verification_status_hidden", "p_geom_bin_hidden"},
    "h001_verification_status_hidden": {"geometry_status_hidden", "h001_verification_status_hidden", "p_geom_bin_hidden"},
    "p_geom_bin_hidden": {"geometry_status_hidden", "h001_verification_status_hidden", "p_geom_bin_hidden"},
    "predicate_family": {"predicate_family", "relation_family_visible"},
    "relation_family_visible": {"predicate_family", "relation_family_visible"},
}

ALLOWED_PREDICATES = {"lying on", "standing on", "lower than"}
ALLOWED_FAMILIES = {"support_contact", "relative_vertical"}
...
def validate_ingestion(summary: dict[str, Any], rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if row.get("predicate_family") not in ALLOWED_FAMILIES:
...
def p_geom_bin(row: dict[str, Any]) -> str:
    existing = row.get("p_geom_bin_hidden")
    if existing not in (None, ""):
        return str(existing)
    try:
        score = float(row.get("p_geom_valid_hidden"))
    except (TypeError, ValueError):
        return "missing"
    if score >= 0.75:
        return "high"
...
def enrich_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        item["p_geom_bin_hidden"] = p_geom_bin(row)
...
def majority_risk(rows: list[dict[str, Any]], predictor: str, target_name: str) -> dict[str, Any]:
    ...
    for group_value, counter in groups.items():
        ...
        if len(counter) > 1:
            large_pure = large_pure or (total >= RISK_THRESHOLDS["large_group_rows"] and majority_rate >= RISK_THRESHOLDS["large_group_purity"])
        top_groups.append(
            {
...
def audit_target(rows: list[dict[str, Any]], target_name: str) -> dict[str, Any]:
    ...
    if len(counts) < 2:
        status = "single_class_provenance_only"
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# H002 V14 Physical Relation-Family Target Independence Audit",
        "",
    ...
    lines.extend(
        [
            ...
            "## Key Findings",
            "",
            f"- Primary reliability target has `min_class_count = {relation['min_class_count']}`, below the posterior gate `{POSTERIOR_MIN_PER_CLASS}`.",
            ...
            f"- Strict clear relation slices: `{relation['strict_clear_slice_count']}`.",
            f"- Diagnostic clear relation slices: `{relation['diagnostic_clear_slice_count']}`.",
            "- Same visible-pair mixed contrast exists, but class mass and shortcut controls still block posterior smoke.",
            "",
            "## Boundary",
            "",
            "- Train-only rows.",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if validation_errors:
        status = "h002_reliability_target_v14_physical_relation_family_target_independence_audit_errors"
        decision = "Validation errors remain; posterior smoke is not allowed."
    elif relation["posterior_allowed"]:
        status = "h002_reliability_target_v14_physical_relation_family_target_independence_audit_ready"
        ...
        status = "h002_reliability_target_v14_physical_relation_family_target_independence_audit_blocked_shortcut_risk"
