#!/usr/bin/env python3
"""Create the H002 reliability target v4 matched-contrast label package."""
...
PLAN_DIR = RGA_ROOT / "reliability_target_v4_matched_contrast_plan"
DEFAULT_PLAN_SUMMARY = PLAN_DIR / "summary.json"
DEFAULT_SAMPLING_CONTRACT = PLAN_DIR / "sampling_contract.json"
DEFAULT_SEEDS = PLAN_DIR / "seed_preview_internal.jsonl"
DEFAULT_ASSET_REQUESTS = PLAN_DIR / "asset_request_preview.jsonl"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v4_matched_contrast_candidate_mining"

REVIEW_SCOPE = "h002_reliability_v4_relation_review"
SCHEMA_VERSION = "h002_reliability_target_v4_matched_contrast_candidate_mining_v1"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sampling-contract", type=Path, default=DEFAULT_SAMPLING_CONTRACT)
...
def blind_review_id(row: dict[str, Any]) -> str:
    return "ftv4mc_" + stable_hash("h002_reliability_v4_matched_contrast:" + str(row["prediction_id"]))[:12]
...
def manifest_row(row: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "h002_reliability_target_v4_matched_contrast_manifest_v1",
        "batch_name": "reliability_target_v4_matched_contrast_candidate_mining",
        "blind_review_id": blind_review_id(row),
        ...
        "matched_contrast_pair_id_hidden": row.get("matched_contrast_pair_id"),
        "matched_contrast_role_hidden": row.get("matched_contrast_role_hidden"),
        "contrast_role_hidden": row.get("contrast_role_hidden"),
        "matched_contrast_level_hidden": row.get("matched_contrast_level_hidden"),
        "matched_contrast_stratum_hidden": row.get("matched_contrast_stratum_hidden"),
...
def pair_summary_rows(seeds: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in seeds:
        by_pair[str(row.get("matched_contrast_pair_id"))].append(row)

    output: list[dict[str, Any]] = []
    for pair_id in sorted(by_pair):
        pair_rows = by_pair[pair_id]
        role_map = {str(row.get("matched_contrast_role_hidden")): row for row in pair_rows}
        ...
        output.append(
            {
                ...
                "matched_contrast_level_hidden": pair_rows[0].get("matched_contrast_level_hidden", ""),
                "matched_contrast_stratum_hidden": pair_rows[0].get("matched_contrast_stratum_hidden", ""),
                "predicate_label": pair_rows[0].get("predicate_label", ""),
                "predicate_family": pair_rows[0].get("predicate_family", ""),
                "object_family_cell_hidden": pair_rows[0].get("object_family_cell_hidden", ""),
...
def stratum_summary_rows(seeds: list[dict[str, Any]]) -> list[dict[str, Any]]:
    by_stratum: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for row in seeds:
        by_stratum[str(row.get("matched_contrast_stratum_hidden"))].append(row)

    output: list[dict[str, Any]] = []
    for stratum in sorted(by_stratum):
        rows = by_stratum[stratum]
        pair_ids = {row.get("matched_contrast_pair_id") for row in rows}
        output.append(
            {
                "matched_contrast_stratum_hidden": stratum,
                "row_count": len(rows),
                "pair_count": len(pair_ids),
                "positive_proxy_rows": sum(1 for row in rows if row.get("matched_contrast_role_hidden") == "positive_proxy"),
                "negative_proxy_rows": sum(1 for row in rows if row.get("matched_contrast_role_hidden") == "negative_proxy"),
...
def validate_inputs(
    ...
    errors: list[dict[str, Any]] = []
    if plan_summary.get("status") != "h002_reliability_target_v4_matched_contrast_plan_ready_with_asset_requests":
        errors.append({"field": "plan_summary.status", "error": "unexpected_status", "value": plan_summary.get("status")})
    if plan_summary.get("next_todo") != "reliability_target_v4_matched_contrast_candidate_mining":
        errors.append({"field": "plan_summary.next_todo", "error": "unexpected_next_todo", "value": plan_summary.get("next_todo")})
    if sampling_contract.get("selected_matching_level") != "predicate_object_rank_controlled":
        errors.append(
            {
                "field": "sampling_contract.selected_matching_level",
                "error": "unexpected_matching_level",
                "value": sampling_contract.get("selected_matching_level"),
            }
        )
    if sampling_contract.get("target_rows") != len(seeds):
        errors.append(
            {
                "field": "sampling_contract.target_rows",
                "error": "seed_count_mismatch",
                "expected": sampling_contract.get("target_rows"),
                "actual": len(seeds),
            }
        )
    if plan_summary.get("preview_selection", {}).get("asset_needed_rows") != len(asset_requests):
        errors.append(
            {
                "field": "plan_summary.preview_selection.asset_needed_rows",
                "error": "asset_request_count_mismatch",
                "expected": plan_summary.get("preview_selection", {}).get("asset_needed_rows"),
    ...
    for row in seeds:
        ...
        pair_roles[str(row.get("matched_contrast_pair_id"))][str(row.get("matched_contrast_role_hidden"))] += 1

    for pair_id, roles in sorted(pair_roles.items()):
        if roles.get("positive_proxy", 0) != 1 or roles.get("negative_proxy", 0) != 1:
            errors.append({"matched_contrast_pair_id": pair_id, "roles": dict(roles), "error": "invalid_pair_role_counts"})
    return errors
...
def label_schema() -> dict[str, Any]:
    return {
        ...
        "completion_fields": {
            "endpoint_identity_v4": ["same_endpoints", "endpoint_identity_issue", "uncertain"],
            "pair_evaluability_v4": ["evaluable", "not_evaluable", "needs_more_evidence"],
            "geometry_support_v4": ["supports", "contradicts", "ambiguous", "not_evaluable"],
            "relation_usefulness_v4": ["useful_nontrivial", "trivial_or_redundant", "not_a_relation", "uncertain"],
            "relation_reliability_v4": ["reliable", "unreliable", "uncertain"],
            "primary_reason_v4": [
                "geometric_support",
...
def report_text(summary: dict[str, Any]) -> str:
    return f"""# H002 Reliability Target V4 Matched Contrast Candidate Mining
...
- Matched-contrast role, stratum, rank, semantic score, geometry status, and proxy labels are hidden from the label surface.
...
- The v4 matched-contrast label package is ready, but only `{summary["counts"]["packet_ready_rows"]}` of `{summary["counts"]["label_rows"]}` rows currently have reusable evidence packets.
...
- The packet-ready fallback sheet is too small for posterior reopening; it is only a debugging/format sanity surface.
- Posterior smoke remains blocked until reviewed labels are ingested and the target-independence audit passes.
...
def main() -> None:
    ...
    sampling_contract = read_json(args.sampling_contract)
    ...
    input_errors = validate_inputs(plan_summary, sampling_contract, seeds, plan_asset_requests)
    ...
    stratum_rows = stratum_summary_rows(seeds)
    leakage = leakage_hits(VISIBLE_FIELDS)
    packet_errors = packet_path_errors(seeds)

    output_paths = {
        "summary": out_dir / "summary.json",
        "report": out_dir / "report.md",
        "label_sheet": out_dir / "matched_contrast_label_sheet.tsv",
        "packet_ready_label_sheet": out_dir / "matched_contrast_packet_ready_label_sheet.tsv",
        "manifest_post_label_only": out_dir / "matched_contrast_manifest_post_label_only.jsonl",
        "selected_candidates_internal": out_dir / "selected_candidates_internal.jsonl",
        "pair_summary": out_dir / "pair_summary.csv",
        "stratum_summary": out_dir / "stratum_summary.csv",
        "asset_request_plan": out_dir / "asset_request_plan.jsonl",
        "label_surface_leakage_hits": out_dir / "label_surface_leakage_hits.jsonl",
        "packet_path_errors": out_dir / "packet_path_errors.jsonl",
        "input_validation_errors": out_dir / "input_validation_errors.jsonl",
        "sampling_contract": out_dir / "sampling_contract.json",
        "v4_label_schema": out_dir / "v4_label_schema.json",
    }
    ...
    boundary = {
        ...
        "contrast_roles_visible_to_labeler": False,
    }
    counts = {
        "label_rows": len(visible_rows),
        "contrast_pairs": len({row.get("matched_contrast_pair_id") for row in seeds}),
        ...
        "family_counts": dict(Counter(str(row.get("predicate_family")) for row in seeds)),
        "hidden_role_counts": dict(Counter(str(row.get("matched_contrast_role_hidden")) for row in seeds)),
        "rank_band_counts_hidden": dict(Counter(str(row.get("rank_band_hidden")) for row in seeds)),
        "geometry_status_counts_hidden": dict(Counter(str(row.get("geometry_status_hidden")) for row in seeds)),
        "label_match_status_counts_hidden": dict(Counter(str(row.get("label_match_status_hidden")) for row in seeds)),
    }
    validation = {
        "input_validation_errors": len(input_errors),
        "label_surface_leakage_hits": len(leakage),
    ...
    if input_errors or leakage or packet_errors:
        status = "h002_reliability_target_v4_matched_contrast_candidate_mining_ready_with_validation_warnings"
    ...
    summary = {
        ...
        "selected_matching_level": sampling_contract.get("selected_matching_level"),
        "selected_matching_keys": sampling_contract.get("selected_matching_keys"),
        "rank_control_policy": sampling_contract.get("rank_control_policy"),
        "label_schema_version": SCHEMA_VERSION,
        "counts": counts,
        "validation": validation,
        "input_paths": {
            "plan_summary": rel_path(args.plan_summary),
            "sampling_contract": rel_path(args.sampling_contract),
        ...
        "next_todo": "reliability_target_v4_matched_contrast_asset_packets",
        "posterior_reopen_gate": sampling_contract.get("posterior_reopen_gate"),
    }
    ...
    write_csv(output_paths["stratum_summary"], stratum_rows)
    write_jsonl(output_paths["asset_request_plan"], asset_requests)
    write_jsonl(output_paths["label_surface_leakage_hits"], leakage)
    write_jsonl(output_paths["packet_path_errors"], packet_errors)
    write_jsonl(output_paths["input_validation_errors"], input_errors)
    write_json(output_paths["sampling_contract"], sampling_contract)
    write_json(output_paths["v4_label_schema"], label_schema())
