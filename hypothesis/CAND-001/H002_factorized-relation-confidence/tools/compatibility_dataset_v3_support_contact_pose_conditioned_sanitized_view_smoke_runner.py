#!/usr/bin/env python3
"""Run train-only learned smoke for support/contact pose-conditioned compatibility."""
...
EXPECTED_PLAN_STATUS = "h002_compatibility_dataset_v3_support_contact_pose_conditioned_sanitized_view_smoke_plan_ready"
EXPECTED_PLAN_NEXT = "compatibility_dataset_v3_support_contact_pose_conditioned_sanitized_view_smoke_runner"
EXPECTED_ROW_SCHEMA = "h002_support_contact_pose_conditioned_smoke_ready_view_v1"

SCHEMA_VERSION = "h002_compatibility_dataset_v3_support_contact_pose_conditioned_sanitized_view_smoke_runner_v1"
STATUS_ERRORS = "h002_compatibility_dataset_v3_support_contact_pose_conditioned_sanitized_view_smoke_runner_input_errors"
STATUS_PASSED = "h002_compatibility_dataset_v3_support_contact_pose_conditioned_sanitized_view_smoke_runner_passed_controls"
STATUS_DIAGNOSTIC = "h002_compatibility_dataset_v3_support_contact_pose_conditioned_sanitized_view_smoke_runner_diagnostic_only_failed_controls"
NEXT_TODO_PASSED = "compatibility_dataset_v3_support_contact_pose_conditioned_result_review"
NEXT_TODO_DIAGNOSTIC = "compatibility_dataset_v3_support_contact_pose_conditioned_failure_analysis"
...
def normalize_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        item["T_e"] = dict(block(row, "T_e"))
        item["Z_e_safe"] = dict(block(row, "Z_e_safe"))
        item["G_e_mesh_pose_contact"] = dict(block(row, "G_e_mesh_pose_contact"))
        item["Q_e_safe"] = dict(block(row, "Q_e_safe"))
...
def t_features_from_dict(t: dict[str, Any]) -> dict[str, float]:
    ...
    out.update(one_hot("T.subject_object", f"{t.get('subject_class_label')}|{t.get('object_class_label')}"))
    return out
...
def z_features(row: dict[str, Any]) -> dict[str, float]:
    z = row.get("Z_e_safe", {})
    return {
        "Z.source_score_available": 1.0 if z.get("source_score_available") else 0.0,
        "Z.source_rank_available": 1.0 if z.get("source_rank_available") else 0.0,
    }
...
def q_features(row: dict[str, Any]) -> dict[str, float]:
    q = row.get("Q_e_safe", {})
    return {
        "Q.aligned_ply_point_features_available": 1.0 if q.get("aligned_ply_point_features_available") else 0.0,
        "Q.hard_surface_pair_allowed": 1.0 if q.get("hard_surface_pair_allowed") else 0.0,
        "Q.point_feature_complete": 1.0 if q.get("point_feature_complete") else 0.0,
        "Q.semseg_obb_available": 1.0 if q.get("semseg_obb_available") else 0.0,
    }
...
def pose_features(g: dict[str, Any]) -> dict[str, float]:
    upness = safe_float(g.get("subject_major_axis_upness"), 0.0)
    ...
    point_contact = safe_float(g.get("point_contact_candidate_ratio_optional"), 0.0)
    point_contact_missing = 1.0 if g.get("point_contact_candidate_ratio_optional") is None else 0.0
    return {
        "pose.upness": upness,
        "pose.lyingness_one_minus_upness": 1.0 - upness,
        "pose.vertical_extent": vertical_extent,
        "pose.flatness": flatness,
        "pose.overlap": overlap,
        "pose.contact": contact,
        "pose.inverse_gap": 1.0 / (1.0 + max(gap, 0.0)),
        "pose.point_contact": point_contact,
        "pose.point_contact_missing": point_contact_missing,
    }
...
def compatibility_interactions_from_dict(t: dict[str, Any], g: dict[str, Any]) -> dict[str, float]:
    ...
    out["C.lying_pose_proxy"] = is_lying * (
        pose["pose.lyingness_one_minus_upness"] + pose["pose.flatness"]
    )
    out["C.standing_pose_proxy"] = is_standing * (
        pose["pose.upness"] + pose["pose.vertical_extent"]
    )
    out["C.shared_contact_support"] = pose["pose.contact"] + pose["pose.overlap"] + pose["pose.inverse_gap"]
    return out
...
def model_feature_fns(rows: list[dict[str, Any]]) -> dict[str, tuple[FeatureFn, FeatureFn]]:
    ...
    return {
        ...
        "M6_factorized_sanitized_TZGQ_pose_interaction": (factorized_features, factorized_features),
...
def count_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row, label in zip(rows, y):
        ...
        point_complete += int(bool(row.get("Q_e_safe", {}).get("point_feature_complete")))
        semseg_complete += int(bool(row.get("Q_e_safe", {}).get("semseg_obb_available")))
    return {
        "groups": len(groups),
...
def validate(
    ...
    for raw, row in zip(raw_rows, rows):
        ...
        if set(raw.get("feature_blocks", {})) != {"T_e", "Z_e_safe", "G_e_mesh_pose_contact", "Q_e_safe"}:
    ...
    for group_id, group_rows in by_group.items():
        ...
        if predicates != ["lying on", "standing on"]:
            errors.append({"error_type": "group_predicates_not_lying_standing", "group_id": group_id, "predicates": predicates})
        g_values = {json.dumps(row.get("G_e_mesh_pose_contact", {}), sort_keys=True) for row in group_rows}
        if len(g_values) != 1:
...
def gate_summary(metrics: dict[str, Any], drops: dict[str, Any], errors: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    m6 = value_or_zero(metrics, "M6_factorized_sanitized_TZGQ_pose_interaction")
    ...
    return {
        "factorized_not_primary": {
            "M6_factorized_sanitized_TZGQ_pose_interaction": m6,
            "interpretation": "M6 is an ablation because this smoke primarily tests C_e, not final p_rel/p_obs.",
        },
        "gate_data_integrity": {
            "pass": gate_data,
        ...
        "overall_interpretation": (
            "support_contact_pose_conditioned_smoke_passed_controls"
            if overall
            else "support_contact_pose_conditioned_smoke_diagnostic_only_failed_controls"
        ),
...
def write_report(path: Path, summary: dict[str, Any], metrics: dict[str, Any], gates: dict[str, Any]) -> None:
    ...
    lines = [
        "# H002 Support/Contact Pose-Conditioned Sanitized View Smoke Runner",
        "",
        "## Status",
        "",
        "```text",
    ...
    lines.extend(
        [
            ...
            "## Interpretation",
            "",
            "This smoke tests whether the same predicate-independent support/contact `G_e` is",
            "interpreted differently when paired with `lying on` or `standing on` semantic content.",
            "The primary result is `M5b_compatibility_TG_pose_interaction`; `M6` is a factorized",
            "ablation and not the final `p_rel/p_obs` claim.",
            "",
...
def main() -> int:
    ...
    metrics_by_pair_text = group_metrics(
        ...
        y,
        predictions,
        lambda row: f"{row.get('T_e', {}).get('subject_class_label')}|{row.get('T_e', {}).get('object_class_label')}",
    )
