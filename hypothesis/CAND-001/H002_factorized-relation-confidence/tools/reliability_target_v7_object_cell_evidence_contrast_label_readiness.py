#!/usr/bin/env python3
"""Validate H002 v7 object-cell evidence-contrast label readiness."""
...
GAP_DIR = RGA_ROOT / "reliability_target_v7_object_cell_evidence_contrast_replacement_asset_packet_gap_audit_codex_proxy_user_requested"
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v7_object_cell_evidence_contrast_label_readiness_codex_proxy_user_requested"

SCHEMA_VERSION = "h002_reliability_target_v7_object_cell_evidence_contrast_label_readiness_v1"
EXPECTED_GAP_STATUS = "h002_reliability_target_v7_object_cell_evidence_contrast_replacement_asset_packet_gap_audit_ready_for_label_readiness"
EXPECTED_NEXT_TODO = "reliability_target_v7_object_cell_evidence_contrast_label_readiness"
EXPECTED_COLUMNS = replacement_packets.FINAL_VISIBLE_FIELDS
REVIEW_FIELDS = replacement_packets.mining.COMPLETION_FIELDS
ALLOWED_PACKET_STATUSES = {"ready", "limited_view_evaluable"}
ALLOWED_GAP_DECISIONS = {"label_ready", "limited_view_evaluable"}
...
def forbidden_value_hits(rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    hits: list[dict[str, Any]] = []
    fields = ["review_scope", "multiview_packet", "pointcloud_or_mesh_packet", "contact_or_context_sheet"]
...
def validate_inputs(
    ...
    if gap_summary.get("next_todo") != EXPECTED_NEXT_TODO:
    ...
    for key in ["validation_usage", "test_usage", "posterior_trained", "posterior_smoke_allowed", "h001_artifacts_modified"]:
        if boundary.get(key) is not False:
            errors.append({"error_type": f"gap_boundary_{key}_not_false", "actual": boundary.get(key)})
...
def validate_label_sheet(
    ...
    for row_number, row in enumerate(label_rows, start=2):
        ...
        if status == "ready" and decision != "label_ready":
            errors.append({"error_type": "ready_status_has_non_ready_gap_decision", "row_number": row_number, "blind_review_id": row.get("blind_review_id", ""), "decision": decision})
        if status == "limited_view_evaluable" and decision == "label_ready":
            errors.append({"error_type": "limited_status_has_label_ready_decision", "row_number": row_number, "blind_review_id": row.get("blind_review_id", "")})
        ...
        hidden_audit_rows.append(
            {
                ...
                "strict_group_key_hidden_present": "strict_group_key_hidden" in manifest,
            }
        )
    ...
    manifest_counter = Counter(
        (
            ...
            str(row.get("normalized_evidence_status_hidden")),
...
def label_schema() -> dict[str, Any]:
    return {
        ...
        "target_after_label_fill": "relation_reliability_state_v7_multiclass_target",
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# H002 V7 Object-Cell Evidence Contrast Label Readiness",
        "",
        ...
        "- Semantic/geometry score, bucket, rank, and object-cell metadata remain hidden from the label sheet.",
        "",
        "## Status",
        "",
        "```text",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    passes_contract = (
        ...
        and packet_status_counts == Counter({"ready": 215, "limited_view_evaluable": 25})
        and gap_decision_counts == Counter({"label_ready": 215, "limited_view_evaluable": 25})
    ...
    status = (
        ...
        if passes_contract
        else "h002_reliability_target_v7_object_cell_evidence_contrast_label_readiness_blocked"
    )
    next_todo = (
        "reliability_target_v7_object_cell_evidence_contrast_label_fill"
        if passes_contract
        else "fix_reliability_target_v7_object_cell_evidence_contrast_label_readiness"
    )
    decision = (
        "The restored 240-row v7 object-cell evidence-contrast sheet is ready for label fill with explicit limited-view caveats."
        if passes_contract
        else "The v7 label-ready sheet is blocked by validation errors."
    )
