#!/usr/bin/env python3
"""Audit schema leakage and shortcut risk for relative-horizontal materialized rows."""
...
EXPECTED_INPUT_NEXT = "compatibility_dataset_v3_relative_horizontal_schema_shortcut_audit_after_materialization"

SCHEMA_VERSION = "h002_compatibility_dataset_v3_relative_horizontal_schema_shortcut_audit_after_materialization_v1"
SMOKE_READY_SCHEMA = "h002_relative_horizontal_smoke_ready_view_v1"
STATUS_READY = (
    "h002_compatibility_dataset_v3_relative_horizontal_schema_shortcut_audit_after_materialization_ready_for_smoke_plan"
)
STATUS_BLOCKED = (
    "h002_compatibility_dataset_v3_relative_horizontal_schema_shortcut_audit_after_materialization_blocked_shortcut_risk"
)
STATUS_ERROR = "h002_compatibility_dataset_v3_relative_horizontal_schema_shortcut_audit_after_materialization_input_errors"
SELECTED_READY = "relative_horizontal_smoke_ready_view_ready"
SELECTED_BLOCKED = "relative_horizontal_blocked_by_schema_or_shortcut_risk"
NEXT_READY = "compatibility_dataset_v3_relative_horizontal_sanitized_view_smoke_plan_after_schema_audit"
NEXT_BLOCKED = "compatibility_dataset_v3_relative_horizontal_path_decision_after_schema_shortcut_audit"
...
INTENDED_INTERACTION_PROBES = {
    "TG_exact_interaction",
    "TG_signed_rule_interaction",
}
HIDDEN_CONSTRUCTION_PROBES = {
    "hidden_candidate_predicate_only",
    "hidden_original_gt_anchor_flag",
    "hidden_selected_frame_compatible",
    "hidden_source_predicate_x_candidate_predicate",
    "hidden_axis_bucket_x_candidate_predicate",
    "hidden_axis_bucket_y_candidate_predicate",
    "hidden_selected_axis_bucket_x_candidate_predicate",
}
    ...
    audit_rows = [
        {
            ...
            "rows": count,
            "allowed": path in ALLOWED_FEATURE_PATHS,
            "blocked_fragment_hits": ";".join(
                fragment for fragment in BLOCKED_FEATURE_FRAGMENTS if fragment in path.lower()
            ),
        }
...
def primary_rows(
    model_rows: list[dict[str, Any]], hidden_rows: list[dict[str, Any]]
    ...
    for row in model_rows:
        ...
        if label not in (0, 1):
            continue
        hidden_row = hidden.get(row["row_id"])
        if hidden_row is None:
            errors.append({"error_type": "missing_hidden_row", "row_id": row["row_id"]})
            continue
        rows.append(row)
        hidden_for_rows.append(hidden_row)
        labels.append(int(label))
    return rows, hidden_for_rows, labels, errors


def geometry_tuple(row: dict[str, Any]) -> tuple[Any, ...]:
    g = row.get("feature_blocks", {}).get("G_e_horizontal", {})
    return (
        value_key(g.get("delta_x_subject_minus_object")),
        value_key(g.get("delta_y_subject_minus_object")),
        value_key(g.get("horizontal_distance")),
    )


def signed_rule_key(row: dict[str, Any]) -> tuple[Any, ...]:
    g = row.get("feature_blocks", {}).get("G_e_horizontal", {})
    predicate = nested_get(row, "feature_blocks.T_e.predicate_label")
    dx = safe_float(g.get("delta_x_subject_minus_object"))
    dy = safe_float(g.get("delta_y_subject_minus_object"))
    x_sign = "positive" if dx is not None and dx > 0 else "negative" if dx is not None and dx < 0 else "zero"
    y_sign = "positive" if dy is not None and dy > 0 else "negative" if dy is not None and dy < 0 else "zero"
    return (predicate, x_sign, y_sign)
...
def build_shortcut_probes(model_rows: list[dict[str, Any]], labels: list[int]) -> list[dict[str, Any]]:
    ...
    probes.append(
        categorical_probe(
            ...
            "Predicate alone should be balanced across positive/negative rows.",
        )
    )
    probes.append(
        categorical_probe(
    ...
    for field in [
        ...
        probes.append(
            numeric_probe(
                ...
                "model_safe_main_view",
                True,
                lambda row, field=field: nested_get(row, f"feature_blocks.G_e_horizontal.{field}"),
                "A single continuous geometry field should be near chance under same-G predicate flip.",
            )
        )
    probes.append(
        categorical_probe(
            ...
            "model_safe_main_view",
            True,
            lambda row: (nested_get(row, "feature_blocks.T_e.predicate_label"), geometry_tuple(row)),
    ...
    probes.append(
        categorical_probe(
            ...
            True,
            signed_rule_key,
            "This is the intended signed horizontal compatibility rule and should be tested with wrong-frame controls later.",
        )
    )
    return probes


def build_hidden_probes(
    model_rows: list[dict[str, Any]], hidden_rows: list[dict[str, Any]], labels: list[int]
) -> list[dict[str, Any]]:
    joined = [{"model": model, "hidden": hidden} for model, hidden in zip(model_rows, hidden_rows)]
    probes: list[dict[str, Any]] = []
    for name, fn, interpretation in [
        ("hidden_axis_pair_only", lambda row: row["hidden"].get("axis_pair"), "Axis-pair alone should be balanced."),
        ("hidden_class_pair_only", lambda row: row["hidden"].get("class_pair"), "Class-pair hidden probe checks object-semantics memorization risk."),
        ...
        ("hidden_candidate_predicate_only", lambda row: row["hidden"].get("candidate_predicate_label"), "Candidate predicate duplicates T_e and is balanced."),
        ("hidden_selected_axis_bucket_only", lambda row: row["hidden"].get("selected_axis_bucket"), "Selected-axis sign alone should be balanced within predicate flips."),
        ("hidden_x_axis_bucket_only", lambda row: row["hidden"].get("x_axis_bucket"), "X sign alone should be balanced within predicate flips."),
        ("hidden_y_axis_bucket_only", lambda row: row["hidden"].get("y_axis_bucket"), "Y sign alone should be balanced within predicate flips."),
        ("hidden_original_gt_anchor_flag", lambda row: row["hidden"].get("is_original_gt_anchor"), "Original/counterfactual flag is construction metadata and must stay hidden."),
        ("hidden_selected_frame_compatible", lambda row: row["hidden"].get("selected_frame_compatible"), "Selected-frame compatibility is the target proxy and must stay hidden."),
        (
            "hidden_source_predicate_x_candidate_predicate",
            lambda row: (row["hidden"].get("source_predicate_label"), row["hidden"].get("candidate_predicate_label")),
            "Construction-rule proxy, expected high but hidden.",
        ),
        (
            "hidden_axis_bucket_x_candidate_predicate",
            lambda row: (row["hidden"].get("x_axis_bucket"), row["hidden"].get("candidate_predicate_label")),
            "Axis-bucket plus predicate is a discretized rule proxy and must stay hidden.",
        ),
        (
            "hidden_axis_bucket_y_candidate_predicate",
            lambda row: (row["hidden"].get("y_axis_bucket"), row["hidden"].get("candidate_predicate_label")),
            "Axis-bucket plus predicate is a discretized rule proxy and must stay hidden.",
        ),
        (
            "hidden_selected_axis_bucket_x_candidate_predicate",
            lambda row: (row["hidden"].get("selected_axis_bucket"), row["hidden"].get("candidate_predicate_label")),
            "Selected-axis bucket plus predicate is a label proxy and must stay hidden.",
        ),
    ]:
...
def group_integrity_audit(
    model_rows: list[dict[str, Any]], group_rows: list[dict[str, Any]]
    ...
    for group in group_rows:
        ...
        axis_pair = group.get("axis_pair")
        expected_predicates = expected_by_axis.get(str(axis_pair), [])
        passed = len(rows) == 2 and predicates == expected_predicates and labels == [0, 1] and len(g_tuples) == 1
        audit = {
        ...
        output.append(
            {
                ...
                "model_use": "relative_horizontal_smoke_ready_if_plan_accepts",
            }
        )
    return output
...
def validation_from_audit(
    ...
    for row in shortcut_rows:
        if row["probe_name"] in INTENDED_INTERACTION_PROBES:
            continue
    ...
    nonconstruction_hidden_high = [row for row in hidden_high if row["probe_name"] not in HIDDEN_CONSTRUCTION_PROBES]
...
def build_report(summary: dict[str, Any], shortcut_rows: list[dict[str, Any]], hidden_rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        "# H002 Relative-Horizontal Schema Shortcut Audit After Materialization",
        "",
        "## Result",
        "",
        "```text",
    ...
    lines += [
        ...
        "## Interpretation",
        "",
        "- Schema leakage passes only if blocked feature paths are absent from model-safe rows.",
        "- T-only and G-only probes should stay near chance.",
        "- `TG_exact_interaction` and `TG_signed_rule_interaction` are intended high signals.",
        "- Hidden construction proxies can be high only if they remain outside model-safe features.",
...
def main() -> None:
    ...
    model_rows, hidden_rows_for_primary, labels, join_errors = primary_rows(model_rows_raw, hidden_manifest_rows)
    initial_errors.extend(join_errors)

    feature_audit_rows, feature_violations = feature_path_audit(model_rows)
    shortcut_rows = build_shortcut_probes(model_rows, labels)
    hidden_probe_rows = build_hidden_probes(model_rows, hidden_rows_for_primary, labels)
    ...
    if initial_errors:
        ...
        selected_path = SELECTED_BLOCKED
    ...
    summary = {
        ...
        "gate": {
            ...
            "allowed_single_feature_pass": not any(
                row["probe_name"] not in INTENDED_INTERACTION_PROBES
