#!/usr/bin/env python3
"""Validate v4 matched-contrast label readiness before label fill."""
...
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v4_matched_contrast_label_readiness"

SCHEMA_VERSION = "h002_reliability_target_v4_matched_contrast_label_readiness_v1"
...
def forbidden_value_hits(rows: list[dict[str, str]], fields: list[str], tokens: list[str]) -> list[dict[str, Any]]:
...
def validate_inputs(gap_summary: dict[str, Any], label_rows: list[dict[str, str]], manifest_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    errors: list[dict[str, Any]] = []
    if gap_summary.get("status") != "h002_reliability_target_v4_matched_contrast_asset_packet_gap_audit_ready_for_label_readiness":
        errors.append({"error_type": "unexpected_gap_status", "value": gap_summary.get("status")})
    if gap_summary.get("next_todo") != "reliability_target_v4_matched_contrast_label_readiness":
...
def validate_label_sheet(
    ...
    duplicate_ids = [blind_id for blind_id, count in Counter(ids).items() if count > 1]
    ...
    for manifest in manifest_rows:
        pair_id = str(manifest.get("matched_contrast_pair_id_hidden"))
        if pair_id in excluded_pair_ids:
            errors.append({"error_type": "excluded_pair_present", "matched_contrast_pair_id_hidden": pair_id})
        pair_roles[pair_id][str(manifest.get("matched_contrast_role_hidden"))] += 1
        pair_rows[pair_id].append(str(manifest.get("blind_review_id")))
        family_counts[str(manifest.get("predicate_family"))] += 1
    ...
    for pair_id, roles in sorted(pair_roles.items()):
        if roles.get("positive_proxy", 0) != 1 or roles.get("negative_proxy", 0) != 1 or len(pair_rows[pair_id]) != 2:
            errors.append(
                {
                    "error_type": "invalid_matched_pair_role_counts",
    ...
    visible_hits = forbidden_header_hits(fieldnames)
    visible_hits.extend(
    ...
    packet_hits = packet_text_hits(label_rows, base_dir)
    pair_summary = [
        {
            "matched_contrast_pair_id_hidden": pair_id,
            "row_count": len(pair_rows[pair_id]),
    ...
    return errors, path_errors, visible_hits + packet_hits, pair_summary
...
def label_schema() -> dict[str, Any]:
    return {
        ...
        "target_after_label_fill": "relation_reliability_v4_binary_target",
        ...
        "limited_view_policy": {
            ...
            "excluded_pairs": "excluded before label fill to preserve matched-pair integrity",
        },
    }
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    role_counts = Counter(str(row.get("matched_contrast_role_hidden")) for row in manifest_rows)
    ...
    pair_count = len({row["matched_contrast_pair_id_hidden"] for row in manifest_rows})
    ...
    status = (
        ...
        and pair_count == 79
        and role_counts.get("positive_proxy", 0) == 79
        and role_counts.get("negative_proxy", 0) == 79
        else "h002_reliability_target_v4_matched_contrast_label_readiness_blocked"
    )
    next_todo = (
        "reliability_target_v4_matched_contrast_label_fill"
        if status == "h002_reliability_target_v4_matched_contrast_label_readiness_ready_for_label_fill"
        else "fix_reliability_target_v4_matched_contrast_label_readiness"
    )
    decision = (
        "The 158-row / 79-pair v4 matched-contrast sheet is ready for label fill."
        if status == "h002_reliability_target_v4_matched_contrast_label_readiness_ready_for_label_fill"
        else "The v4 matched-contrast label-ready sheet is blocked by validation errors."
    )
