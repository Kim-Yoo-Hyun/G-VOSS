#!/usr/bin/env python3
"""Audit v6 shortcut-controlled partial asset packets before label fill."""
...
PACKET_DIR = RGA_ROOT / "reliability_target_v6_shortcut_controlled_asset_packets_codex_proxy_user_requested"
DEFAULT_PACKET_SUMMARY = PACKET_DIR / "summary.json"
DEFAULT_FULL_LABEL_SHEET = PACKET_DIR / "v6_shortcut_controlled_full_label_sheet.tsv"
DEFAULT_FULL_MANIFEST = PACKET_DIR / "v6_shortcut_controlled_full_manifest_post_label_only.jsonl"
DEFAULT_GENERATED_PACKET_MANIFEST = PACKET_DIR / "generated_packet_manifest.jsonl"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v6_shortcut_controlled_asset_packet_gap_audit_codex_proxy_user_requested"

SCHEMA_VERSION = "h002_reliability_target_v6_shortcut_controlled_asset_packet_gap_audit_v1"
...
def synthetic_ready_packet(manifest_row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "packet_status": manifest_row.get("evidence_packet_status") or "ready",
        "subject_image_count": 1,
        "object_image_count": 1,
        "contact_sheet_ready": True,
        "mesh_packet_ready": True,
        "multiview_packet": (manifest_row.get("packet_paths") or {}).get("multiview_packet", ""),
        "pointcloud_or_mesh_packet": (manifest_row.get("packet_paths") or {}).get("pointcloud_or_mesh_packet", ""),
        "contact_or_context_sheet": (manifest_row.get("packet_paths") or {}).get("contact_or_context_sheet", ""),
    }
...
def decide_row(packet_row: dict[str, Any], manifest_row: dict[str, Any]) -> dict[str, Any]:
    ...
    elif mesh_ready and family in {"support_contact", "relative_vertical"}:
        decision = "geometry_only_evaluable"
        reason = "mesh packet is available for the geometric relation family, but multiview/contact evidence is incomplete"
        normalized_status = "limited_view_evaluable"
    else:
    ...
    return {
        ...
        "candidate_bucket_hidden": manifest_row.get("candidate_bucket_hidden"),
    }
...
def materialize_packet(row: dict[str, Any], decision: dict[str, Any], output_dir: Path, packet_dir: Path) -> dict[str, str]:
    ...
    if "contact_or_context_sheet" not in output_paths and decision["normalized_evidence_status"] == "limited_view_evaluable":
...
def label_sheet_rows(
    ...
    for row in label_rows:
        ...
        if decision["row_gap_decision"] == "replacement_needed":
            ...
            continue
        updated.update(materialize_packet(updated, decision, output_dir, packet_dir))
...
def validate_inputs(packet_summary: dict[str, Any], label_rows: list[dict[str, Any]], manifests: list[dict[str, Any]]) -> list[dict[str, Any]]:
    errors: list[dict[str, Any]] = []
    if packet_summary.get("next_todo") != "reliability_target_v6_shortcut_controlled_asset_packet_gap_audit":
        errors.append({"error": "unexpected_next_todo", "value": packet_summary.get("next_todo")})
    if packet_summary.get("status") != "h002_reliability_target_v6_shortcut_controlled_asset_packets_partial_needs_gap_audit":
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# H002 Reliability Target V6 Shortcut-Controlled Asset Packet Gap Audit",
        "",
        ...
        "- This decides packet usability before v6 label readiness.",
        "- Limited-view rows are kept only when mesh evidence is available.",
        "",
        "## Status",
        "",
        f"`{summary['status']}`",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    label_ready_family_counts = Counter(str(row.get("predicate_family")) for row in ready_manifest_rows)
    label_ready_bucket_counts = Counter(str(row.get("candidate_bucket_hidden")) for row in ready_manifest_rows)
    label_ready_family_bucket_counts = Counter(
        f"{row.get('predicate_family')}|{row.get('candidate_bucket_hidden')}" for row in ready_manifest_rows
    )
    replacement_requests = [
        {
            "replacement_request_reason": "v6_shortcut_controlled_row_excluded_by_asset_gap_audit",
            "blind_review_id": row.get("blind_review_id"),
    ...
    status = (
        ...
        if len(ready_label_rows) == len(label_rows)
        and not output_errors
        and not leakage_hits
        and not input_errors
        else "h002_reliability_target_v6_shortcut_controlled_asset_packet_gap_audit_needs_replacement"
    )
    next_todo = (
        "reliability_target_v6_shortcut_controlled_label_readiness"
        if status == "h002_reliability_target_v6_shortcut_controlled_asset_packet_gap_audit_ready_for_label_readiness"
        else "reliability_target_v6_shortcut_controlled_replacement_mining"
    )
    decision = (
        "Proceed to label readiness with the full 240-row queue; partial rows are kept with explicit limited-view evidence caveats."
        if status == "h002_reliability_target_v6_shortcut_controlled_asset_packet_gap_audit_ready_for_label_readiness"
        else "Do not proceed to label fill until replacement rows are mined."
    )
    ...
    summary = {
        ...
        "counts": {
            ...
            "ready_family_counts": dict(sorted(label_ready_family_counts.items())),
            "ready_bucket_counts": dict(sorted(label_ready_bucket_counts.items())),
            "ready_family_bucket_counts": dict(sorted(label_ready_family_bucket_counts.items())),
        },
