#!/usr/bin/env python3
"""Ingest H002 v14 physical relation-family labels and run quick target probes."""
...
FILL_DIR = RGA_ROOT / "reliability_target_v14_physical_relation_family_label_fill"
CANDIDATE_DIR = RGA_ROOT / "reliability_target_v14_physical_relation_family_candidate_mining"

DEFAULT_FILL_SUMMARY = FILL_DIR / "summary.json"
DEFAULT_FILLED_SHEET = FILL_DIR / "filled_label_sheet_v14.tsv"
DEFAULT_HIDDEN_MANIFEST = CANDIDATE_DIR / "hidden_audit_manifest_v14.jsonl"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v14_physical_relation_family_label_ingestion"

SCHEMA_VERSION = "h002_reliability_target_v14_physical_relation_family_label_ingestion_v1"
EXPECTED_FILL_STATUS = "h002_reliability_target_v14_physical_relation_family_label_filled_codex_proxy_visible_only"
EXPECTED_FILL_NEXT_TODO = "reliability_target_v14_physical_relation_family_label_ingestion"
NEXT_TODO = "reliability_target_v14_physical_relation_family_target_independence_audit"

LABEL_SOURCE = "codex_proxy_v14_physical_relation_visible_only_user_requested"
MIN_CLASS_MASS_FOR_POSTERIOR = 50

MULTICLASS_TARGET = "physical_relation_family_reliability_v14_multiclass"
BINARY_TARGET = "physical_relation_family_reliability_v14_binary"
GEOMETRY_SUPPORT_TARGET = "physical_relation_family_geometry_support_v14_binary"
USEFULNESS_TARGET = "physical_relation_family_usefulness_v14_binary"
ENDPOINT_TARGET = "physical_relation_family_endpoint_identity_v14_multiclass"
COVERAGE_TARGET = "physical_relation_family_coverage_v14_multiclass"
...
HIDDEN_AUDIT_FIELDS = [
    ...
    "fallback_applied_hidden",
    "any_hard_endpoint_hidden",
    "floor_as_object_hidden",
...
ALLOWED_PREDICATES = {"lying on", "standing on", "lower than"}
ALLOWED_PREDICATE_FAMILIES = {"support_contact", "relative_vertical"}
ALLOWED_RELIABILITY_STATES = {"accept_reliable", "reject_unreliable", "abstain_uncertain"}
ALLOWED_GEOMETRY_SUPPORT_STATES = {"supports", "contradicts", "ambiguous", "not_evaluable"}
ALLOWED_USEFULNESS_STATES = {"useful_nontrivial", "trivial_or_redundant", "not_a_relation", "uncertain"}
ALLOWED_ENDPOINT_STATES = {"clear", "uncertain", "wrong_endpoint", "not_evaluable"}
...
def validate_label_rows(fieldnames: list[str], rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        reliability_state = row.get("relation_reliability_state_v14", "")
        geometry_state = row.get("geometry_support_state_v14", "")
        usefulness_state = row.get("relation_usefulness_state_v14", "")
        endpoint_state = row.get("endpoint_identity_state_v14", "")
        coverage_state = row.get("coverage_state_v14", "")
        ...
        if row.get("relation_family_visible") not in {"support/contact relation", "relative vertical relation"}:
            errors.append({"error_type": "unexpected_visible_family", "row_number": row_number, "blind_review_id": blind_id, "family": row.get("relation_family_visible")})
        for field in COMPLETION_FIELDS:
            if not str(row.get(field, "")).strip() and field != "uncertainty_reason_v14":
...
def validate_manifest_rows(manifest_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row_number, row in enumerate(manifest_rows, start=1):
        ...
        if row.get("predicate_family") not in ALLOWED_PREDICATE_FAMILIES:
        ...
        if row.get("source_queue_hidden") not in {"RGA-HL", "RGA-LH"}:
        ...
        if row.get("any_hard_endpoint_hidden") is not False:
            errors.append({"error_type": "hard_endpoint_present", "row_number": row_number, "blind_review_id": blind_id, "actual": row.get("any_hard_endpoint_hidden")})
        if row.get("floor_as_object_hidden") is not False:
            errors.append({"error_type": "floor_as_object_present", "row_number": row_number, "blind_review_id": blind_id, "actual": row.get("floor_as_object_hidden")})
    return errors
...
def joined_rows(label_rows: list[dict[str, str]], manifest_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for label_row in label_rows:
        ...
        manifest = dict(manifest_by_id[blind_id])
        manifest.setdefault("p_geom_bin_hidden", p_geom_bin(manifest.get("p_geom_valid_hidden")))
        subject_label = label_row.get("subject_label", "")
        object_label = label_row.get("object_label", "")
        reliability_state = label_row["relation_reliability_state_v14"]
        geometry_support_state = label_row["geometry_support_state_v14"]
        usefulness_state = label_row["relation_usefulness_state_v14"]
        ...
        row = {
            ...
            **{field: label_row.get(field) for field in VISIBLE_CONTEXT_FIELDS},
            "reviewer_id_v14": label_row.get("reviewer_id_v14"),
            "review_round_v14": label_row.get("review_round_v14"),
            "label_policy_v14": label_row.get("label_policy_v14"),
            "relation_reliability_state_v14": reliability_state,
            ...
            "geometry_support_state_v14": geometry_support_state,
            "geometry_support_binary_target": geometry_support_binary,
            "geometry_support_binary_usable": geometry_support_binary is not None,
            "relation_usefulness_state_v14": usefulness_state,
            "relation_usefulness_binary_target": usefulness_binary,
            "relation_usefulness_binary_usable": usefulness_binary is not None,
            "endpoint_identity_state_v14": label_row.get("endpoint_identity_state_v14"),
            "coverage_state_v14": label_row.get("coverage_state_v14"),
            "primary_reason_v14": label_row.get("primary_reason_v14"),
            "uncertainty_reason_v14": label_row.get("uncertainty_reason_v14"),
            "review_notes_v14": label_row.get("review_notes_v14"),
...
def target_record(row: dict[str, Any], target_name: str, target_value: Any) -> dict[str, Any]:
    return {
        ...
        "quota_cell_id_hidden": row.get("quota_cell_id_hidden"),
    }
...
def target_rows(rows: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    ...
    endpoint_rows = [
        target_record(row, ENDPOINT_TARGET, row["endpoint_identity_state_v14"])
        for row in rows
    ]
    coverage_rows = [
        target_record(row, COVERAGE_TARGET, row["coverage_state_v14"])
        for row in rows
    ]
    abstain_rows = [
        {
            **target_record(row, BINARY_TARGET, None),
            "abstain_reason": row["uncertainty_reason_v14"] or row["primary_reason_v14"],
            "relation_reliability_state_v14": row["relation_reliability_state_v14"],
            "geometry_support_state_v14": row["geometry_support_state_v14"],
            "relation_usefulness_state_v14": row["relation_usefulness_state_v14"],
            "endpoint_identity_state_v14": row["endpoint_identity_state_v14"],
            "coverage_state_v14": row["coverage_state_v14"],
        }
        for row in rows
...
def probe_risks(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    label_specs = [
        ...
        (rows, "endpoint_identity_state_v14"),
    ]
...
def group_contrast(rows: list[dict[str, Any]], group_field: str) -> list[dict[str, Any]]:
    ...
    for group_value, group_rows in grouped.items():
        relation_counts = Counter(row["relation_reliability_state_v14"] for row in group_rows)
        ...
        endpoint_values = {row["endpoint_identity_state_v14"] for row in group_rows}
        out.append(
            {
                "group_field": group_field,
                "group_value": group_value,
                "rows": len(group_rows),
                "accept": relation_counts.get("accept_reliable", 0),
                "reject": relation_counts.get("reject_unreliable", 0),
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# H002 V14 Physical Relation-Family Label Ingestion",
        "",
        ...
        "## Result",
        "",
        "Ingested the locked v14 filled labels and joined hidden audit metadata by `blind_review_id`.",
        "",
        "```text",
        f"rows = {counts['rows']}",
        ...
        f"same_quota_cell_mixed_reliability_binary_groups = {viability['same_quota_cell_mixed_reliability_binary_groups']}",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    quota_cell_contrast = group_contrast(rows, "quota_cell_id_hidden")
    ...
    state_counts = Counter(row["relation_reliability_state_v14"] for row in rows)
    geometry_support_counts = Counter(row["geometry_support_state_v14"] for row in rows)
    usefulness_counts = Counter(row["relation_usefulness_state_v14"] for row in rows)
    endpoint_counts = Counter(row["endpoint_identity_state_v14"] for row in rows)
    coverage_counts = Counter(row["coverage_state_v14"] for row in rows)
    ...
    reason_counts = Counter(row["primary_reason_v14"] for row in rows)
    uncertainty_counts = Counter(row["uncertainty_reason_v14"] for row in rows)
    ...
    source_queue_counts = Counter(str(row.get("source_queue_hidden")) for row in rows)
    quota_cell_counts = Counter(str(row.get("quota_cell_id_hidden")) for row in rows)
    ...
    same_quota_mixed = sum(1 for row in quota_cell_contrast if row["mixed_reliability_binary"])
    ...
    output_paths = {
        ...
        "quota_cell_contrast_summary": output_dir / "quota_cell_contrast_summary.csv",
    ...
    if validation_errors:
        status = "h002_reliability_target_v14_physical_relation_family_label_ingestion_errors"
        ...
        status = "h002_reliability_target_v14_physical_relation_family_label_ingested_positive_sparse"
    elif risk_flags:
        status = "h002_reliability_target_v14_physical_relation_family_label_ingested_with_probe_risk"
    else:
        status = "h002_reliability_target_v14_physical_relation_family_label_ingested_ready_for_independence_audit"
    ...
    summary = {
        ...
        "counts": {
            ...
            "relation_reliability_state_v14": dict(state_counts),
            "geometry_support_state_v14": dict(geometry_support_counts),
            ...
            "source_queue_hidden": dict(source_queue_counts),
            "quota_cell_id_hidden": dict(quota_cell_counts),
            ...
            "quota_cell_groups": len(quota_cell_contrast),
        ...
        "target_viability": {
            ...
            "same_quota_cell_mixed_reliability_binary_groups": same_quota_mixed,
    ...
    write_csv(output_paths["quota_cell_contrast_summary"], quota_cell_contrast)
