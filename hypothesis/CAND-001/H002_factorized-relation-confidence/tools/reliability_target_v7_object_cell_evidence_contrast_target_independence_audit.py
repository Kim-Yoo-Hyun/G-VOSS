#!/usr/bin/env python3
"""Audit H002 v7 object-cell evidence-contrast target independence."""
...
DEFAULT_INGESTION_DIR = RGA_ROOT / "reliability_target_v7_object_cell_evidence_contrast_label_ingestion_codex_proxy_user_requested"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v7_object_cell_evidence_contrast_target_independence_audit_codex_proxy_user_requested"
...
TARGET_SCHEMA_VERSIONS = {
    RELIABILITY_MULTICLASS: "h002_reliability_target_v7_object_cell_evidence_contrast_multiclass_row_v1",
    RELIABILITY_BINARY: "h002_reliability_target_v7_object_cell_evidence_contrast_binary_row_v1",
    GEOMETRY_TARGET: "h002_geometry_support_v6_object_cell_evidence_contrast_binary_row_v1",
    USEFULNESS_TARGET: "h002_relation_usefulness_v6_object_cell_evidence_contrast_binary_row_v1",
}
...
SLICE_SPECS = {
    "original_v7": {
        "balanced_keys": [],
        "reason": "full v7 object-cell evidence-contrast target",
        "priority": 99,
    },
    "family_balanced_v7": {
...
def counts_for(rows: list[dict[str, Any]], target_name: str) -> dict[str, Any]:
    ...
    return {
        ...
        "by_semantic_geometry_bucket": dict(sorted(Counter(group_value(row, "semantic_geometry_bucket_hidden") for row in rows).items())),
        ...
        "by_endpoint_pattern": dict(sorted(Counter(group_value(row, "endpoint_pattern_hidden") for row in rows).items())),
        "by_strict_group_key": dict(sorted(Counter(group_value(row, "strict_group_key_hidden") for row in rows).items())),
...
def validate_ingestion_summary(summary: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    expected_status = "h002_reliability_target_v7_object_cell_evidence_contrast_label_ingested_with_probe_risk"
    expected_next = "reliability_target_v7_object_cell_evidence_contrast_target_independence_audit"
...
def per_target_decision(target_name: str, summaries: list[dict[str, Any]], errors: list[dict[str, Any]]) -> dict[str, Any]:
    original = next(item for item in summaries if item["slice_name"] == "original_v7")
    ...
    if errors:
        ...
        decision = "Fix row validation errors before using v7 object-cell evidence-contrast target slices."
        next_step = "fix_reliability_target_v7_object_cell_evidence_contrast_target_independence_errors"
    elif strict:
        ...
        next_step = "reliability_target_v7_object_cell_evidence_contrast_source_feature_join"
    elif diagnostic:
        ...
        next_step = "reliability_target_v7_object_cell_evidence_contrast_path_decision"
...
def global_decision(target_decisions: dict[str, Any], errors: list[dict[str, Any]]) -> tuple[str, str, str]:
    if errors:
        return (
            "h002_reliability_target_v7_object_cell_evidence_contrast_target_independence_audit_errors",
            "Fix v7 target audit validation errors before any target decision.",
            "fix_reliability_target_v7_object_cell_evidence_contrast_target_independence_errors",
        )
    ...
    if multiclass["status"] == "strict_controlled_slice_ready":
        return (
            "h002_reliability_target_v7_object_cell_evidence_contrast_target_independence_audit_multiclass_reliability_ready",
            "The primary multiclass relation reliability target has a strict controlled slice; source feature join may be planned next.",
            "reliability_target_v7_object_cell_evidence_contrast_source_feature_join",
        )
    if binary["status"] == "strict_controlled_slice_ready":
        return (
            "h002_reliability_target_v7_object_cell_evidence_contrast_target_independence_audit_binary_only",
            "Only the binary diagnostic relation reliability target has a strict slice; primary multiclass evidence remains blocked.",
            "reliability_target_v7_object_cell_evidence_contrast_path_decision",
        )
    return (
        "h002_reliability_target_v7_object_cell_evidence_contrast_target_independence_audit_blocked_shortcut_risk",
        (
            "The v7 targets have usable class mass, but no controlled slice clears the main shortcut risks. "
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    lines = [
        "# H002 Reliability Target V7 Object-Cell Evidence-Contrast Target Independence Audit",
        "",
        ...
        "- V7 labels are user-requested Codex proxy labels, not independent human annotation.",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary = {
        "schema_version": "h002_reliability_target_v7_object_cell_evidence_contrast_target_independence_audit_summary_v1",
        "status": status,
