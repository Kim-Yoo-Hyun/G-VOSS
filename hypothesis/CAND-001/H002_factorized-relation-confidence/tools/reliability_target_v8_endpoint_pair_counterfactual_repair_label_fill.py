#!/usr/bin/env python3
"""Fill the restored H002 v8 repair label sheet as a Codex proxy."""
...
DEFAULT_LABEL_PATH_BASE = READINESS_DIR
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v8_endpoint_pair_counterfactual_repair_label_fill_codex_proxy_user_requested"

SCHEMA_VERSION = "h002_reliability_target_v8_endpoint_pair_counterfactual_repair_label_fill_summary_v1"
STATUS_READY = "h002_reliability_target_v8_repair_label_filled_codex_proxy_user_requested"
STATUS_ERROR = "h002_reliability_target_v8_repair_label_fill_errors"
EXPECTED_READINESS_STATUS = "h002_reliability_target_v8_repair_label_readiness_ready_for_label_fill"
EXPECTED_NEXT_TODO = "reliability_target_v8_endpoint_pair_counterfactual_repair_label_fill"
REVIEWER_ID = "(codex_proxy_v8_repair_user_requested_visible_only)"
REVIEW_ROUND = "1"
...
SUPPORT_SURFACES = visible_fill.SUPPORT_SURFACES | {"bathtub", "shower", "books", "book", "stool", "ottoman", "suitcase"}
LYING_LIKE = {
    "blanket",
    "clothes",
    "cushion",
...
def validate_inputs(
    ...
    if readiness_summary.get("status") != EXPECTED_READINESS_STATUS:
        errors.append({"error_type": "unexpected_readiness_status", "expected": EXPECTED_READINESS_STATUS, "actual": readiness_summary.get("status")})
    if readiness_summary.get("next_todo") != EXPECTED_NEXT_TODO:
        errors.append({"error_type": "unexpected_readiness_next_todo", "expected": EXPECTED_NEXT_TODO, "actual": readiness_summary.get("next_todo")})
    ...
    for key in ["validation_usage", "test_usage", "posterior_trained", "posterior_smoke_allowed", "multi_view_as_model_input", "h001_artifacts_modified"]:
        if boundary.get(key) is not False:
            errors.append({"error_type": f"readiness_boundary_{key}_not_false", "actual": boundary.get(key)})
    ...
    if set(visible_ids) != set(manifest_ids):
        errors.append({"error_type": "visible_manifest_id_mismatch", "visible_only": sorted(set(visible_ids) - set(manifest_ids))[:10], "manifest_only": sorted(set(manifest_ids) - set(visible_ids))[:10]})
    for row_number, row in enumerate(rows, start=2):
        for field in REVIEW_FIELDS + ["label_notes_v6"]:
            if str(row.get(field, "")).strip():
                errors.append({"row_number": row_number, "blind_review_id": row.get("blind_review_id"), "field": field, "error_type": "review_field_already_filled"})
        if not visible_packet_paths_exist(row, label_path_base):
            errors.append({"row_number": row_number, "blind_review_id": row.get("blind_review_id"), "error_type": "visible_packet_path_missing"})
    return errors
...
def endpoint_identity(row: dict[str, str]) -> str:
    ...
    labels = {norm_label(row.get("subject_label", "")), norm_label(row.get("object_label", ""))}
    if labels & GENERIC_LABELS:
        return "uncertain"
    return "clear"


def support_pose_answer(predicate: str, subject: str, obj: str) -> tuple[str, str, str]:
    pred = norm_label(predicate)
    sub = norm_label(subject)
    ob = norm_label(obj)
    if sub == ob:
        return "ambiguous", "not_a_relation", "predicate_definition_ambiguous"
    if ob in STRUCTURAL_LABELS and sub not in visible_fill.WALL_ATTACHMENT_LIKE:
        return "supports", "trivial_or_redundant", "trivial_room_surface_or_structure"
    if ob not in SUPPORT_SURFACES:
        return "ambiguous", "uncertain", "ambiguous_contact"
    if pred == "lying on":
        if sub in LYING_LIKE:
            return "supports", "useful_nontrivial", ""
        if sub in STANDING_LIKE:
            return "contradicts", "useful_nontrivial", ""
        return "ambiguous", "uncertain", "ambiguous_contact"
    if pred == "standing on":
        if sub in STANDING_LIKE:
            return "supports", "useful_nontrivial", ""
        if sub in LYING_LIKE:
            return "contradicts", "useful_nontrivial", ""
        return "ambiguous", "uncertain", "ambiguous_contact"
    return "ambiguous", "uncertain", "predicate_definition_ambiguous"


def vertical_relation_answer(predicate: str, subject: str, obj: str) -> tuple[str, str, str]:
    geometry, _mesh, usefulness, uncertainty = visible_fill.vertical_answer(predicate, subject, obj)
    mapped_geometry = {
    ...
    mapped_usefulness = {
        ...
        "ontology_mismatch": "useful_nontrivial",
        "uncertain": "uncertain",
    }.get(usefulness, "uncertain")
    mapped_uncertainty = {
        "ambiguous_relation": "ambiguous_vertical_order",
        "ontology_mismatch": "",
        "trivial_dense_relation": "",
        "none": "",
    }.get(uncertainty, "ambiguous_vertical_order")
    return mapped_geometry, mapped_usefulness, mapped_uncertainty


def answer_axes(row: dict[str, str]) -> dict[str, str]:
    ...
    if family == "relative_vertical":
        geometry, usefulness, uncertainty = vertical_relation_answer(row.get("predicate_label", ""), row.get("subject_label", ""), row.get("object_label", ""))
    elif family == "support_contact":
        geometry, usefulness, uncertainty = support_pose_answer(row.get("predicate_label", ""), row.get("subject_label", ""), row.get("object_label", ""))
    else:
        geometry, usefulness, uncertainty = "ambiguous", "uncertain", "predicate_definition_ambiguous"

    if geometry == "supports" and usefulness == "useful_nontrivial":
        reliability = "accept_reliable"
        primary = "geometric_support"
        pair_eval = "evaluable"
        uncertainty_reason = ""
    elif geometry == "contradicts":
        reliability = "reject_unreliable"
        primary = "geometric_contradiction"
        pair_eval = "evaluable"
        uncertainty_reason = ""
    elif usefulness == "trivial_or_redundant":
        reliability = "reject_unreliable"
        primary = "trivial_room_surface_or_structure"
        pair_eval = "evaluable"
        uncertainty_reason = ""
    elif usefulness == "not_a_relation":
        reliability = "reject_unreliable"
        primary = "semantic_ontology_mismatch"
        pair_eval = "evaluable"
        uncertainty_reason = ""
    else:
        reliability = "abstain_uncertain"
        primary = "insufficient_evidence"
        pair_eval = "predicate_ambiguous"
        uncertainty_reason = uncertainty or ("ambiguous_contact" if family == "support_contact" else "ambiguous_vertical_order")

    return {
        "endpoint_identity_v6": endpoint,
        "pair_evaluability_v6": pair_eval,
        "geometry_support_v6": geometry,
        "relation_usefulness_v6": usefulness,
        "relation_reliability_state_v6": reliability,
        "primary_reason_v6": primary,
        "uncertainty_reason_v6": uncertainty_reason,
    }


def fill_row(row: dict[str, str]) -> dict[str, Any]:
    filled: dict[str, Any] = dict(row)
    filled.update(answer_axes(row))
    filled["reviewer_id"] = REVIEWER_ID
    filled["review_round"] = REVIEW_ROUND
    filled["label_notes_v6"] = (
        "codex proxy v8 repair visible-only fill; used visible subject/object/predicate labels, "
        "predicate family question, cue text, and ready evidence packet availability; did not use "
        "hidden semantic score/rank, p_geom_valid, geometry status, source queue, label geometry "
        "bucket, label-match status, endpoint-pair metadata, pair id, or target-construction metadata"
    )
    return filled


def validate_filled_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    errors: list[dict[str, Any]] = []
    for row_number, row in enumerate(rows, start=2):
        ...
        for field in REVIEW_FIELDS:
            ...
            elif field in ALLOWED_REVIEW_VALUES and value not in ALLOWED_REVIEW_VALUES[field]:
...
def label_record(row: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": "h002_reliability_target_v8_endpoint_pair_counterfactual_repair_proxy_label_v1",
        ...
        "provenance": {
            "batch_name": "reliability_target_v8_endpoint_pair_counterfactual_repair_label_fill",
            ...
            "used_semantic_geometry_bucket_for_label_decision": False,
            ...
            "used_endpoint_pair_metadata_for_label_decision": False,
...
def multiclass_target_rows(filled_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in filled_rows:
        ...
        rows.append(
            {
                "schema_version": "h002_reliability_target_v8_endpoint_pair_counterfactual_repair_multiclass_target_v1",
                "blind_review_id": row["blind_review_id"],
                "scan_id": row["scan_id"],
                "scene_context_id": row["scene_context_id"],
                "subject_id": row["subject_id"],
                ...
                "target_source": "codex_proxy_user_requested_visible_only_v8_repair_label_fill",
                "split": "train_only",
                "paper_evidence_allowed": False,
            }
        )
...
def binary_target_rows(filled_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in filled_rows:
        ...
        rows.append(
            {
                "schema_version": "h002_reliability_target_v8_endpoint_pair_counterfactual_repair_binary_target_v1",
                "blind_review_id": row["blind_review_id"],
                "scan_id": row["scan_id"],
                "scene_context_id": row["scene_context_id"],
                "subject_id": row["subject_id"],
...
def geometry_support_target_rows(filled_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for row in filled_rows:
        value = row["geometry_support_v6"]
        if value not in {"supports", "contradicts"}:
            continue
        rows.append(
            {
                "schema_version": "h002_reliability_target_v8_endpoint_pair_counterfactual_repair_geometry_support_target_v1",
                "blind_review_id": row["blind_review_id"],
                "predicate_label": row["predicate_label"],
                "predicate_family": row["predicate_family"],
                "geometry_support_v6": value,
                "geometry_support_v6_binary": 1 if value == "supports" else 0,
                "split": "train_only",
                "paper_evidence_allowed": False,
            }
        )
...
def usefulness_target_rows(filled_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for row in filled_rows:
        value = row["relation_usefulness_v6"]
        if value not in {"useful_nontrivial", "trivial_or_redundant", "not_a_relation"}:
            continue
        rows.append(
            {
                "schema_version": "h002_reliability_target_v8_endpoint_pair_counterfactual_repair_usefulness_target_v1",
                "blind_review_id": row["blind_review_id"],
                "predicate_label": row["predicate_label"],
                "predicate_family": row["predicate_family"],
                "relation_usefulness_v6": value,
                "relation_usefulness_v6_binary": 1 if value == "useful_nontrivial" else 0,
                "split": "train_only",
                "paper_evidence_allowed": False,
            }
        )
...
def manifest_counter(manifest_rows: list[dict[str, Any]], group_key: str) -> dict[str, int]:
    counts = Counter(diagnostic_group_value(row, group_key) for row in manifest_rows)
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "- Label decisions use only labeler-visible identity, predicate/family question, cue text, and packet availability.",
        "- Hidden semantic/geometry score, bucket, rank, geometry status, source queue, endpoint-pair metadata, and target-construction metadata are not used before label lock.",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    fill_errors = validate_filled_rows(filled_rows)
    ...
    geometry_targets = geometry_support_target_rows(filled_rows)
    usefulness_targets = usefulness_target_rows(filled_rows)
    ...
    geometry_target_counts = Counter(row["geometry_support_v6_binary"] for row in geometry_targets)
    usefulness_target_counts = Counter(row["relation_usefulness_v6_binary"] for row in usefulness_targets)

    output_paths = {
        "summary": output_dir / "summary.json",
        "report": output_dir / "report.md",
        "completed_sheet": output_dir / "completed_v8_repair_label_sheet_codex_proxy_user_requested.tsv",
        "repair_proxy_labels": output_dir / "repair_proxy_labels.jsonl",
        ...
        "geometry_support_v6_binary_targets": output_dir / "geometry_support_v6_binary_targets.jsonl",
        "relation_usefulness_v6_binary_targets": output_dir / "relation_usefulness_v6_binary_targets.jsonl",
    ...
    next_todo = "fix_reliability_target_v8_endpoint_pair_counterfactual_repair_label_fill" if errors else "reliability_target_v8_endpoint_pair_counterfactual_repair_label_ingestion"
    decision = (
        "Input or fill validation errors block ingestion."
        if errors
        else (
            "Filled the 200-row v8 repair endpoint-pair counterfactual sheet as a user-requested Codex proxy. "
            "This creates hypothesis-stage multiclass labels and binary diagnostic target rows for ingestion "
            "and target-independence audit, but it is not paper metric evidence and does not unlock posterior "
            "smoke by itself."
        )
    )
    ...
    summary = {
        ...
        "counts": {
            ...
            "label_geometry_bucket_post_label_only": manifest_counter(manifest_rows, "label_geometry_bucket_hidden"),
            "source_queue_post_label_only": manifest_counter(manifest_rows, "source_queue_hidden"),
            "geometry_status_post_label_only": manifest_counter(manifest_rows, "geometry_status_hidden"),
            ...
            "geometry_support_target_rows": len(geometry_targets),
            "geometry_support_positive_rows": geometry_target_counts.get(1, 0),
            "geometry_support_negative_rows": geometry_target_counts.get(0, 0),
            "usefulness_target_rows": len(usefulness_targets),
            "usefulness_positive_rows": usefulness_target_counts.get(1, 0),
            "usefulness_negative_rows": usefulness_target_counts.get(0, 0),
        },
    ...
    write_jsonl(output_paths["repair_proxy_labels"], label_rows)
    ...
    write_jsonl(output_paths["geometry_support_v6_binary_targets"], geometry_targets)
    write_jsonl(output_paths["relation_usefulness_v6_binary_targets"], usefulness_targets)
