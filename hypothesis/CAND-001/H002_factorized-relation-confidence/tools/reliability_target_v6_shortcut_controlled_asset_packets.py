#!/usr/bin/env python3
"""Generate packets for H002 v6 shortcut-controlled candidate rows."""
...
CANDIDATE_DIR = RGA_ROOT / "reliability_target_v6_shortcut_controlled_candidate_mining_codex_proxy_user_requested"
...
DEFAULT_PREVIOUS_MANIFEST = CANDIDATE_DIR / "v6_shortcut_controlled_manifest_post_label_only.jsonl"
DEFAULT_ASSET_REQUESTS = CANDIDATE_DIR / "asset_request_plan.jsonl"
DEFAULT_OUTPUT_DIR = RGA_ROOT / "reliability_target_v6_shortcut_controlled_asset_packets_codex_proxy_user_requested"
DEFAULT_SCAN_ROOT = REPO_ROOT / "local_dataset/3RScan/scans"

SCHEMA_VERSION = "h002_reliability_target_v6_shortcut_controlled_asset_packets_v1"
REVIEW_SCOPE = "h002_reliability_v6_shortcut_controlled_packeted_review"
GENERATED_PACKET_SOURCE = "generated_v6_shortcut_controlled_asset_packet"
EXISTING_PACKET_SOURCE = "existing_independent_or_prior_asset_packet"
MISSING_PACKET_SOURCE = "missing_v6_shortcut_controlled_asset_packet"
...
def full_manifest_row(seed: dict[str, Any], previous_manifest_by_id: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    row["batch_name"] = "reliability_target_v6_shortcut_controlled_asset_packets"
    row["schema_version"] = "h002_reliability_target_v6_shortcut_controlled_asset_packet_manifest_v1"
...
def bucket_summary_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        by_cell[(str(row.get("predicate_family")), str(row.get("candidate_bucket_hidden")))].append(row)
...
def validate_inputs(
    ...
    errors: list[dict[str, Any]] = []
    if candidate_summary.get("next_todo") != "reliability_target_v6_shortcut_controlled_asset_packets":
    ...
    selected_cells = Counter((str(row.get("predicate_family")), str(row.get("candidate_bucket_hidden"))) for row in seeds)
    for family in mining.PRIMARY_FAMILIES:
        for bucket in mining.BUCKET_ORDER:
            if selected_cells[(family, bucket)] != 30:
                errors.append(
                    {
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    lines = [
        "# H002 Reliability Target V6 Shortcut-Controlled Asset Packets",
        "",
        ...
        "- Candidate bucket, semantic score/rank, geometry score/status, and source queue metadata remain hidden.",
        "",
        "## Status",
        "",
        f"`{summary['status']}`",
    ...
    for key, value in counts["by_family"].items():
        lines.append(f"| `{key}` | {value} |")
    lines.extend(["", "## Bucket Counts", "", "| Bucket | Rows |", "| --- | ---: |"])
    for key, value in counts["by_bucket"].items():
        lines.append(f"| `{key}` | {value} |")
    lines.extend(
        [
            "",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    updated_seeds = [
        update_seed_with_packet(row, generated_by_id.get(str(row["blind_review_id"])))
    ...
    label_ready_manifest_rows = [
        row for row in manifest_rows if row.get("evidence_packet_status") == "ready"
    ]
    generated_manifest_rows = [
        row for row in manifest_rows if row.get("asset_packet_source_hidden") == GENERATED_PACKET_SOURCE
    ]
    ...
    bucket_counts = Counter(str(row.get("candidate_bucket_hidden")) for row in updated_seeds)
    family_bucket_counts = Counter(
        f"{row.get('predicate_family')}|{row.get('candidate_bucket_hidden')}" for row in updated_seeds
    )
    ...
    status = (
        "h002_reliability_target_v6_shortcut_controlled_asset_packets_ready_for_label_readiness"
        if all_ready and not generated_non_ready and not path_errors and not leakage_hits and not validation_errors
        else "h002_reliability_target_v6_shortcut_controlled_asset_packets_partial_needs_gap_audit"
        if status_counts.get("ready", 0) > 0 and not leakage_hits and not validation_errors
        else "h002_reliability_target_v6_shortcut_controlled_asset_packets_blocked"
    )
    next_todo = (
        "reliability_target_v6_shortcut_controlled_label_readiness"
        if status == "h002_reliability_target_v6_shortcut_controlled_asset_packets_ready_for_label_readiness"
        else "reliability_target_v6_shortcut_controlled_asset_packet_gap_audit"
        if status == "h002_reliability_target_v6_shortcut_controlled_asset_packets_partial_needs_gap_audit"
        else "fix_reliability_target_v6_shortcut_controlled_asset_packets"
    )
    decision = (
        "The full 240-row v6 shortcut-controlled sheet is packet-complete and can proceed to label-readiness checks."
        if status == "h002_reliability_target_v6_shortcut_controlled_asset_packets_ready_for_label_readiness"
        else "V6 shortcut-controlled packets are partial. Inspect non-ready rows or missing paths before label fill."
        if status == "h002_reliability_target_v6_shortcut_controlled_asset_packets_partial_needs_gap_audit"
        else "V6 shortcut-controlled packet generation is blocked by validation, leakage, or path errors."
    )

    output_paths = {
        "summary": output_dir / "summary.json",
        "report": output_dir / "report.md",
        "full_label_sheet": output_dir / "v6_shortcut_controlled_full_label_sheet.tsv",
        "label_ready_sheet": output_dir / "v6_shortcut_controlled_label_ready_sheet.tsv",
        "full_manifest_post_label_only": output_dir / "v6_shortcut_controlled_full_manifest_post_label_only.jsonl",
        "label_ready_manifest_post_label_only": output_dir / "v6_shortcut_controlled_label_ready_manifest_post_label_only.jsonl",
    ...
    summary = {
        ...
        "counts": {
            ...
            "full_manifest_rows": len(manifest_rows),
