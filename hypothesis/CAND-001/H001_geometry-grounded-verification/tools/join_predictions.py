#!/usr/bin/env python3
"""Join baseline prediction rows with H001 geometry evidence.

The join is intentionally row-preserving: every prediction row receives one
verification row, including predicates outside the current H001 geometry scope.
"""
...
from export_calibration import compute_features, load_scan_geometries
from export_point_support import (
    DEFAULT_POINT_THRESHOLDS,
    assign_point_status,
    compute_object_stats,
    local_support_stats,
)
from fit_calibration import dot, raw_numeric_features, sigmoid, vectorize
from apply_verifier_v2 import (
...
HYPOTHESIS_ROOT = SCRIPT_PATH.parents[1]
...
SCHEMA_VERSION = "h001_prediction_geometry_v1"
POLICY_NAME = "h001-prediction-join-v0"
G2_POLICY_NAME = "h001-prediction-join-g2"
...
def main() -> int:
    ...
    manifest = {
        "schema_version": "h001_prediction_geometry_manifest_v1",
        ...
        "notes": [
            ...
            (
                ...
                else "G2 emits obb_only, point_subtype, and point_subtype_no_soft_support variants."
            ),
        ],
    }
