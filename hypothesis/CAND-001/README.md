...
- Paper handoff is ready in `paper/preview.md`, bilingual `paper/outline.md`, reviewed first-pass `paper/draft.md`, `paper/figures.md`, and verified/layout-reviewed draft SVGs under `paper/generated/figures/`; title candidates, three contribution statements, abstract skeleton, Introduction logic, table/figure caption drafts, claim-consistency review, related-work positioning, problem/method formalization, Figure 1-3 asset plan/source lock, table/appendix placement, limitation/reviewer-defense prose skeletons, first-pass manuscript prose, draft claim/evidence review, draft Figure 1-3 generation, and top-tier novelty/layout figure review are now fixed, with cross-source results/failure analysis treated as empirical validation. The next drafting step is to replace Related Work citation placeholders in `paper/draft.md`. Table 6/Open3DSG caveat compression is deferred until the paper-body logic is stable. Do not change the locked taxonomy without schema version bump.
