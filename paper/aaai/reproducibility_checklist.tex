...
yes. The contribution is limited to two exact guarantees for the proposed
projection and routing operators; it is not a statistical generalization bound.
...
yes. Proofs of both propositions are included in the supplement.
...
NA. The proofs use only finite-group re-indexing and a direct pairwise exchange argument.

\question{All theoretical claims are demonstrated empirically to hold}{(yes/partial/no/NA)}
yes. Orbit errors, family composition, and support/contact selection preservation are checked on every evaluation row and reported.
