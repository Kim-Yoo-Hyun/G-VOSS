...
counterfactual negatives. The predicate-family map, hard-rule thresholds,
...
prediction rows are not used to retune them. We use \pgeom{} as a soft
geometry-risk term, with $R^{geom}_i=-\log \pgeom_i$ and utility
\begin{equation}
U_{\lambda}(i)=\log \mathrm{score}^{sem}_i-\lambda R^{geom}_i.
\end{equation}
The reported probabilistic operating point fixes $\lambda=1$, giving
\begin{equation}
...
We report \texttt{semantic\_only}, \texttt{probabilistic\_recalibrated},
\texttt{rule\_verified\_point\_subtype}, and
\texttt{family\_conditional\_risk}. The last ranks by
$\mathrm{score}^{sem}_i \cdot \pgeomfam_i$, where
$\pgeomfam_i$ is produced by a frozen relation-family-specific
calibrator, testing whether support/contact, proximity, and vertical relations
should share one pooled geometry-risk surface.
