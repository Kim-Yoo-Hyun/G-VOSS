...
\sysname{} is a \method{} framework for 3D scene graph relation predictions. It is
...
\sysname{} uses relation-family-specific checks for \texttt{support\_contact},
...
The checks do not create new relation labels and do not relax exact-label
recall into family-level matches. They only define whether a source-predicted
row is physically consistent, inconsistent, or unsupported for the measured
families.
...
prediction rows are not used to retune them. The main probabilistic re-ranking
score is
\begin{equation}
